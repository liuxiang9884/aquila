...
- 当前已落地 V1a dry-run scaffold 和 live sample 前置纯逻辑：`tools/gate/order_session_rtt_probe/`、
  ...
  CSV schema / writer；sample flow 已保存每个 Ack 的 `*_ack_receive_local_ns` 和 stage status，并校验 Ack
  `local_order_id` 必须匹配当前 stage 的 expected id；
...
- live safety path 设计上优先走 WebSocket reduce-only market close：GTC cancel reject / terminal 不确定时立即补 close；IOC
  place Ack 后因没有 cancel，也立即补 close。无仓位时 Gate reduce-only close 可能被拒绝，该 reject 属于预期 flat-safe
  结果，不作为 probe 失败。当前代码仍是 Ack-driven sample executor，尚未接入 feedback reader、terminal feedback、
  cancel reject safety transition、close terminal 确认或 REST guard；这些是启用 `--execute` 前的 blocker。
...
当前 sample executor 只处理 order Ack path：它可以记录 GTC place / GTC cancel / IOC place Ack RTT，并保存 Ack 接收时间和
stage status；但还没有把 cancel final reject、terminal uncertainty、feedback fill、safety close terminal feedback 和
REST guard 串入 live 状态机。下一步启用真实下单前，必须先让 cancel reject / timeout / fill 进入 reduce-only close，
并且 close Ack 后继续等待 terminal feedback 或 REST/position-known-flat 证明安全，不能只凭 close Ack 结束 sample。
