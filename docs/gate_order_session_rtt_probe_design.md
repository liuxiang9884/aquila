...
- live safety path 设计上优先走 WebSocket reduce-only market close：GTC cancel reject / terminal 不确定时立即补 close；IOC
  ...
  Ack-driven sample executor，已接入 GTC cancel reject 的 immediate close 派发，但尚未接入 feedback reader、terminal
  feedback、timeout / fill safety transition、close terminal 确认或 REST guard；这些是启用 `--execute` 前的 blocker。
...
stage status；同时已处理 GTC cancel final reject，收到后立即派发 GTC reduce-only close，且 safety close reject 在没有
flat 证明时会让 sample 失败。下一步启用真实下单前，仍必须把 terminal uncertainty、timeout、feedback fill、safety close
terminal feedback 和 REST guard 串入 live 状态机；close Ack 后必须继续等待 terminal feedback 或 REST/position-known-flat
证明安全，不能只凭 close Ack 结束 sample。
