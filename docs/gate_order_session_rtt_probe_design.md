...
- 对齐率：`1798 / 1798` samples。
- `>10ms` tail count：`12`。
- `>10ms` 的 `pcap_request_to_ack_ms` 总和：`200.750ms`。
- `>10ms` 的 `gate_x_in_to_x_out_ms` 总和：`193.369ms`。
- `>10ms` Gate duration share：`96.32%`。
- `>10ms` residual p50 / p95 / max：`0.604ms / 0.790ms / 0.799ms`。
- `>10ms` by session：`private-00:1`、`private-01:6`、`private-02:2`、`private-04:1`、`private-05:2`；
  Gate `conn_id` 分布在 5 个连接上，不是单个本地 session / Gate connection 孤立问题。
- 该次 pcap 按 `10.0.1.154` 过滤，summary 中 `by_group` / `by_ip` 都是
  `private-10.0.1.154` / `10.0.1.154`，不能用这一份 pcap 比较不同 private IP。

因此，本轮 `>10ms` tail 主要由 Gate 同一时钟域内的 `x_in_time -> x_out_time` 解释，范围进一步收窄到
Gate 收到 request 后、发出 submit Ack response 前的 Gate edge / app / order path 内部阶段。`5ms-10ms`
...
| `L5` | 可结合 no TLS pcap 和 runtime Gate `x_in_time` / `x_out_time` 做离线对齐分析，计算 residual / Gate share。 |
...
python3 scripts/gate/analyze_order_session_rtt_pcap.py \
  ...
  --local-ip <local_ip> \
  --remote-ip <connect_ip> \
  --output /home/liuxiang/tmp/<run_id>/gate_x_time_alignment.csv \
  --summary-output /home/liuxiang/tmp/<run_id>/gate_x_time_summary.txt
```
...
- `ack_response_to_ack_receive_us` 大：才回头查本机 RX、read、parse 或 owner thread。
