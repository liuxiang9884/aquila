...
| `./build/debug/tools/data_reader_recorder` | 用 `RealtimeDataReader::Drain()` 从 Gate / Binance SHM 导出连续 `aquila::BookTicker` binary；如果配置包含 trade source，也会写独立 `aquila::Trade` binary。 |
