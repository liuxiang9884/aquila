...
普通 Aquila 订单的 `clientOid` 固定为 29 字符：

```text
a1-<12-char Crockford Base32 run namespace>-<13-char uppercase Base36 local_order_id>
```

字段 offset 固定为 `[0..2] schema`、`[3..14] run namespace`、`[15] separator`、
`[16..28] local order id`。`local_order_id` 仍是进程内 `uint64_t`，wire suffix 左侧补零且必须无损
round trip；不修改 OrderPool、SHM ABI 或 strategy lane。Fresh-run prepare 每轮从 OS CSPRNG 取一次完整
60 bit namespace，使用大写 Crockford alphabet `0123456789ABCDEFGHJKMNPQRSTVWXYZ`。不建立注册表或查重；
一百万个 run 至少发生一次随机碰撞的近似概率约为 `4.3e-7`，因此这是概率唯一性，不是数学保证。

Checked-in order/feedback session config 使用不可下单的保留模板 `000000000000`。Login-only/dry-run 可以读取模板；
gateway/feedback `--connect`、RTT live preflight/execute 和 place/cancel encoder 都拒绝模板。LeadLag 与 gateway-smoke
prepare 必须把同一个 fresh namespace 写入 manifest、所有 route 的 run-scoped OrderSession config 和 feedback config，
任一缺失、digest 变化或值不一致都 fail closed。

Request id 用于 operation response correlation，`clientOid` 用于跨 run 订单身份、生命周期和 feedback routing。两者不能
互相替代。Ack 只有同时匹配 request id、当前 run namespace 和 `local_order_id` 才能完成 inflight correlation。
Place/cancel 的 encoder 使用固定容量 buffer；namespace/Base36 格式化不分配内存，热路径不得为 JSON 格式化引入无界动态分配。
...
Parser 先按固定 offset 分类 `clientOid`，再解析必需字段和 scope：

- 当前 namespace 的合法 `a1`：允许继续解析并发布；
- 合法 foreign namespace：记录 `foreign_run_namespace` 诊断后忽略，不触发 continuity failure；
- 历史 `a-<decimal local_order_id>`：记录 legacy 诊断后忽略，不发布；
- 当前 namespace 已完整命中但长度、分隔符、Base36 或数值边界非法：触发既有 continuity failure；
- 非 Aquila 或无法归属当前 namespace 的值：按 foreign/unroutable 忽略。
...
`OrderSession`。Gateway SHM 使用通用 v4 command/event ABI，place/cancel/response 端到端传播 `group_id`；ABI 仍不包含
Bitget numeric error code、connection id 或额外诊断字段。v3/v4 producer 与 consumer 不允许混跑，升级后必须一起重启。
...
仍在运行的 LeadLag / Bitget trading 进程，再生成 `/home/liuxiang/tmp/<run_id>/configs/` 下的五份 overlay 和
...
`guard_summary.json`，另有 manifest、五份 runtime config 和各进程日志。Runner exit `0` 后，pipeline 还会复核 CSV/summary 的
...
V1 不修改 `OrderPool`，所以新进程仍可能复用低 56-bit counter；但 2026-07-24 起普通 Bitget
`clientOid` 已加入 fresh 60-bit run namespace，相同 `local_order_id` 不再确定性复用 wire identity。该修复只提供
跨 run 概率隔离，不提供 crash-state persistence、历史订单自动恢复或同一 run resume；V1 仍明确禁止跨进程恢复后继续交易：
...
- 跨 run 审计链使用固定宽度 `clientOid → namespace/local_order_id → archived manifest/run_id`，不按历史订单自动
  重建状态并恢复开仓；
- 只有未来要支持 overlap、read-only reconcile 后 resume 或 crash-state recovery 时，persistent `local_order_id/clientOid`
  才重新成为前置条件。

`scripts/lead_lag/prepare_bitget_live_run.py prepare` 只生成 `/home/liuxiang/tmp/<run_id>/configs/` 下的 strategy、
gateway、feedback 和逐 route OrderSession overlay，以及 `bitget_live_manifest.json`。它逐 route 把 gateway 的
`order_session_config` 与 strategy 的 LeadLag config 改写为绝对路径，并把同一个 fresh namespace 写入全部交易 session；
该命令不连接交易所、不读取账户、不创建 SHM。Operator 确认 feedback 与 manifest 对应 gateway
确实使用 manifest 中配置后，以两个实际 PID 执行 `mark-applied --gateway-pid <pid> --feedback-pid <pid>`。manifest schema v3
记录 `client_oid_schema=a1`、run namespace、全部生成配置的路径和 SHA-256；验证会重新读取所有 TOML，并复核 digest、
namespace exact equality、路径、SHM、`route_count=1/4`、逐 route 三字段凭据、category、position/margin mode 和 private
