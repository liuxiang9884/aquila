...
- 四路 LeadLag gateway：`config/order_gateways/bitget_order_gateway_4routes.toml` 启动四条独立 private
  ...
  `order_session_fanout=4` 且 `require_min_entry_quantity=true`，否则在生成 runtime manifest 时 fail closed。
...
child，四张各自拥有 `local_order_id/route_id`，整个 group 只占一个 `parallel` slot。开启
`require_min_entry_quantity=true` 后，策略使用整数 quantity units 在选择 route 和提交第一张 child 前证明 entry quantity
恰好等于 instrument `min_quantity`；否则输出 `reason=entry_quantity_not_minimum` 并整组拒绝。Risk reservation 按实际
ready route 数乘以单 child 暴露。Normal close、stoploss 和 retry 不受 entry-min gate 限制，仍向每条 ready route 发送
当前 group 已知总仓位的 full-size `reduce-only` child，由交易所净仓限制防止反向开仓。
...
WebSocket endpoint 一致性；四路时还要求所有 Bitget pair 的 `order_session_fanout=4` 和
`require_min_entry_quantity=true`。同时从 `/proc` 绑定 PID、start time、executable、`--connect` 与实际绝对 `--config`。
