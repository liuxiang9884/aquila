...
`/home/liuxiang/tmp/bitget_gateway_smoke_20260714T061702Z/`。因此 fanout=1 gateway 证据门已通过；下一门是
signal-conditioned LeadLag，尚无 Bitget LeadLag 真实订单证据。Gateway 单笔零成交结果不能外推 fillability、latency、多 route
或长期稳定性。Dedicated-account flat、余额、IP 白名单和 endpoint 可用性都是当次运行事实，不得外推为永久状态。
...
截至 2026-07-14，tiny-position stop-and-flat 与 fanout=1 gateway 两门已有上述当次证据，LeadLag 门仍未执行。任一对应证据门
缺失时，不得宣称其 live safety、fillability 或 latency 已验证；本次 gateway 零成交也不构成 gateway fillability 结论。
