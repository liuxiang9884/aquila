...
- 同一当前二进制中，accepted event-only / raw diagnostic-field parser 中位数约为 `763ns / 1034ns`；原始字段解析增加约
  `271ns`，且尚未包含 structured log 格式化/写入成本。后续 live 必须观察 owner-thread CPU 和尾延迟。
