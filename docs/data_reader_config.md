...
本文只定义 realtime/historical reader、poll/drain、typed binary recorder 与 probe 配置。历史 live smoke 流水不在配置参考中维护；
当前 typed SHM ABI 见 `docs/data_session_shm_communication_design.md`。
...
`drain`、recorder 和 historical replay 必须使用当前 typed binary format v1；旧 raw struct、旧 64-byte `BookTicker` 或旧 clock-domain artifact 需要重录。Live 验证应记录 run id、source per-feed count、`skipped`、`overruns`、header feed/type、ABI 和退出原因，临时输出写入 `/home/liuxiang/tmp`。
