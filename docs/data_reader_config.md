...
- historical / replay binary source：一个 `binary_file` source 必须显式写 `feed = "book_ticker"` 或 `feed = "trade"`
...
typed binary format v1 `BookTicker` 或 `Trade` 文件，适合 replay / 对账。每个 `.bin` 文件以 16-byte
`MarketDataBinaryHeader` 开头，随后是连续 payload records。`HistoricalDataReader` 第一版仍只接受一个
`binary_file` source，不做 `BookTicker` / `Trade` mixed replay 或跨 feed 排序。`HistoricalDataReader`
和 Python numpy 脚本不再接受旧 raw struct streams；旧 artifact 需要用新版 recorder 或 converter 重录 / 重转。
...
Trade binary replay / probe 示例：

```toml
[data_reader]
name = "trade_binary_probe"
max_events_per_drain = 4096
...
feed = "trade"
files = [
  "/home/liuxiang/tmp/run/recorded_trade.bin",
]
start_position = "earliest_visible"
read_mode = "drain"
required = true
...
`scripts/market_data/compare_fusion_tardis_book_ticker.py`。该脚本按 typed `BookTicker` binary 读取
...
| `data_reader.sources.files` | 无，binary file 必须显式配置 | `BookTicker` 或 `Trade` 二进制文件列表，按配置顺序读取。SHM 不使用。 |
...
`binary_file` source 必须显式写 `feed`，并使用 `start_position = "earliest_visible"` 和
`read_mode = "drain"`。SHM source 仍默认 `book_ticker`。第一版 `HistoricalDataReader` 只接受一个
`binary_file` source；多日 / 分片 replay 用同一个 source 下的多个 `files` 表达，跨 source merge
必须在离线数据程序中预先完成。该 source 的 `feed` 可为 `book_ticker` 或 `trade`，但不能同时混合两类 feed。

所有 `binary_file` 输入都必须是 typed binary format v1：每个 `.bin` 文件以 16-byte
`MarketDataBinaryHeader` 开头，随后是连续 payload records。构造期会检查文件存在、大小至少为 16 bytes、
header `magic` / `version` / `header_size` / `flags`、header feed/type 与 TOML `feed` 一致、`record_size`
等于对应 payload ABI size，并校验 `(file_size - 16) % record_size == 0`。文件大小和 manifest `bytes`
都包含 header；record 数量为 `(file_size - 16) / record_size`。0-byte 文件无效；header-only 文件表示
0 records，是合法的已完成输入。`HistoricalDataReader` 和相关 Python numpy 脚本不再接受旧 raw struct
streams。通过校验后，reader 在构造期完成 read-only mmap；`Poll()` / `Drain()` 热路径只按 cursor 从 payload
区域拷贝 `BookTicker` 或 `Trade` 并推进状态，不打开文件、不抛出异常。文件读完后 `Poll()` 返回 0 且
`finished() == true`；外层 runtime 需要在 idle hook 中主动停止 replay loop。
...
- recorder / probe 启动日志会打印 `book_ticker_abi_size=sizeof(aquila::BookTicker)` 和 `trade_abi_size=sizeof(aquila::Trade)`。typed binary header 的 `record_size` 与对应 ABI size 一致；SHM attach 仍会校验 producer header 中的 ABI size。
- 单文件写入路径使用二进制追加或截断模式，由 `--mode` 明确控制，默认截断；rotation 模式只支持截断。
- rotation manifest 每行一个 JSON object，记录 segment `sequence`、`file`、`records`、`bytes`、typed metadata `format`、`version`、`feed`、`header_bytes`、`record_size`、first / last `exchange_ns`、first / last `local_ns` 和 `closed_reason`。manifest `bytes` 包含 header；脚本可用 manifest typed metadata 做 preflight，但二进制文件 header 仍是事实源。正在写的 `.tmp` 不写入 manifest；BookTicker replay 只读取 BookTicker manifest 中已关闭的 `.bin`。
...
- 本地测试 `data_reader_recorder_test` 覆盖两个 `BookTicker` SHM source 写入同一个 typed binary output、mixed `BookTicker` / `Trade` SHM source 写入两个独立 typed binary output，以及双 feed typed rotation segment / manifest 的路径。

BookTicker / Trade rotation replay 使用同一个 `HistoricalDataReader` 单 source 模型。现有 `binary_file`
source 已支持一个 source 下多个 `files`；可用 manifest 生成 replay data reader TOML：
...
`data_reader_probe`、`lead_lag_replay` 或其他 `HistoricalDataReader` 使用方。Trade manifest 需要显式加
`--feed trade`：
...
生成的 Trade TOML 用于 `data_reader_probe` 或其他显式消费 `OnTrade()` 的工具；当前不要直接喂给
`lead_lag_replay`，因为 LeadLag replay 策略尚未实现 trade signal 消费。
...
`data_reader_recorder` 写出单个 `BookTicker` replay binary。该样例生成于 typed binary format v1 前；当前实现已迁移，旧 artifact 需要重录。临时配置、日志和输出均在：
...
recorder 由 `timeout` 发送 SIGTERM 后正常收尾。该旧样例生成于当前 `event_ns` ABI 之前，当时
`BookTicker` payload 为 64 bytes；当前 `sizeof(aquila::BookTicker)=72`，旧 raw / 旧 ABI artifact
需要重录。recorder summary：
...
```text
result=ok mode=historical stop_reason=finished polls=4 handler_book_tickers=15685 handler_trades=0 diagnostics_total_count=15685 files_completed=1
```

结论：本次 live record smoke 中 Gate / Binance 两个 source 都有记录；该历史输出曾可被当时的
`HistoricalDataReader` 顺序读完。当前 `HistoricalDataReader` 只接受 typed binary format v1，旧 artifact
需要重录；两个 live SHM source 的 `skipped=0`、`overruns=0`。
