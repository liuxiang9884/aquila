...
截至优化代码 checkpoint `bcdc358`，本阶段共接受 10 项生产代码优化：
...
| `bcdc358` | Strategy/Gateway/SHM/OrderSession 共享 numeric `OrderPlaceRequest`，只在 session 生成 price/quantity text | no-log SHM 整链 Gate/Bitget `p50` 分别改善 `12.90%/14.33%`，均为 5/5 组同向；direct 整链分别改善 `11.15%/11.73%`，均为 5/5 组同向 | 采用 |
...
| Gate numeric request strategy → SHM → OrderSession write | `p50 2151.5 → 1874 ns` | `-12.90%` | `d829337` baseline 与 `bcdc358` candidate 的五组 no-log 同起止点 A/B，5/5 组同向；该测量面与 gateway-only submit 不同，不做百分比叠加 |
| Bitget numeric request strategy → SHM → OrderSession write | `p50 2013.5 → 1725 ns` | `-14.33%` | `d829337` baseline 与 `bcdc358` candidate 的五组 no-log 同起止点 A/B，5/5 组同向；该测量面与 gateway-only submit 不同，不做百分比叠加 |
...
### 2.10 `bcdc358`：numeric order request 与 OrderSession 格式化

Strategy 不再为每个 pending order 保存 price/quantity text。`OrderPlaceRequest` 直接携带
`double price`、`double quantity`、instrument catalog 已确定的 decimal places 和
`char symbol[32]`；Direct Gateway 与 SHM V3 使用同一 POD request，Gate/Bitget
OrderSession 在单次 encoder 调用的栈 buffer 中生成 fixed-decimal view，再通过
`FMT_COMPILE` 的固定 JSON format 写入最终发送 buffer。`OrderCancelRequest` 只保留
local identity、parent correlation 和 route，session 的 exchange-order-id cache 与 fallback
语义不变。SHM command 从 168 bytes 缩小到 104 bytes，其中 place/cancel payload 分别为
80/24 bytes。

正式 A/B 使用 `d829337` baseline、完成态 candidate、CPU 28、五组交替顺序、每侧每组
10 repetitions、每次 4096 samples。benchmark 只做 logger 初始化并把 level 设为
`critical`；成功路径的 `NOVA_INFO` 在前端被过滤，不进入异步队列、backend 格式化或输出。

SHM 同起止点整链结果：

| 交易所 / 分段 | baseline（median ± MAD） | candidate（median ± MAD） | 提升 | 同向组数 |
| --- | ---: | ---: | ---: | ---: |
| Gate full `p50` | 2151.5 ± 58 ns | 1874 ± 36.5 ns | 12.90% | 5/5 |
| Gate strategy `p50` | 189.5 ns | 111 ns | 41.42% | 5/5 |
| Gate SHM handoff `p50` | 1002.5 ns | 861 ns | 14.11% | 5/5 |
| Gate OrderSession `p50` | 960.5 ns | 896.5 ns | 6.66% | 4/5 |
| Bitget full `p50` | 2013.5 ± 23 ns | 1725 ± 27 ns | 14.33% | 5/5 |
| Bitget strategy `p50` | 188 ns | 110 ns | 41.49% | 5/5 |
| Bitget SHM handoff `p50` | 962 ns | 829 ns | 13.83% | 4/5 |
| Bitget OrderSession `p50` | 860.5 ns | 781 ns | 9.24% | 5/5 |

Direct 同起止点整链 `p50` 中，Gate 从 `1251.5 ± 5.5 ns` 降至
`1112 ± 26 ns`，改善 `11.15%`、5/5 同向；Bitget 从 `1185.5 ± 49 ns` 降至
`1046.5 ± 15.5 ns`，改善 `11.73%`、5/5 同向。

本机 tail 在约 11–12 µs 处受到系统抖动影响。SHM full `p99` 虽然 Gate/Bitget 都是
5/5 同向，direct Gate `p99.9` 却回归 `1.05%` 且仅 2/5 同向；较早测量的 Gate tail
方向也有冲突。因此采用依据是四条整链稳定的 `p50`，不声明稳定 `p99.9` 收益。

采用原因：同起止点 A/B 证明这不是把 Strategy 的 text 成本简单搬到 session。Strategy
删除 text lifetime，SHM command 缩小，且当前 integer-scaled fixed-decimal + compiled
JSON 使 session 本身也改善；Gate/Bitget direct 与 SHM 四条整链 `p50` 全部 5/5 同向。
typed replay 两侧均处理 5,509,187 条 BBO、产生 282 个信号，CSV byte-identical，
SHA-256 为
`b14b486e951fda21a5429b1c36709cc2e707033b2e180bf3bda5989869fdc65c`。

证据目录：
`/home/liuxiang/tmp/aquila-gate-bitget-order-request-format-e2e-results/formal-v11-shm`、
`/home/liuxiang/tmp/aquila-gate-bitget-order-request-format-e2e-results/formal-v12-direct`、
`/home/liuxiang/tmp/aquila-gate-bitget-order-request-format-e2e-results/final-order-session-profile`
和
`/home/liuxiang/tmp/aquila-gate-bitget-order-request-format-e2e-results/final-replay`。
...
| 尝试 | 结果摘要 | 是否采用 | 原因 |
| --- | --- | --- | --- |
| 首版 numeric request：动态 fmt precision / 未优化 formatter，并按不同起止点比较 Strategy、OrderSession 和 GatewayWorker | Gate encoder place `268.8 → 234.2 ns`，但局部 OrderSession / StrategyContext / GatewayWorker `p50` 分别回归 `6.3%/4.1%/3.2%`；Bitget encoder place 回归 `67.8%`，三层局部 submit `p50` 回归 `17.9%/15.9%/11.3%`；LeadLag actual-config 总路径 `p50` 改善约 `6.0%` | 否 | 局部 benchmark 起止点不一致，且动态 precision 明显更慢；首版已撤销。随后建立同起止点 E2E benchmark，并以 integer-scaled fixed-decimal + compiled JSON 重新实现，完成态见 `bcdc358` |

首版候选完成了七个实施部分、focused/full tests 和 typed replay，但当时的 component
screening 不能证明整体回归。该版生产修改已撤销；完整 patch 保存在
...
- CPU：Intel Xeon Platinum 8488C；早期 benchmark 固定 CPU 29，`30e27fe` 和
  `bcdc358` 的 fresh baseline/candidate 固定 CPU 28。
