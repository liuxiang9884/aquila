...
| `reject_reason` | 拒单原因；`intent_rejected_v1` 行可包括 `drift_guard`、`parallel_limit`、`risk_limit`、`stale_lead_quote`、`stale_lag_quote` 和本地下单准备阶段拒绝原因。 | `lead_lag_order_feedback.reject_reason` 或 `lead_lag_order_intent_rejected.reason`。 |
