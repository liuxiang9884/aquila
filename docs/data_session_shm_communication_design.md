...
Gate / Binance data session 通过共享内存把标准化 `aquila::BookTicker` 发布给 strategy、recorder、TUI 和 probe 进程。Gate SBE data session 还可以在同一个 SHM object 中发布标准化 `aquila::Trade`。当前只支持固定大小 typed record，不引入通用 event ring 或 payload pool。
...
Gate data session 可用 slot-writer fast path，直接在 queue producer slot 中解码 `BookTicker` 或
SBE `publicTrade` entry。`Trade` 是 trade 热路径最终目标；生产路径不把 SBE repeating group
先 materialize 成 `std::array`、`std::vector` 或 batch container。
...
reader 不做多 source 全局时间排序。`RealtimeDataReader` 多 source round-robin 只提供公平轮询，不按 `exchange_ns` / `local_ns` merge。当前 strategy `DataReader` 仍只消费 `BookTicker`；`TradeShmReader` 是 Gate SBE public trade 的独立 typed reader 能力，尚未接入 LeadLag 策略主路径。
...
Data session 已采用类似 Gate `OrderSession` Ack latency 的编译期分层诊断，但采样点围绕接收路径设计。目标不是把所有字段写进 `BookTicker`，而是在保持 64-byte `BookTicker` ABI 不变的前提下，只对超过阈值的 BookTicker 写出结构化 Nova log，帮助区分交易所 timestamp 语义、WebSocket / TCP 接收、本机 userspace、SHM publisher 和 reader 侧丢点。
...
| `L1` | outlier correlation | 当 `BookTicker.local_ns - BookTicker.exchange_ns > threshold_ns` 时输出 `data_session_book_ticker_latency_outlier`，字段包括 `exchange`、`source_id`、`symbol_id`、`book_ticker_id`、`latency_ns`。 |
...
2. Gate SBE engine timestamp：如需区分 Gate engine `bbo.t` 与 websocket server send `bbo.time`，可在 L1 log 中增加 `gate_engine_ns` 和 `gate_engine_to_ws_ns`；当前 `BookTicker.exchange_ns` 继续使用 `bbo.time`。
