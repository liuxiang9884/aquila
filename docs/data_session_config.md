...
本文只定义配置 schema、默认值和 loader 合并规则；typed SHM protocol 见
`docs/data_session_shm_communication_design.md`，fusion 部署见 `docs/market_data_fusion.md`。
...
外部 Sirius reference 的 Gate data-center 模式解析 TOML，调用
...
| 字段 | 默认值 | 含义 |
| --- | --- | --- |
| `file` | 无，必须显式配置 | instrument CSV 路径。仓库示例使用 repo-root relative path，例如 `config/instruments/usdt_future_universe.csv`。 |
...
read path 的机制对照见 `docs/websocket_client.md`。这些字段影响 burst
