...
| `trigger_exchange_ns` | `lead_lag_signal_triggered` / `lead_lag_order_intent` / `lead_lag_order_submitted` / report CSV | experiment | exchange timestamp ns | 记录触发 BBO 的 `BookTicker.exchange_ns`，用于和历史 signal CSV / replay 对齐；Gate SBE `bbo` 当前为 WebSocket server send timestamp，Binance 为 event time。 | 若信号时序诊断结束且 report 不再依赖可删除。 |
