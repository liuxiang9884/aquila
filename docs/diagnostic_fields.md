...
| `decision` / `shadow_block_reason` | `lead_lag_signal_decision` | experiment | text | 记录生成参数 shadow 口径是否会阻断本次 signal。该日志只在 `execute.taker_buffer` 或 `execute.freshness_shadow` 非 off 时输出，默认 legacy 配置不新增该日志；当前 `decision=shadow_blocked` 仅表示 shadow 规则会阻断，真实订单仍继续走现有路径；`decision=sent` 表示该日志点 shadow 不阻断，最终真实订单是否提交仍以 `lead_lag_order_submitted` / rejected 日志为准；`shadow_block_reason` 当前包括 `freshness_lead`、`freshness_lag` 或 `-`。 | shadow 配置转为 enforce 或 report 不再需要对比时重审。 |
...
| `generated_freshness_enabled` / `generated_freshness_would_block` / `generated_lead_threshold_ms` / `generated_lag_threshold_ms` | `lead_lag_signal_decision` | experiment | bool / ms | 记录启动前生成的 `execute.freshness_shadow` 是否启用、按该阈值是否会阻断开仓，以及 lead / lag 阈值。该 shadow 规则只观察，不改变真实下单行为。 | freshness shadow 转为正式 guard 或 report 不再需要对比时重审。 |
