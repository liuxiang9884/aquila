...
| `lead_id` / `lag_id` | `lead_lag_order_response` | experiment | market data ticker id | Ack 处理时策略已记录的 lead / lag 最新 `BookTicker.id`，用于判断订单 Ack 返回前策略看到的两侧行情进度。 | 若后续改为结构化 order-response CSV 或不再分析 Ack 时行情进度，可删除或迁移。 |
