...
| `max_lead_freshness_ns` / `max_lag_freshness_ns` / `freshness_guard_pass` / `freshness_reject_reason` | `lead_lag_order_intent` / `lead_lag_order_intent_rejected` / `lead_lag_order_submitted` / report CSV | experiment | ns / bool / text | open order freshness guard 的阈值、结果和原因；当前统一配置默认 lead `5ms`、lag `20ms`，只作用于 `kOpenLong` / `kOpenShort`，close / stoploss 不用该 guard。 | 若 freshness guard 稳定后字段收敛，可保留阈值和结果，删除临时 reason 细分。 |
