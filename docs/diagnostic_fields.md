...
| Level | 名称 | 能力边界 | 代表字段 |
| --- | --- | --- | --- |
| `L0` | Off | 不编译 Order Ack outlier 阶段归因；仅保留交易正确性所需状态和既有业务日志。 | 不采集 diagnostic window、`TCP_INFO`、socket timestamping 或 pcap 对齐字段。 |
...
| `L5` | Pcap + Gate header | 在 L4 基础上保留 no TLS pcap 与 Gate Ack header 对齐分析能力，进一步拆 `pcap request -> Ack response` 与 Gate `x_in_time -> x_out_time`。 | `pcap_request_to_ack_ms`、`gate_x_in_to_x_out_ms`、`residual_ms`、`gate_share`、`x_in_time`、`x_out_time`、`conn_id`。 |
