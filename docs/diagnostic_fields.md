...
`CriticalSession` 会按 `request_sequence` 维护多个 active probe；control frame 只推进全局 kernel byte id，不扩展订单
probe 的匹配范围。RX software timestamp 在处理对应 Ack 时按 `request_sequence` 归属，避免多个 active order probe 之间
互相覆盖。`max_active_probes` 控制每条连接可同时保留的 timestamping probe slot，默认 `16384`；耗尽时该 request
没有 socket timestamping 阶段归因，但订单发送本身不受影响。
编译期开关 `AQUILA_ENABLE_SOCKET_TIMESTAMPING_ATTRIBUTION` 默认 `ON`；设为 `OFF` 时 `SO_TIMESTAMPING` apply、
errqueue drain、TX/RX probe 匹配和相关 per-write 状态都会编译为 no-op，运行期配置中的 timestamping 字段仍可解析但不会启用归因。
