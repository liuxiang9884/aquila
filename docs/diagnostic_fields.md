...
| `local_order_id` / `parent_id` / `group_id` / `route_id` | `order_event.csv` | experiment | uint64 / uint64 / uint64 / uint16 | 关联 one-shot execution、entry/close 和实际 gateway route；smoke 将本轮 numeric run id 同时写入 `parent_id/group_id`，当前 `route_id=0`。 | 同上。 |
...
| `fill_probe_order_event` | stdout log / `order_event.csv` | experiment | log key / CSV row | 记录 order gateway response 或 private feedback event，用于按 `local_order_id`、`parent_id`、`group_id`、`route_id` 串联回报；probe 当前以 `node_id` 作为 `group_id`。 | 同上。 |
...
| `order_event.csv` schema | `order_event.csv` | experiment | CSV fields | gateway response / feedback 明细：`local_order_id`、`parent_id`、`group_id`、`route_id`、event/response/feedback kind、exchange order id、exchange/local timestamp、price、quantity、fill / left quantity、finish / reject reason。 | 同上。 |
