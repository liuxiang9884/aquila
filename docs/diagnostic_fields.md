...
| `bbo_to_strategy_ns` / `strategy_to_signal_ns` / `signal_to_request_send_ns` / `trigger_to_request_send_ns` | `signal.csv` / `latency.csv` | experiment | ns | 计算 BBO ingress 到策略、策略计算、signal 到下单发送、BBO ingress 到下单发送的本地闭环。 | 同上。 |
