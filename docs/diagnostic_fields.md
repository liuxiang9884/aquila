...
| Level | 名称 | 能力边界 | 代表字段 |
| --- | --- | --- | --- |
| `L0` | Off | 不编译 Order Ack outlier 阶段归因；仅保留交易正确性所需状态和既有业务日志。Gate Ack JSON header 中的 `x_in_time` / `x_out_time` 会作为普通 response 字段解析和输出，不受 Ack diagnostic level 控制。 | 不采集 diagnostic window、`TCP_INFO`、socket timestamping 或 pcap 对齐字段；保留 `exchange_x_in_ns`、`exchange_x_out_ns`、`exchange_x_in_to_x_out_ns`。 |
...
- `exchange_x_in_ns` / `exchange_x_out_ns` / `exchange_x_in_to_x_out_ns` 是 L0 普通 response 字段，
...
| `x_in_time` / `x_out_time` | `gate_x_time_alignment.csv` | experiment | Gate header timestamp | 从 no TLS pcap 还原出的 Gate Ack response 原始 header timestamp；runtime log 中对应 ns 字段为 `exchange_x_in_ns` / `exchange_x_out_ns`。 | 同上。 |
...
| `exchange_x_in_ns` | `gate_order_response` / report CSV | stable | Gate header `x_in_time` 转 ns，缺失为 `0` / CSV 空字段 | Gate 收到 request 的 header timestamp；与 `exchange_x_out_ns` 同一 Gate 时钟域，只用于 Gate 内部处理段诊断，不可与本地时间直接相减。 | Gate 不再提供 `x_in_time` 或 response schema 变更时重审。 |
| `exchange_x_out_ns` | `gate_order_response` / report CSV | stable | Gate header `x_out_time` 转 ns，缺失为 `0` / CSV 空字段 | Gate 发出 Ack response 的 header timestamp；当前 `exchange_ns` 也优先使用该值。 | 同上。 |
| `exchange_x_in_to_x_out_ns` | `gate_order_response` / report CSV | stable | ns，缺失为 `0` / CSV 空字段 | `exchange_x_out_ns - exchange_x_in_ns`，Gate 同一时钟域内 request ingress 到 Ack response egress 的处理段。 | 同上。 |
...
| `ack_exchange_x_in_ns` / `ack_exchange_x_out_ns` / `ack_exchange_x_in_to_x_out_ns` | `order_detail.csv` / `latency.csv` | stable | Gate timestamp ns / ns duration | 将 runtime `gate_order_response.exchange_x_*` 按 Ack 合并进 report；用于不抓包时统计 Gate 同钟域 request ingress 到 Ack response egress 的处理段。 | Gate Ack header 字段不可用或 schema 变更时重审。 |
