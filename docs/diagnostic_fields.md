...
| `lead_exchange_ns` / `lag_exchange_ns` | `lead_lag_signal_triggered` / `lead_lag_order_intent` / `lead_lag_order_submitted` / `lead_lag_order_response` / report CSV | experiment | exchange timestamp ns | 记录 signal 触发时 lead / lag 两侧最新 BBO 的 `BookTicker.exchange_ns`，用于 replay 对齐和解释 raw price 所属行情；order response 中记录 Ack / response 处理时策略已看到的两侧最新 BBO timestamp。 | 若 signal / order / position CSV 不再做双边行情对账可删除。 |
