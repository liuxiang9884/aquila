...
| `decision` / `shadow_block_reason` | `lead_lag_signal_decision` | experiment | text | 记录生成参数 shadow 口径是否会阻断本次 signal。当前 `decision=shadow_blocked` 仅表示 shadow 规则会阻断，真实订单仍继续走现有路径；`decision=sent` 表示该日志点 shadow 不阻断，最终真实订单是否提交仍以 `lead_lag_order_submitted` / rejected 日志为准；`shadow_block_reason` 当前包括 `freshness_lead`、`freshness_lag` 或 `-`。 | shadow 配置转为 enforce 或 report 不再需要对比时重审。 |
