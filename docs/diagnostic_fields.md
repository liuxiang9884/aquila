...
| `reject_reason` | `lead_lag_order_intent_rejected` / `order_detail.csv` / report CSV | experiment | text | open signal 已触发但未提交订单时的拒绝原因；`parallel_limit` 表示当前 pair 的 active execution group 数量已达到 `execute.parallel`，策略未提交订单。`order_route_not_ready` 表示 signal 触发时没有可用 order gateway route。其他当前值包括 `drift_guard`、`risk_limit`、`stale_lead_quote`、`stale_lag_quote` 和本地下单准备阶段拒绝原因。 | 若 rejected intent schema 收敛到稳定枚举后重审。 |
