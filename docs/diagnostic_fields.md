...
open-order snapshot、sequence continuity 或 reconcile 已完成。V1 只发布累计 order facts；不订阅 `fill` / `fast-fill`，也不
推导 maker/taker role、逐笔成交或 feedback reject。
...
| `reject_reason` | `lead_lag_order_intent_rejected` / `order_detail.csv` / report CSV | experiment | text | open signal 已触发但未提交订单时的拒绝原因；`parallel_limit` 表示当前 pair 的 active execution group 数量已达到 `execute.parallel`。`order_route_not_ready` 表示没有可用 order gateway route。`entry_quantity_not_minimum` 表示启用 `execute.require_min_entry_quantity` 后，实际 order price 推导的整数 quantity units 不等于 instrument `min_quantity`，策略在提交第一张 fanout child 前整组拒绝。其他当前值包括 `drift_guard`、`risk_limit`、`stale_lead_quote`、`stale_lag_quote` 和本地下单准备阶段拒绝原因。 | 若 rejected intent schema 收敛到稳定枚举后重审。 |
