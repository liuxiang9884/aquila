...
| `tools/gate/order_session_rtt_probe/` | Gate `OrderSession` RTT probe V1a；当前为配置解析、candidate IP 读取、dry-run plan、`--live-preflight`、pinned session config builder、sample flow / executor / id allocator / CSV writer 前置逻辑 |
