...
5. Fusion / data session 继续优化前先区分行情 pipeline 证据和订单收益。Binance / Bitget 每路 source 对比只覆盖尾部 SHM 可见窗口；不要把行情 latency 或 microbench 直接宣称为 fillability / PnL。Bitget endpoint A/B 复测应沿用同构启动、同 symbols、BBO-only、同 recorder 结构和独立 CPU 绑定。
