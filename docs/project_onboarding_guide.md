...
1. LeadLag Go reference 迁移：`lag_vol_guard` 已完成 replay-only audit 和 ORDI_USDT sweep，当前证据不支持进入 live hot path。除非要用更宽 symbol / 更长区间做反证，否则下一轮优先做独立 replay / signal-only 的 `drift_guard` 对照，先明确它和现有 `drift_limit` 是替代还是并存。迁移边界和命令见 `strategy/lead_lag/README.md` 的“Go reference 迁移边界”和“回放与输出”。
2. Drift / close 执行差异：迁移 `normal_close_retry_aggressive` 前先拆 `normal_close_slippage` 与 `stoploss_slippage`，否则当前 `close_slippage` 会同时影响 normal close 和 stoploss。
