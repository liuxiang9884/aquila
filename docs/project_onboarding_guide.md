...
- Gate SBE `publicTrade` 已有标准化 `Trade`、零 batch-copy decoder fast path 和独立 `Trade` SHM channel；JSON trade 未实现，LeadLag 策略主路径仍不读取 trade channel。
