...
- Bitget `OrderFeedbackSession` 已完成加强版 Grill Me 和设计批准，但尚未实现。V1 只订阅 account-wide `order` topic，使用累计 `cumExecQty` / `avgPrice` 映射现有 `OrderFeedbackEvent`，仅支持 `usdt-futures`、`one_way_mode`、`crossed`；断线或可能丢失 Aquila 订单事实的不可恢复解码错误发布全局 `kContinuityLost`。Bitget 不提供初始 open-order snapshot 或 sequence，REST bootstrap/reconcile 保持在 session 外。实现阶段必须补 parser/session/SHM integration test 和 release benchmark，边界见 `docs/superpowers/specs/2026-07-10-bitget-order-feedback-session-design.md`。
...
| Bitget trading | `exchange/bitget/trading/order_session.h`、`exchange/bitget/trading/operation_response_parser.h`、`exchange/bitget/trading/order_session_runtime_adapter.h`、`config/order_sessions/bitget_order_session.toml`、`tools/bitget/bitget_order_session_probe.cpp` |
...
1. 下一轮继续 Bitget `OrderFeedbackSession`：先读 `docs/superpowers/specs/2026-07-10-bitget-order-feedback-session-design.md`、Gate feedback parser/session 和 `core/trading/order_feedback_shm.h`，再写 implementation plan 并按 TDD 实现。设计边界已经批准，不重新引入 `fill` / `fast-fill`、REST reconcile、rate limiter 或真实订单；实现时必须运行 parser/session/SHM integration test 和 release benchmark。
...
先运行 `git status --short --branch` 和 `git log --oneline -8`，再读 `AGENTS.md`、`README.md`、本文件和 `docs/evaluation_support.md`；当前 branch / ahead / dirty 只信 `git status`。派发 subagent 必须按 `AGENTS.md` 选择项目级 `aquila_xhigh_worker`，不能只在 prompt 中声称 model / effort。进入设计、架构、实现计划或关键交易链路取舍时，先按 `AGENTS.md` 询问是否启用 Grill Me；普通设计建议 `grill-me-basic`，订单 / 行情 / 风控 / 恢复 / 并发 / 低延迟主路径建议 `grill-me-enhanced`。下一轮继续 Bitget `OrderFeedbackSession`：先读 `docs/superpowers/specs/2026-07-10-bitget-order-feedback-session-design.md`、Gate feedback parser/session 和 `core/trading/order_feedback_shm.h`；设计已经加强版 Grill Me 和用户批准，当前尚未实现，下一步是 implementation plan、TDD、SHM integration test 和 release benchmark。Bitget `OrderSession` 已实现于 `exchange/bitget/trading/`，使用 high availability private endpoint，直接 operation response 不能当作 accepted / fill / cancel terminal；login-only smoke 未发送真实订单，尚未接入 LeadLag、REST reconcile 或 rate limiter。2026-07-01 LeadLag A/B live 的本地输出和数据文件已在 2026-07-08 清理，不要继续追那些 tmp run dir 或 partial report。Gate OBU / OrderBook 讨论先读 `docs/gate_obu_order_book_notes.md`；当前只完成讨论和 Python quick probe，尚未实现 OBU decoder、本地 book builder、深度 SHM / recorder。若 `git status` 显示 `core/market_data/types.h` dirty，先读该文件 diff，里面可能是用户未提交的 `Orderbook` 草案，不要当作已完成 ABI。Gate 多路 order gateway 设计见 `docs/gate_order_gateway_shm_design.md`、`docs/agent-handoff-gate-trade-architecture.md` 和 `docs/strategy_order_component_model.md`；order fillability 见 `docs/exchange_matching_fillability_notes.md` 和 `docs/lead_lag_cancelled_order_fillability_analysis.md`。Gate 凭据环境变量已统一为 `GATE_TEST_KEY` / `GATE_TEST_SECRET` 和 `GATE_PROBE_KEY` / `GATE_PROBE_SECRET`，不要再用旧无前缀名称。Instrument catalog 大 universe 是 `config/instruments/usdt_future_universe.csv`（Gate 494、Binance 494、Bitget 438），旧 `usdt_futures_common_gate_binance_20260701.csv` 已重命名。Bitget market data 支持 `books1` / `BookTicker` 和 `publicTrade` / `Trade`，行情 handoff 为 `docs/agent-handoff-bitget-market-data.md`。Bitget UTA REST read-only 脚本在 `scripts/bitget/account/query_bitget_account.py`，默认使用 `BITGET_TEST_KEY` / `BITGET_TEST_SECRET` / `BITGET_TEST_PASSPHRASE`；当前 Bitget key 仍需用户处理 IP 白名单后再测账户可用资金。Bitget BBO normal vs high speed public SBE 30 分钟同步 A/B 结果见同一 handoff：normal p99/p99.9 明显更稳，high speed 未显示 BBO 延迟收益；证据仅限行情接入和 fusion pipeline。DataReader recorder 已切到 typed binary format v1，`binary_file` source 必须显式写 `feed`，旧 raw artifact 需要重录。Fusion refactor 和多轮 review 已完成，核心入口在 `core/market_data/fusion/`、`tools/market_data/data_fusion_tool_support.h`、`tools/market_data/data_fusion_launch_config_parser.h`、`tools/gate/gate_data_fusion.cpp`、`tools/binance/binance_data_fusion.cpp` 和 `tools/bitget/bitget_data_fusion.cpp`；已有证据只覆盖行情 fusion pipeline / microbench，不能外推为 fillability / PnL。新实盘启动仍必须按 `docs/lead_lag_live_operations_pipeline.md`，先 rebuild release binary，并串联 freshness 与 taker-buffer slippage preflight。
