...
- 2026-06-30 Gate 多路 `OrderSession` / `MultiOrderSessionGateway` baseline 已落地，`gate_order_session_rtt_probe` 的 fanout batch scheduler 也已在 dry-run / unit-test 层落地，但 live execute fanout timing 尚未接入。下一版生产设计已锁定在 `docs/gate_order_gateway_shm_design.md`：strategy 与 order gateway 拆成 2 个进程，strategy 进程 1 个 owner thread，gateway 进程 N 个 `OrderSessionWorker` thread；跨进程用一个 SHM 对象承载 N 路 `command_queue` 和 N 路 `event_queue`，`N` 运行时配置且最大 `16`；启动等待全部 N 路 `kReady`，`startup_ready_timeout_s` 默认 `30`；运行时 not-ready route 跳过并限频 warning。多路层目标是把策略 / OMS 已生成的 child order 低 skew 地 fanout 到 N 条 Gate trading connection；duplicate / split、winner、overfill 和非赢家 cancel 触发点都属于策略 / OMS，不属于 gateway。每个 child order 必须有唯一 `local_order_id`，同一 fanout batch 共享 `parent_id`，不修改 `LocalOrderIdCodec`；cancel 默认回到原始 session；`OrderFeedbackSession` 仍保持单账户级事实源。当前 baseline 不宣称 fillability 改善。
...
- 多路 `OrderSession` 当前已完成参数化 baseline gateway：`exchange/gate/trading/multi_order_session_gateway.h` 支持 `1 thread : n OrderSession`、`gateway_route_id=0..n-1` 显式路由、`core::kAutoGatewayRoute` ready-session round-robin、cancel / cache / forget 回原 session；测试使用 `n=4`。`gate_order_session_rtt_probe` 的 `FanoutBatchDispatchScheduler` 已覆盖同一 batch 授予全部 session、等待全部 `samples_started` 后再 cooldown 的调度模型，但 live `--execute` 仍未切到 fanout batch。下一版目标形态改为独立 `order-gateway-process`：strategy 进程通过 SHM `command_queue[i]` 把 child order 发给 `OrderSessionWorker[i]`，worker 通过 `event_queue[i]` 回传 Ack / final response / ready 状态；`PlaceOrder()` 的 `kOk` 只表示 command 已进入 queue，不表示 socket write complete。Ack / final response 仍由各 `OrderSession` 按唯一 `local_order_id` fan-in 到 `TradingRuntime -> OrderManager -> Strategy`，gateway 不解释 Ack。回报仍由单独 `gate-feedback-process` 的单个 `OrderFeedbackSession` 写 SHM，再由策略进程读取。
