...
- 当前 32 物理 core 机器必须遵守 `docs/runtime_cpu_allocation.md`：`0-15` 为实盘保留区，`16-31` 为测试 / diagnostics / benchmark 区；测试任务默认不要占用实盘 hot path core。30-symbol fusion order-gateway smoke / validate 配置为了避开已有 fusion / strategy / feedback CPU，临时使用 `16-19` 作为 4 条 order route worker；若按该配置启动真实订单，`16-19` 必须视为本轮 live 占用，不能并发测试负载。
