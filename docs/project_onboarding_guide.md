...
- 已完成 flat-account、tiny-position、隔离 `ContinuityLost` stop-and-flat、ZEC 小额 filled open / close、unfilled-cancel smoke 和本地端到端 benchmark。2026-05-22 release run 暴露的 IOC partial-fill terminal feedback 与 decimal residual 问题已代码修复，但仍需完整 strategy 小额 live smoke 复核。
...
LeadLag：先读 `docs/lead_lag_live_runtime_plan.md`、`docs/lead_lag_live_operations_pipeline.md`、`docs/lead_lag_ack_latency_outlier_analysis.md`、`docs/lead_lag_runtime_latency_improvement_plan.md`、`docs/lead_lag_live_replay_testing.md` 和 `docs/lead_lag_reconcile_design.md`。真实订单 `--execute` 默认用 `config/strategies/lead_lag_requested_11symbols_live_strategy_20260522.toml`；flat-account / tiny-position / ContinuityLost stop-and-flat、ZEC 小额 filled open / close、unfilled-cancel smoke 和本地端到端 benchmark 已完成。2026-05-25 12-symbol 1 小时 run flat 退出但无成交，最大 Ack RTT `219.023ms`；2026-05-26 30 分钟拆核 run flat 退出但无成交，最大 Ack RTT `6.738ms`，最大 exchange Ack-to-finish `37.336ms` 属于 Gate IOC terminal lifecycle，不是 Ack path outlier。多 Gate `OrderSession` / WebSocket 连接即使同 URL 也可能 Ack RTT 不同；多连接测试必须按 session / connection 记录 remote endpoint、local port、owner CPU、Ack RTT 分布和 TCP diagnostics。两轮 12-symbol run 都不能证明 IOC partial-fill / decimal filled close blocker 已复核；复核前不要继续无人值守真实订单长跑。
