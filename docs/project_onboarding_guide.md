...
- Bitget UTA public SBE `books1` BBO 已接入并合并到 `main`：`seq -> BookTicker.id`，`sts * 1000 -> exchange_ns`，`ts * 1000 -> event_ns`，data session ingress `CLOCK_REALTIME -> local_ns`。Bitget `publicTrade` 已接入：`execId -> Trade.id`，`side=0/1 -> Buy/Sell`，`sts * 1000 -> exchange_ns`，`ts * 1000 -> event_ns`，group index/count 写入 `batch_index` / `batch_count`。当前不支持 Bitget order/private feed 或交易端；原 `feature/bitget-bbo-market-data` worktree 和本地 branch 已删除。入口和配置见 `docs/agent-handoff-bitget-market-data.md`。
...
4. DataReader / recorder 相关变更先读 `docs/data_reader_config.md` 和 `docs/data_session_shm_communication_design.md`。typed binary header feed/type 是强约束；旧 raw artifact 不兼容。需要大 symbol universe 时使用 `config/instruments/usdt_future_universe.csv`，不要再引用旧 `usdt_futures_common_gate_binance_20260701.csv` 文件名。
5. Gate OBU / OrderBook 继续讨论先读 `docs/gate_obu_order_book_notes.md`。下一步优先确认 published `OrderBook<10>` ABI：是否包含 `symbol_id` / `exchange`、count 类型、array-of-struct 还是 struct-of-arrays、是否继续用 `double` 发布；实现前先补 Gate OBU decoder / local book builder 单测，覆盖 `numInGroup` 不满 level、empty update、`size == 0` 删除和 gap resubscribe。
6. Fusion / data session 继续优化前先区分行情 pipeline 证据和订单收益。Binance / Bitget 每路 source 对比只覆盖尾部 SHM 可见窗口；不要把行情 latency 或 microbench 直接宣称为 fillability / PnL。Bitget endpoint A/B 复测应沿用同构启动、同 symbols、BBO-only、同 recorder 结构和独立 CPU 绑定。
7. Ack latency / RTT outlier 复现时用 private plain all-stage config，分开看 Ack RTT、Gate `x_in -> x_out`、上行 / 下行、socket timestamping 和 pcap residual。
8. FixedOrderedSlotPool / `parallel=n` 迁移必须端到端同步 core order contract、order gateway SHM、LeadLag execution state、日志、report CSV、脚本测试和文档。不能停在 `parent_id` 与 `group_id` 混用的半迁移状态。
9. CPU isolation、ENA IRQ 或 interrupt moderation 调整先读 `docs/runtime_cpu_allocation.md`，并用 A/B 证据记录 CPU、affinity、IRQ、kernel 和命令；没有 benchmark / profile / live smoke 证据时不宣称性能收益。

## 给下一个对话的提示

先运行 `git status --short --branch` 和 `git log --oneline -8`，再读 `AGENTS.md`、`README.md`、本文件和 `docs/evaluation_support.md`；当前 branch / ahead / dirty 只信 `git status`。进入设计、架构、实现计划或关键交易链路取舍时，先按 `AGENTS.md` 询问是否启用 Grill Me；普通设计建议 `grill-me-basic`，订单 / 行情 / 风控 / 恢复 / 并发 / 低延迟主路径建议 `grill-me-enhanced`。2026-07-01 LeadLag A/B live 的本地输出和数据文件已在 2026-07-08 清理，不要继续追那些 tmp run dir 或 partial report。Gate OBU / OrderBook 讨论先读 `docs/gate_obu_order_book_notes.md`；当前只完成讨论和 Python quick probe，尚未实现 OBU decoder、本地 book builder、深度 SHM / recorder。若 `git status` 显示 `core/market_data/types.h` dirty，先读该文件 diff，里面可能是用户未提交的 `Orderbook` 草案，不要当作已完成 ABI。Gate 多路 order gateway 设计见 `docs/gate_order_gateway_shm_design.md`、`docs/agent-handoff-gate-trade-architecture.md` 和 `docs/strategy_order_component_model.md`；order fillability 见 `docs/exchange_matching_fillability_notes.md` 和 `docs/lead_lag_cancelled_order_fillability_analysis.md`。Gate 凭据环境变量已统一为 `GATE_TEST_KEY` / `GATE_TEST_SECRET` 和 `GATE_PROBE_KEY` / `GATE_PROBE_SECRET`，不要再用旧无前缀名称。Instrument catalog 大 universe 是 `config/instruments/usdt_future_universe.csv`（Gate 494、Binance 494、Bitget 438），旧 `usdt_futures_common_gate_binance_20260701.csv` 已重命名。Bitget market data 已合并到 `main`，原 `feature/bitget-bbo-market-data` worktree / 本地 branch 已删除；接手先读 `docs/agent-handoff-bitget-market-data.md`，当前支持 Bitget `books1` / `BookTicker` 和 `publicTrade` / `Trade`，但不要假定 Bitget order/private feed 或交易端已完成。Bitget UTA REST read-only 脚本在 `scripts/bitget/account/query_bitget_account.py`，默认使用 `BITGET_TEST_KEY` / `BITGET_TEST_SECRET` / `BITGET_TEST_PASSPHRASE`；当前 Bitget key 仍需用户处理 IP 白名单后再测账户可用资金。Bitget BBO normal vs high speed public SBE 30 分钟同步 A/B 结果见同一 handoff：normal p99/p99.9 明显更稳，high speed 未显示 BBO 延迟收益；证据仅限行情接入和 fusion pipeline。DataReader recorder 已切到 typed binary format v1，`binary_file` source 必须显式写 `feed`，旧 raw artifact 需要重录。Fusion refactor 和多轮 review 已完成，核心入口在 `core/market_data/fusion/`、`tools/market_data/data_fusion_tool_support.h`、`tools/market_data/data_fusion_launch_config_parser.h`、`tools/gate/gate_data_fusion.cpp`、`tools/binance/binance_data_fusion.cpp` 和 `tools/bitget/bitget_data_fusion.cpp`；已有证据只覆盖行情 fusion pipeline / microbench，不能外推为 fillability / PnL。新实盘启动仍必须按 `docs/lead_lag_live_operations_pipeline.md`，先 rebuild release binary，并串联 freshness 与 taker-buffer slippage preflight。
