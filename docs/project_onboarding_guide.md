...
- 2026-06-28 Gate 多路 `OrderSession` / `MultiOrderSessionGateway` 讨论结论已写入 Gate 交易架构文档，但尚未实现。推荐 V1 是在既有 `OrderManager` 后放一个 composite gateway，内部管理显式 `RouteTable(local_order_id -> session_index)`、session health 和账户级 submit / cancel / pending budget；每个拆分子订单必须有唯一 `local_order_id`，cancel 默认回到原始 session；`OrderFeedbackSession` 仍保持单账户级事实源。
...
- 多路 `OrderSession` 当前只完成设计讨论：生产 V1 推荐 `StrategyOrderOwnerThread` 内单线程拥有 `TradingRuntime`、`OrderManager`、`MultiOrderSessionGateway` 和 N 个 `OrderSession`，避免 submit 主路径引入 IPC / worker queue；worker-per-session 只适合 RTT / fillability 实验，不作为默认实盘架构。回报仍由单独 `gate-feedback-process` 的单个 `OrderFeedbackSession` 写 SHM，再由策略进程读取。
