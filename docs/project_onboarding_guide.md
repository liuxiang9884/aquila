...
先运行 `git status --short --branch` 和 `git log --oneline -8`，再读 `AGENTS.md`、`README.md`、本文件和 `docs/evaluation_support.md`。当前 branch / ahead / dirty 只信 `git status`。Fusion / Tardis BBO 对账见 `docs/fusion_tardis_bbo_comparison.md`：Binance 20260627 30-symbol Tardis 侧全部 strict 匹配到 fusion，Tardis 是 fusion 总量的 `99.9352%`；Gate strict 差异主要来自 timestamp 语义，`near_ms=5` 后 Tardis-only candidate 为 `10,085` 条，Gate BTC_USDT fusion 应按 `id` / 发布顺序复现，不按 `exchange_ns` 重排。LeadLag 当前 Go / C++ 对照和剩余差异见 `strategy/lead_lag/README.md` 的“当前 Go / C++ 对照”和 `docs/lead_lag_go_cpp_strategy_alignment.md`。最近围绕 Go reference 迁移已明确：`freshness_auto` 和 `taker_buffer` 不放入实时策略热路径；freshness 启动前用 `lead_lag_freshness_preflight` + `scripts/lead_lag/apply_freshness_preflight_summary.py` 生成固定 `max_lag_freshness_ms`；`freshness_shadow` 已删除；taker buffer 启动前用 `scripts/lead_lag/generate_preflight_config_params.py --lag-price-tick` 把 pct 转成 `open_slippage_ticks` / `close_slippage_ticks`，再用 `scripts/lead_lag/apply_taker_buffer_slippage.py` 写回策略 TOML；该 preflight 不覆盖 `stoploss_slippage_ticks` 或 normal close retry 参数。C++ open signal 的 `required_edge` 会用 fixed `open_slippage_ticks` / `close_slippage_ticks` 折算基础 entry / normal close 执行成本，但不读取 `execute.taker_buffer` pct 直接筛 signal；open short 已有回归覆盖，ORDI_USDT 三天 zero ticks vs 5 ticks replay 在当前样本上 signals CSV 完全一致。LeadLag 执行配置已拆分 close / stoploss slippage，并用 `close_retry_times` + `close_retry_slippage_step_ticks` 实现 normal close retry 递增 ticks；旧 `open_slippage`、`close_slippage`、`normal_close_retry_aggressive` 不再解析。`parallel-limit` 当前是 post-signal open guard，会输出 `lead_lag_order_intent_rejected reason=parallel_limit`；`lag_vol_guard` 已有 replay-only audit：用 `lead_lag_replay --lag-vol-guard-audit-output` 输出 `lag_vol_guard_audit.csv`，再用 `scripts/lead_lag/summarize_guard_audit.py` 对齐 `order_detail.csv` / `position.csv`；ORDI_USDT 三天 sweep 显示 `cooldown=3m/5m/10m/15m` 都降低 signal-only PnL，默认 `15m` 主要靠 cooldown 错杀正贡献 open signal，因此当前只保留离线 audit 工具，不把 `lag_vol_guard` 放进实盘策略。`drift_guard` 已按 Go reference 口径替代旧 `trigger.drift_limit`，实盘定位为 open-only emergency sanity guard；配置使用 `[lead_lag.pairs.trigger.drift_guard] enabled / drift_instant / ratio_std / ratio_std_window / drift_mean / drift_mean_window` 和 `capacity.drift_guard_window_capacity`，checked-in 策略 TOML 已迁移，parser 会拒绝旧 `drift_limit` 和 `drift_guard.mode`。`drift_guard` 命中会输出 `lead_lag_order_intent_rejected reason=drift_guard`，report 以 `source_schema=intent_rejected_v1` 关联到 signal；replay audit 也输出 raw `drift_instant` / `ratio_std` / `drift_mean` 和 `drift_guard_outcome`。如果要回到旧 pre-signal `drift_limit`，先读 `strategy/lead_lag/README.md` 的“`drift_limit` 历史实现与回退说明”，按其中代码 / 配置 / 测试清单做完整替换，不要和当前 `drift_guard` 同时 enforce。旧 agent 计划 / 设计稿已删除；Gate / Binance fastest-route threaded bundle 文档现在是 `docs/gate_fastest_route_fusion_threaded_bundle_guide.md`。订单 fillability 任务仍可继续：先读 `docs/lead_lag_cancelled_order_fillability_analysis.md`，使用 `signal_lag_id` / `ack_lag_id` / `accepted_lag_id` / `cancelled_lag_id` 对齐 Gate lag fusion canonical BookTicker bin；字段名是 `cancelled_*_id`。新实盘启动仍必须按 `docs/lead_lag_live_operations_pipeline.md`，先 rebuild release binary，并串联 freshness 与 taker-buffer slippage preflight。
