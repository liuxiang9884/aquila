...
- 当前 LeadLag latency 状态：2026-05-25 出现的 `219.023ms` Ack RTT outlier 已补 diagnostic / affinity / report 字段；2026-05-26 拆核 30 分钟 run 未复现 Ack outlier。Ack outlier 根因仍未证明，后续只有复现后再结合 diagnostic / 调度证据分析。
- 当前建议分支入口：`main`。
...
- WebSocket endpoint 配置使用 `host` / 可选 `connect_ip` / `port`：`connect_ip` 为空时 TCP 解析 `host`，非空时 TCP 直连该 IP；TLS SNI / 证书校验和 WebSocket Host 始终使用 `host`。这用于后续 Gate remote IP pinning 验证，不包含自动选优。
...
LeadLag：先读 `docs/lead_lag_live_runtime_plan.md`、`docs/lead_lag_live_operations_pipeline.md`、`docs/lead_lag_ack_latency_outlier_analysis.md`、`docs/lead_lag_runtime_latency_improvement_plan.md`、`docs/lead_lag_live_replay_testing.md` 和 `docs/lead_lag_reconcile_design.md`。真实订单 `--execute` 默认用 `config/strategies/lead_lag_requested_11symbols_live_strategy_20260522.toml`；flat-account / tiny-position / ContinuityLost stop-and-flat、ZEC 小额 filled open / close、unfilled-cancel smoke 和本地端到端 benchmark 已完成。IOC partial-fill / decimal filled close 当前不再作为 active blocker，后续遇到 terminal feedback、filled close 或 REST residual 异常再复查。2026-05-25 12-symbol 1 小时 run flat 退出但无成交，最大 Ack RTT `219.023ms`；2026-05-26 30 分钟拆核 run flat 退出但无成交，最大 Ack RTT `6.738ms`，最大 exchange Ack-to-finish `37.336ms` 属于 Gate IOC terminal lifecycle，不是 Ack path outlier。多 Gate `OrderSession` / WebSocket 连接即使同 URL 也可能 Ack RTT 不同；已按 session 输出 `order_session_id`、connected endpoint / owner CPU、send CPU、ack CPU、diagnostic CPU 和可选 `TCP_INFO`，`TCP_INFO` 默认关闭；2026-05-27 3 session smoke 已证明字段可用且连接可能落到不同 remote IP，但 N=1 不代表 RTT 分布；多连接测试必须按 session / connection 记录 remote endpoint、local port、owner CPU、Ack RTT 分布和 TCP diagnostics。
