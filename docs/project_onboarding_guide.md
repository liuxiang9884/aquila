...
| Runtime CPU / ENA IRQ / system tuning | `docs/runtime_cpu_allocation.md` |
| Gate 交易架构 | `docs/agent-handoff-gate-trade-architecture.md`、`docs/strategy_order_component_model.md` |
| Gate 多路 OrderSession / MGW | `docs/gate_order_gateway_shm_design.md`、`docs/agent-handoff-gate-trade-architecture.md`、`docs/strategy_order_component_model.md`、`docs/gate_order_session_rtt_probe_design.md` |
| LeadLag FixedOrderedSlotPool / parallel=n | `docs/lead_lag_fixed_ordered_slot_pool_parallel.md`、`core/base/fixed_ordered_slot_pool.h`、`benchmark/strategy/lead_lag_group_container_benchmark.cpp` |
...
- Gate / Binance `BookTicker` data session、SHM、strategy `DataReader`、Gate submit/cancel、Gate private `futures.orders` feedback、`OrderManager`、`TradingRuntime`、LeadLag replay / live-orders、TUI market data monitor demo 已落地。Gate SBE data session 已支持在同一个 WebSocket 连接上按 `feeds = ["book_ticker", "trade"]` 同时订阅 BBO 和 `publicTrade`；Binance data session 已支持同连接 mixed `/public/ws/<symbol>@bookTicker` 与 live-probed raw `/public/ws/<symbol>@trade`。两者都发布到同一个 SHM object 内的 `BookTicker` / `Trade` 两个 typed channel；策略 `DataReader` 当前仍只消费 `BookTicker`。
- 2026-07-01 Gate 多路 `OrderSession` 已落地两层实现：`exchange/gate/trading/multi_order_session_gateway.h` 是单进程 `1 thread : n OrderSession` baseline，`core/trading/order_gateway_client.h` + `tools/gate/gate_order_gateway.cpp` 是独立 `order-gateway-process` / SHM V2。生产形态见 `docs/gate_order_gateway_shm_design.md`：strategy 与 order gateway 拆成 2 个进程，strategy 进程 1 个 owner thread，gateway 进程 N 个 `OrderSessionWorker` thread；跨进程用一个 SHM 对象承载 N 路 `command_queue`、N 路 `event_queue` 和 header `route_states[16]`，`N` 运行时配置且最大 `16`；启动等待全部 N 路 `kReady`，`startup_ready_timeout_s` 默认 `30`；运行时 not-ready route 跳过。多路层目标是把策略 / OMS 已生成的 child order 低 skew 地 fanout 到 N 条 Gate trading connection；duplicate / split、winner、overfill 和非赢家 cancel 触发点都属于策略 / OMS，不属于 gateway。每个 child order 必须有唯一 `local_order_id`；当前生产代码仍使用 `parent_id` 归组，同步迁移计划已锁定后续删除 `parent_id` 并改用 per-symbol `group_id` 与 runtime `group_index`。cancel 默认回到原始 session；`OrderFeedbackSession` 仍保持单账户级事实源。当前只完成 validate-only / 单元 / smoke 测试，不宣称 fillability 或 latency 收益。
- 2026-07-01 已启动 30-symbol LeadLag A/B live 对照，当前接手时必须重新检查进程、guard、REST dry-run、health check 和 partial report，不要假定仍在同一状态。A 组是当前 `main` 多 `OrderSession` order-gateway 路径，run dir `/home/liuxiang/tmp/20260701_102201_30symbols_ogw_24h/`，启动时间约 `2026-07-01 10:28 UTC`，目标 24h；B 组是 historical single `OrderSession` same-thread baseline，worktree `/home/liuxiang/dev/aquila/.worktrees/lead-lag-single-thread-ab-d9f1457`、branch `ab/lead-lag-single-thread-baseline-d9f1457`、commit `d9f1457`，run dir `/home/liuxiang/tmp/20260701_143803_30symbols_single_thread_probe_ab/`，使用 `PROBE_KEY` / `PROBE_SECRET` 和当前 A 组 fusion canonical SHM。2026-07-01 15:36 UTC handoff partial report：A `signals=1426`、`submitted=860`、`filled_orders=15`、`positions=11`、actual net PnL `-0.068648912`，report dir `/home/liuxiang/tmp/aquila_partial_reports/20260701_102201_30symbols_ogw_24h_handoff_partial/`；B `signals=262`、`submitted=29`、`filled_orders=2`、`positions=1`、actual net PnL `0.00388482`，report dir `/home/liuxiang/tmp/aquila_partial_reports/20260701_143803_30symbols_single_thread_probe_ab_handoff_partial/`。A 组截至 handoff health 仍有 2026-07-01 14:00 UTC 左右的 `kUnknownResult` / `needs_reconcile` 记录，其中包含 `HYPE_USDT`，后续分析前先重新确认 pause / resume 和账户实际仓位。
- 2026-07-01 LeadLag live report 的 `position.csv` 多 entry 匹配逻辑已修复，提交 `f077842`。`scripts/lead_lag/analyze_order_detail.py::build_position_detail_rows()` 现在按 FIFO entry lot 匹配多路 duplicate entry / exit，并按 matched volume 分摊 entry / exit fee；同一 position 多笔 entry 时 `position_key` 会变为 `run_id:symbol_id:position_id:<entry_local_order_id>:<exit_local_order_id>`。这修正了多路 duplicate fill 下把 `12` 开仓 / `12` 平仓误报为 `over_closed` 的问题；`over_closed` 现在只表示 exit 成交量超过 report 可匹配 entry 剩余量。字段说明见 `docs/lead_lag_live_report_csv_schema.md`。
- 当前 32 物理 core 机器必须遵守 `docs/runtime_cpu_allocation.md`：`0-15` 为实盘保留区，`16-31` 为测试 / diagnostics / benchmark 区；测试任务默认不要占用实盘 hot path core。30-symbol fusion order-gateway smoke / validate 配置为了避开已有 fusion / strategy / feedback CPU，临时使用 `16-19` 作为 4 条 order route worker；若按该配置启动真实订单，`16-19` 必须视为本轮 live 占用，不能并发测试负载。2026-07-02 已在该文档补充当前 AWS/KVM + ENA baseline、CPU isolation 候选 profile、ENA IRQ pool、RPS / XPS / interrupt moderation 建议和验证命令；这些是候选系统优化方案，不代表已上线或已证明性能收益。
- 临时 log、scratch config、live snapshot、benchmark 临时产物默认写入 `/home/liuxiang/tmp`。
- 2026-07-02 已新增 core/base 固定容量 FIFO ordered slot pool `core/base/fixed_ordered_slot_pool.h`：`FixedOrderedSlotPool<T, kCapacity>` 使用 `occupied_mask_`、`active_indices_` 和 `active_positions_`，`kCapacity <= 64`，`Initialize(n)` clamp 到编译期容量，初始化后无动态分配；单测在 `test/core/base/base_structures_test.cpp`。同时合并两个 benchmark worktree：`benchmark/strategy/lead_lag_group_container_benchmark.cpp` 对比 `ActiveIndex` / `FixedOrderedSlotPool` / `LinkedList`，最新 full-active `n=2/4/8/16/32/64` 输出见 `/home/liuxiang/tmp/lead_lag_group_container_benchmark_fixed_ordered_slot_pool_20260703.txt`；`benchmark/strategy/lead_lag_submit_breakdown_benchmark.cpp` 新增 order-gateway fanout batch lower-bound 对比。当前仅证明 benchmark 层行为和相对成本，不代表生产策略已切换 group container 或 batch API。`parallel=n` 多 group 迁移边界、fanout child 与 parallel slot 的区别、`kMaxLeadLagExecutionGroups = 16` 直接替换建议、`parent_id` 删除和 core `group_id` / `group_index` 元数据迁移见 `docs/lead_lag_fixed_ordered_slot_pool_parallel.md`。
...
- 真实订单默认配置仍是 `config/strategies/lead_lag_requested_11symbols_live_strategy_20260522.toml`。30-symbol 配置入口是 `config/strategies/lead_lag_30symbols_live_strategy_20260604.toml`，其 nested strategy config 为 `config/strategies/lead_lag_30symbols_2bps_notional_20260604.toml`。20260627 30-symbol fusion 单 session 配置是 `config/strategies/lead_lag_30symbols_fusion_live_strategy_20260627.toml`；order-gateway validate-only 配置是 `config/strategies/lead_lag_30symbols_fusion_order_gateway_live_strategy_20260627.toml`，对应 `config/order_gateways/gate_order_gateway_30symbols_private_plain_20260627.toml` 和 fanout pair config `config/strategies/lead_lag_30symbols_fusion_2bps_5bps_order_gateway_20260627.toml`。20260701 新 30-symbol fusion order-gateway live 配置入口是 `config/strategies/lead_lag_30symbols_fusion_order_gateway_live_strategy_20260701.toml`，对应 `config/instruments/usdt_futures_common_gate_binance_20260701.csv`、Gate / Binance `data_fusion_30symbols_20260701`、`config/data_readers/strategy_data_reader_30symbols_fusion_20260701.toml`、`config/order_gateways/gate_order_gateway_30symbols_private_plain_20260701.toml` 和 pair config `config/strategies/lead_lag_30symbols_fusion_2bps_2bps_5bps_lag200_order_gateway_20260701.toml`；该 pair config 使用 lead freshness `5ms`、lag freshness `200ms`、open / close slippage ticks 均使用原 close `2bps` 换算值、stoploss `5bps` 按 Gate futures ticker last price 转 ticks、`close_retry_times=2`，open notional 按 Gate 最小下单名义金额乘 2 后上取整到 10 / 100。原 `config/strategies/lead_lag_30symbols_fusion_0bps_2bps_5bps_order_gateway_20260701.toml` 保留为上一次 30-symbol order-gateway live run 的阈值快照。
...
- Gate data / order / feedback 在 private plain 场景使用 `fxws-private.gateapi.io:80`；Binance data session 仍按 public TLS `/public/ws` raw streams，默认只订阅 `bookTicker`，显式配置后可混合订阅 raw `trade`。
...
- 30-symbol report 必须显式传本轮 run 对应的 `--instrument-catalog`，否则默认 12-symbol catalog 可能导致 multiplier / PnL 不完整；20260701 配置使用 `config/instruments/usdt_futures_common_gate_binance_20260701.csv`，旧 20260604 / 20260606 report 仍按当时 run 使用 `config/instruments/usdt_futures_common_gate_binance_20260602.csv`。
- `config/instruments/usdt_futures_common_gate_binance_*.csv` 是 Gate / Binance USDT 永续交集合约 catalog；`contract_multiplier` 是 report / PnL 使用的显式字段，当前与 `notional_multiplier` 保持一致。
...
- Gate SBE `publicTrade` 和 Binance raw `trade` 已有标准化 `Trade` 与独立 `Trade` SHM channel；Gate SBE trade 使用零 batch-copy decoder fast path，Binance raw trade 使用 JSON `e=trade` 分发后写 `Trade` slot。LeadLag 策略主路径仍不读取 trade channel。
...
- 多路 `OrderSession` 当前已完成参数化 baseline gateway 和独立 SHM order gateway：`exchange/gate/trading/multi_order_session_gateway.h` 支持 `1 thread : n OrderSession`、`gateway_route_id=0..n-1` 显式路由、`core::kAutoGatewayRoute` ready-session round-robin、cancel / cache / forget 回原 session；`core/trading/order_gateway_client.h`、`exchange/gate/trading/order_gateway_worker.h`、`tools/gate/gate_order_gateway.cpp` 实现 2 进程 / N worker 的生产 V2，header `route_states[16]` 用于 ready / not-ready / stopped 丢事件兜底，但不覆盖 `SIGKILL` / crash 后旧 ready 滞留，后续需要 heartbeat / owner-death 设计。`tools/lead_lag/live_strategy.cpp` 已支持 `[strategy.order_gateway]` live-orders backend，`OrderGatewayClient::PlaceOrder()` 的 `kOk` 只表示 command 已进入 queue，不表示 socket write complete；Ack / final response 经 `event_queue[i] -> TradingRuntime -> OrderManager -> Strategy`，gateway 不解释 Ack。回报仍由单独 `gate-feedback-process` 的单个 `OrderFeedbackSession` 写 SHM，再由策略进程读取。当前只验证了 config / unit / missing-SHM smoke 和 validate-only，真实 order gateway live smoke 仍未执行。
...
| Core base structures | `docs/lead_lag_fixed_ordered_slot_pool_parallel.md`、`core/base/fixed_ordered_slot_pool.h`、`test/core/base/base_structures_test.cpp`、`benchmark/strategy/lead_lag_group_container_benchmark.cpp` |
...
| LeadLag benchmark docs | `docs/lead_lag_benchmark_environment_tail_analysis.md`、`benchmark/strategy/lead_lag_runtime_benchmark.cpp`、`benchmark/strategy/lead_lag_strategy_benchmark.cpp`、`benchmark/strategy/lead_lag_submit_breakdown_benchmark.cpp`、`benchmark/strategy/lead_lag_group_container_benchmark.cpp` |
...
FixedOrderedSlotPool / group-container focused checks:
...
7. Gate 多路 OrderSession：若继续多路下单，先读 `docs/gate_order_gateway_shm_design.md`、`docs/agent-handoff-gate-trade-architecture.md` 的“多路 OrderSession 讨论结论”和 `docs/strategy_order_component_model.md` 的“多路 OrderSession 扩展边界”。当前代码已完成独立 `order-gateway-process`、SHM `command_queue` / `event_queue`、strategy ready gate、LeadLag `order_session_fanout` 和 30-symbol order-gateway validate-only 配置；下一步是按 live pipeline 启动 `tools/gate/gate_order_gateway` + LeadLag strategy 做 guarded 小额 smoke，并用 probe / live report 量化 skew、Ack RTT 和 fillability。涉及订单链路、反馈、并发或低延迟取舍时，按 `AGENTS.md` 先询问是否启用 Grill Me，默认建议 `grill-me-enhanced`；正式宣称 fill-rate / latency 收益前必须拿 live smoke 或 probe 证据。
8. 硬件 / 系统优化：如果继续 CPU isolation、ENA IRQ 或 interrupt moderation 调整，先读 `docs/runtime_cpu_allocation.md` 的“硬件 / 系统优化基线”。当前建议先把 ENA IRQ 从 live hot core 搬到 dedicated IRQ pool，再做 interrupt moderation A/B；不要把该候选 profile 当作已验证收益。
9. FixedOrderedSlotPool / multi-group state：如果继续把 LeadLag `ExecutionState` 从 `std::vector<ExecutionGroup>` 迁到 FIFO ordered slot pool，先读 `docs/lead_lag_fixed_ordered_slot_pool_parallel.md`、`core/base/fixed_ordered_slot_pool.h` 和 `benchmark/strategy/lead_lag_group_container_benchmark.cpp`；当前讨论锁定 LeadLag 直接迁移候选为 `FixedOrderedSlotPool<ExecutionGroup, 16>`，生产配置应拒绝 `parallel > 16`，不保留 vector-like `groups()` API，并在同一不可中断迁移中彻底删除 `parent_id`。新归因字段为 per-symbol `group_id` 和 runtime diagnostic `group_index`，字段下沉到 core `OrderCreateRequest` / `StrategyOrder`、order gateway SHM、Gate / LeadLag 日志和 report CSV；新脚本不兼容旧 `parent_id` 日志。性能结论需重新跑 group-container benchmark、LeadLag strategy / submit focused tests、order gateway tests 和 report script tests。

## 给下一个对话的提示

先运行 `git status --short --branch` 和 `git log --oneline -8`，再读 `AGENTS.md`、`README.md`、本文件和 `docs/evaluation_support.md`；当前 branch / ahead / dirty 只信 `git status`。进入设计、架构、实现计划或关键交易链路取舍时，先按 `AGENTS.md` 询问是否启用 Grill Me；普通设计建议 `grill-me-basic`，订单 / 行情 / 风控 / 恢复 / 并发 / 低延迟主路径建议 `grill-me-enhanced`。当前最重要的现场是 2026-07-01 30-symbol LeadLag A/B live：A 组 run dir `/home/liuxiang/tmp/20260701_102201_30symbols_ogw_24h/` 是当前多 `OrderSession` order-gateway 24h run，B 组 run dir `/home/liuxiang/tmp/20260701_143803_30symbols_single_thread_probe_ab/` 是 historical `d9f1457` single `OrderSession` same-thread baseline，worktree `/home/liuxiang/dev/aquila/.worktrees/lead-lag-single-thread-ab-d9f1457`，PROBE account，复用 A 组 fusion canonical SHM。接手后先重新检查进程、guard、REST dry-run、health check 和 partial report；2026-07-01 15:36 UTC handoff partial report 仅作为快照：A `signals=1426`、`submitted=860`、`filled_orders=15`、`positions=11`、actual net PnL `-0.068648912`，B `signals=262`、`submitted=29`、`filled_orders=2`、`positions=1`、actual net PnL `0.00388482`。A 组 14:00 UTC 左右有 `kUnknownResult` / `needs_reconcile` 记录，其中包含 `HYPE_USDT`，继续分析前先确认 pause / resume 和账户实际仓位。LeadLag report 多 entry FIFO 匹配已在 `f077842` 修复，`over_closed` 不再代表多路 duplicate entry 只取第一笔 entry 的误分类；字段见 `docs/lead_lag_live_report_csv_schema.md`。Gate 多路 order gateway 设计见 `docs/gate_order_gateway_shm_design.md`、`docs/agent-handoff-gate-trade-architecture.md` 和 `docs/strategy_order_component_model.md`；order fillability 继续读 `docs/lead_lag_cancelled_order_fillability_analysis.md`。如果继续系统层低延迟优化，先读 `docs/runtime_cpu_allocation.md` 的“硬件 / 系统优化基线”，其中记录了当前 ENA queue / IRQ baseline、candidate CPU isolation profile 和验证命令；这些仍是候选方案，不要宣称性能收益。另外，当前 main 已包含 `FixedOrderedSlotPool<T, kCapacity>` 和两个 benchmark worktree 的合并结果；如果继续 `parallel=n` 多 group 容器迁移，先看 `docs/lead_lag_fixed_ordered_slot_pool_parallel.md`、`core/base/fixed_ordered_slot_pool.h`、`test/core/base/base_structures_test.cpp`、`benchmark/strategy/lead_lag_group_container_benchmark.cpp`，不要假定 LeadLag `ExecutionState` 已切换；当前讨论建议直接迁到 `FixedOrderedSlotPool<ExecutionGroup, 16>`，同时删除 `parent_id`，改用 core-level `group_id` / `group_index`，并同步 SHM、日志、report 脚本和文档。新实盘启动仍必须按 `docs/lead_lag_live_operations_pipeline.md`，先 rebuild release binary，并串联 freshness 与 taker-buffer slippage preflight。
