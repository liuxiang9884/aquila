...
- 当前 LeadLag latency 状态：2026-05-25 出现的 `219.023ms` Ack RTT outlier 已补 diagnostic / affinity / report 字段；2026-05-26 拆核 30 分钟 run 未复现 Ack outlier。Ack RTT outlier 当前处于 inactive investigation，等待复现后再结合 diagnostic / 调度证据分析。2026-05-26 的 exchange Ack-to-finish `37.336ms` 属于 terminal lifecycle latency，候选假设按 Gate 交易所内部订单队列 / IOC terminal lifecycle 延迟标记，仍需后续复现证明。Gate `OrderSession` RTT probe 第一版设计已落地，先做 measurement-only，采集 GTC place、GTC cancel 和 IOC place 三类 Ack RTT，不自动 score / 切换；当前代码已有 V1a single-session dry-run scaffold、pinned session config builder、sample flow / executor / id allocator / CSV writer 前置逻辑和 `--live-preflight`。sample flow 已保存 Ack 接收时间 / stage status，校验 Ack / final response `local_order_id`，并已覆盖 GTC cancel reject、feedback fill / timeout 后进入 reduce-only close、close Ack 后等待 terminal feedback 和 close terminal confirmation 的纯状态流转；配置已支持 `probe.order.order_mode = "ioc" | "gtc" | "ioc+gtc"` 和同 cycle 内 order session 非阻塞 pacing `probe.sampling.order_session_interval_ms`；feedback reader、REST guard 和 single-session live order sample 尚未接入，`--execute` 仍拒绝，尚不提交真实订单。
- 当前建议分支入口：`main`。
...
| LeadLag 实盘长跑 / 测试 | `docs/lead_lag_live_runtime_plan.md`、`docs/lead_lag_live_operations_pipeline.md`、`docs/lead_lag_ack_latency_outlier_analysis.md`、`docs/lead_lag_runtime_latency_improvement_plan.md` |
...
- 2026-05-28 已新增 `docs/gate_order_session_rtt_probe_design.md`：第一版计划实现独立 `gate_order_session_rtt_probe` measurement tool，对多个 `connect_ip` 采集 `gtc_place_ack_rtt_ns`、`gtc_cancel_ack_rtt_ns` 和 `ioc_place_ack_rtt_ns`，暂不自动 score / 自动切换。RTT probe 前置 IP discovery 脚本已支持 system resolver、explicit UDP resolver、历史连接日志、candidate file、pinned WebSocket handshake 和可选 `futures.login` 验证；2026-05-28 多 resolver 1800s discovery 得到 48 个候选 IP，随后 48/48 通过 `futures.login`。probe 改为 Gate `BookTicker` 行情事件驱动，不固定只测 ZEC；每个 cycle 选 `active_session_count=8` 个 IP，默认 `order_mode="ioc+gtc"` 时先串行跑一轮 GTC place / Ack 后立即 cancel，再串行跑一轮 IOC place，也可选 `ioc` 或 `gtc` 单独子路径；GTC / IOC 各自下单前都用该 symbol 最新 BBO 和 catalog `price_limit_down * 0.5` 计算不会成交的 passive buy price；`order_session_interval_ms=0` 时同 cycle 下一个 order session 等下一条行情，非 0 时发完当前 order session 后设置非阻塞 deadline，到点用最新 BBO 下单，不额外等新行情；完整 cycle 后 `cooldown_ms=500`。当前代码已有 V1a dry-run scaffold 和 live sample 前置纯逻辑：`tools/gate/order_session_rtt_probe/`、`config/order_session_rtt_probe/gate_order_session_rtt_probe.toml`、`gate_order_session_rtt_probe` target 和 `gate_order_session_rtt_probe_test`；已覆盖配置解析、candidate IP 读取 / 去重 / 校验、dry-run plan、pinned `connect_ip` session config 派生、single-session `--live-preflight`、passive order 构造、order mode 状态流转、同 cycle order session pacing、GTC cancel reject / feedback fill / timeout -> reduce-only close 纯状态流转、close Ack 后等待 terminal feedback、close terminal filled confirmation、safety close rejected / timeout fail、sample executor 订单派发、invalid passive order 拒绝、feedback lane `local_order_id` 分配、Ack 接收时间 / stage status 记录、Ack / final response `local_order_id` 校验和 sample CSV writer 父目录创建。默认只取第一个 login-verified IP 生成 1 个 cycle，`--execute` 会在读取 candidate 文件前显式拒绝。配置解析已严格区分字段缺失和类型错误，candidate 文件非注释行必须是数字 IP，feedback / REST / continuity / dedicated-account / reduce-only safety guardrail 在 V1a 配置中不能关闭。live 前 blocker：feedback reader、REST / position-known-flat 证明和 single-session live order sample 尚未接入 live executor；REST 只做 run start preflight、fatal / continuity-lost 处理和 run end 整体账户检查 / 市价 reduce-only 兜底；connection log 仍是 planned，sample CSV writer 已实现但尚未被 live executor 调用。
- 多 Gate `OrderSession` / WebSocket 连接即使使用同一 URL，Ack RTT 分布也可能不同；相同 hostname 不保证相同 remote IP、gateway shard、per-connection queue、网络路径或本地 owner thread 状态。`OrderSession` 多连接诊断已落地 `order_session_id`、connected endpoint / owner CPU、send CPU、ack CPU、diagnostic CPU 和可选 `TCP_INFO`；`TCP_INFO` 默认关闭，需要 `order_session.diagnostics.enable_tcp_info=true`。2026-05-27 3 条独立 order session 并发 smoke 证明同一 hostname 会落到不同 remote IP，endpoint / CPU / TCP_INFO 字段在真实订单路径可用；每条连接只有 1 笔订单，不构成稳定 RTT 分布结论。多连接测试必须按 session / connection 维度记录 remote endpoint、local port、owner CPU、Ack RTT 分布和 TCP diagnostics，细节见 `docs/lead_lag_runtime_latency_improvement_plan.md`。
...
- 12-symbol live order 配置当前 `open_slippage=2` / `close_slippage=2` ticks，`max_gross_notional=2000.0`，`max_holding_position` 未启用；`RAVE_USDT`、`SIREN_USDT`、`RIVER_USDT` 走 0.1 张 decimal quantity，Gate WS 下单带 decimal header 时 quote JSON `size`。
...
当前 `--execute` 仍应在读取 candidate file、连接网络或访问账户前拒绝；真实下单前必须先接入 feedback reader、REST guard 和 single-session live order sample。
...
5. 若继续 Ack latency 复核，按 `docs/lead_lag_live_operations_pipeline.md` 使用 affinity profile，并在 report 中同时区分 Ack RTT、send-to-finish 本地闭环和 exchange Ack-to-finish；`219ms` Ack RTT outlier 当前等待复现，根因只有在复现并结合 diagnostic / pidstat / perf sched 后才能下结论。terminal lifecycle latency 先按 Gate 内部订单队列 / IOC lifecycle 候选假设标记，后续复现时单独分析，不并入 Ack path。若继续连接寻优，先读 `docs/gate_order_session_rtt_probe_design.md`，当前已有 V1a dry-run scaffold、sample flow / executor / id allocator / CSV writer 前置逻辑、`ioc` / `gtc` / `ioc+gtc` order mode、同 cycle order session 非阻塞 pacing、GTC cancel reject / feedback fill / timeout -> reduce-only close、close Ack 等待 terminal feedback 的纯状态流转和 `--live-preflight`，可先运行 `build/debug/tools/gate_order_session_rtt_probe --config config/order_session_rtt_probe/gate_order_session_rtt_probe.toml` 验证 candidate IP / cycle plan；下一步先接入 feedback reader、REST guard 和 single-session live order sample，不自动 score / 切换。
...
DataReader：先读 `docs/data_reader_config.md`。`data_reader_recorder` 已能用 `RealtimeDataReader::Drain()` 从 Gate / Binance `BookTicker` SHM 写 merged replay binary，支持 rotation + manifest replay config；recorder 是只读 SHM consumer，可和 LeadLag / demo 实盘并行，完整 dump 使用临时 `drain` 配置并观察 `overruns` / `skipped` / CPU 抢占。
...
LeadLag：先读 `docs/lead_lag_live_runtime_plan.md`、`docs/lead_lag_live_operations_pipeline.md`、`docs/lead_lag_ack_latency_outlier_analysis.md`、`docs/lead_lag_runtime_latency_improvement_plan.md`、`docs/gate_order_session_rtt_probe_design.md`、`docs/lead_lag_live_replay_testing.md` 和 `docs/lead_lag_reconcile_design.md`。真实订单 `--execute` 默认用 `config/strategies/lead_lag_requested_11symbols_live_strategy_20260522.toml`；flat-account / tiny-position / ContinuityLost stop-and-flat、ZEC 小额 filled open / close、unfilled-cancel smoke 和本地端到端 benchmark 已完成。IOC partial-fill / decimal filled close 当前不再作为 active blocker，后续遇到 terminal feedback、filled close 或 REST residual 异常再复查。2026-05-25 12-symbol 1 小时 run flat 退出但无成交，最大 Ack RTT `219.023ms`；该 Ack RTT outlier 当前等待复现后再结合新 diagnostic / 调度证据分析，不继续凭单次样本推断。2026-05-26 30 分钟拆核 run flat 退出但无成交，最大 Ack RTT `6.738ms`，最大 exchange Ack-to-finish `37.336ms` 属于 Gate IOC terminal lifecycle，不是 Ack path outlier；当前候选假设是 Gate 交易所内部订单队列 / IOC terminal lifecycle 延迟，仍需复现证明。多 Gate `OrderSession` / WebSocket 连接即使同 URL 也可能 Ack RTT 不同；已按 session 输出 `order_session_id`、connected endpoint / owner CPU、send CPU、ack CPU、diagnostic CPU 和可选 `TCP_INFO`，`TCP_INFO` 默认关闭；2026-05-27 3 session smoke 已证明字段可用且连接可能落到不同 remote IP，但 N=1 不代表 RTT 分布。WebSocket 已支持 `host` / 可选 `connect_ip` / `port`，Python 和 C++ live probe 已验证 Gate IP pinning 的 logical host 模式；`scripts/gate/discover_gate_ws_ips.py` 已生成 48 个 login verified candidate IP，当前文件为 `/home/liuxiang/tmp/login_verified_candidates_20260528_072242/candidate_ips_login.txt`。当前 `gate_order_session_rtt_probe` 已有 V1a dry-run scaffold、pinned session config builder、sample flow / executor / id allocator / CSV writer 前置逻辑和 `--live-preflight`，sample flow 已记录 Ack 接收时间 / stage status，校验 Ack / final response `local_order_id`，并覆盖 GTC cancel reject / feedback fill / timeout -> reduce-only close、close Ack 等待 terminal feedback 和 close terminal confirmation 纯状态流转；配置支持 `probe.order.order_mode = "ioc" | "gtc" | "ioc+gtc"`，`probe.sampling.order_session_interval_ms=0` 表示同 cycle 下个 order session 等下一条行情，非 0 表示非阻塞 deadline 到点后用最新 BBO 下单；可用 `build/debug/tools/gate_order_session_rtt_probe --config config/order_session_rtt_probe/gate_order_session_rtt_probe.toml` 验证 candidate IP / cycle plan。`--execute` 会在读取 candidate 文件前拒绝，配置解析为严格类型/范围校验，feedback / REST / continuity / dedicated-account / reduce-only guardrail 不能关闭。下一步先接入 feedback reader、REST guard 和 single-session live order sample，再扩展到多 `connect_ip` 采集 GTC place、GTC cancel、IOC place Ack RTT，不自动 score / 切换。live 前 blocker：REST / position-known-flat 证明尚未接入，不能只凭 close Ack 结束 sample；REST 只做 run start preflight、fatal / ContinuityLost 处理和 run end 整体账户检查 / 市价 reduce-only 兜底；sample 统计用 Nova/Quill CSV，connection log 仍是 planned，线程模型和未来 rotating / coroutine scout 方案见 `docs/gate_order_session_rtt_probe_design.md`。
