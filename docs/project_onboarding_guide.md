...
- 2026-07-10 Bitget UTA v3 单路 `OrderFeedbackSession` 已实现：独立 private connection login 后只订阅 account-wide `order` topic，按 `clientOid=a-<local_order_id>` 路由现有 feedback SHM；覆盖累计 `new` / partial / filled / cancelled facts、V1 scope validation、同 generation decode continuity latch、disconnect / producer-restart continuity、TOML config 和 login/subscribe-only probe。parser/session/config/SHM integration tests 已通过；text hot path 使用 order-data-first classification，避免每条数据重复跑 control parser。release CPU 16、3 repetitions、每项 20,000 percentile samples 的本机最新基线中，accepted median p50/p99/p99.9 为 `422/467/918 ns`，session classification→accepted 为 `422/470/944 ns`，parser→SHM→drain 为 `449/490/924 ns`，原始 JSON 在 `/home/liuxiang/tmp/bitget_order_feedback_parser_benchmark_review_final.json`。这不是 live feedback latency；REST baseline/reconcile、rate limiter、LeadLag、`fill` / `fast-fill` 和真实订单仍未实现。
- 2026-07-10 `bitget_order_session_rtt_probe` 已实现 single / multi-session 严格串行 IOC probe：direct operation Ack 与 feedback zero-fill `Cancelled` 双证据才算正常样本；unexpected fill 只尝试一次 reduce-only sell IOC safety close，并始终使 run 失败。默认 / `--live-preflight` 不读 credential、不 attach SHM、不建 WebSocket，且 `orders_sent=0`；gtest 覆盖 Ack/feedback 乱序、duplicate、timeout、continuity、late fill、safety close、bounded queue、coordinator 和真实 order feedback SHM fixture。尚未执行 `--execute`，没有 live RTT 或账户 flat 证据；工具内 REST guard、reconcile、rate limiter、LeadLag 和 fanout 仍未实现。设计与入口见 `docs/superpowers/specs/2026-07-10-bitget-order-session-rtt-probe-design.md`、`tools/bitget/order_session_rtt_probe/`。
...
1. Bitget RTT probe 下一步先由用户处理 key IP 白名单，再按外部 REST account / positions / orders baseline、独立 `OrderFeedbackSession` login+subscribe、probe `--live-preflight`、单 connection 单个 IOC 的顺序逐级验证；每一步都保留 dedicated account 和非零退出边界。工具尚无 REST guard，未取得 baseline / run-end flat 证据前不要运行 `--execute`。之后再处理正式 REST bootstrap/reconcile、reconnect unknown window、account-level rate limiter 与 LeadLag wiring。
...
Bitget `OrderFeedbackSession` 已按批准设计实现并完成 parser/session/config/SHM integration tests 与 release benchmark。`bitget_order_session_rtt_probe` 也已实现 single / multi-session 严格串行 IOC、Ack+feedback 双证据和 unexpected-fill safety close；默认和 `--live-preflight` 不触碰 credential / SHM / network，当前没有运行真实订单。下一步不是直接 `--execute`：先处理 Bitget key IP 白名单，再做外部 REST flat baseline、独立 feedback session smoke、probe preflight，最后才讨论单 connection 单个 IOC。工具内 REST guard、reconcile、rate limiter、LeadLag wiring 和 live RTT 证据仍不存在。
