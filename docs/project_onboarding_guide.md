...
| LeadLag 实盘长跑 / 测试 | `docs/lead_lag_live_runtime_plan.md`、`docs/lead_lag_live_operations_pipeline.md`、`docs/lead_lag_ack_latency_outlier_analysis.md` |
...
- 2026-05-26 已把 `219.023ms` Ack RTT outlier 的问题、已确认事实和下一轮验证建议整理到 `docs/lead_lag_ack_latency_outlier_analysis.md`。关键结论：Ack 来自 Gate order session，不是 feedback session；`request_send_local_ns` 和 `ack_local_receive_ns` 是同一 runtime 线程上的本地时间点。CPU4 在 outlier 所在窗口 10 分钟采样中满载，且 strategy / order session / feedback session 都绑 CPU4 并 active spin；本地调度 / 同核竞争是强候选，但现有日志还不能区分 OS deschedule、runtime hook 内部耗时、socket/TLS read 延迟或网络路径抖动。
...
| `docs/lead_lag_ack_latency_outlier_analysis.md` | 继续分析 LeadLag live Ack 延迟 outlier | 219ms Ack RTT 事实、同线程口径、CPU4 调度证据、下一轮 instrumentation / perf sched 建议 |
...
如果继续 LeadLag 长时间实盘运行和测试，先读 `docs/lead_lag_live_runtime_plan.md`、`docs/lead_lag_live_operations_pipeline.md`、`docs/lead_lag_ack_latency_outlier_analysis.md`、`docs/lead_lag_live_replay_testing.md` 和 `docs/lead_lag_reconcile_design.md`。当前 strategy 层订单闭环、Python REST emergency flatten helper、`lead_lag_strategy --execute` live-orders handoff、flat-account / tiny-position emergency smoke、隔离 `ContinuityLost` stop-and-flat smoke、外围 `scripts/lead_lag/run_live_with_guard.py`、ZEC_USDT `--smoke-open-close` 小额 filled open / close、`--smoke-unfilled-cancel` 小额挂单撤单 smoke 和本地端到端 benchmark 已完成。2026-05-25 requested 12-symbol live order 配置已统一 `open_slippage=2` / `close_slippage=2` ticks；同日 12-symbol guarded live-orders 1 小时运行正常退出 flat，`signals=58`、`orders=58`、`finished=58`、`filled=0`，报告在 `reports/20260525_133511_12pair_live_2ticks/`。这轮无成交，只证明 guarded 运行和反馈闭环在 cancelled-only 场景正常，不证明 IOC partial-fill / decimal filled close blocker 已复核；复核前不要继续无人值守真实订单长跑。该 run 的最大 Ack RTT 为 `219.023ms`，Ack 来自 Gate order session，同一 runtime 线程记录 send / receive；CPU4 同期满载且多个 active-spin 组件绑 CPU4，本地调度 / 同核竞争是强候选，但需要下一轮拆 CPU、加 runtime 分段时间戳并配合 `pidstat` / `perf sched` 验证。实盘启动、10 分钟巡检、report 生成和 zip 打包触发词流程已从 `AGENTS.md` 迁到 `docs/lead_lag_live_operations_pipeline.md`，`AGENTS.md` 只保留索引。普通任务不要 push，除非用户明确要求；用户输入“结束对话”时按 `AGENTS.md` 自动提交交接文档并 push。
