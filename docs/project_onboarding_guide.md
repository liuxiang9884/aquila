...
1. Cancelled order fillability：下一轮优先任务是结合当前实盘 `order_detail.csv` 和 Gate lag fusion recorder bin 检查未成交开仓 IOC 是否在 BBO 视角可成交。先读 `docs/lead_lag_cancelled_order_fillability_analysis.md`，再确认 `20260619_095317_28symbols_no_h_30d_fusion_off_l0_live` 是否仍在运行；如果仍运行，先做只读 REST dry-run、生成最新 status report，并把 `gate_live_fusion_canonical.bin` 按目标 symbol 拆分。第一批建议从 `BEAT_USDT` 或高 cancel 数 symbol 开始。
2. LeadLag live：如果复查 30-symbol run `20260606_052542_30symbols_30d_private_lagref_forceclaim`，先读 `reports/20260606_052542_30symbols_30d_private_lagref_forceclaim/report.md`、`guarded_live.stdout.log` 和 `health_checks.log`；如果复查 fusion live run `20260617_073936_28symbols_no_h_30d_fusion_off_l0_live`，先读其 tmp 运行目录、停后 REST flat 文件和 `docs/lead_lag_live_runtime_plan.md`。两者均已停止并 flat，不要按 active run 处理。新实盘启动仍按 `docs/lead_lag_live_operations_pipeline.md`，并先 rebuild release binary。
3. Freshness guard：该 30-symbol run 已启用 per-pair freshness guard。继续分析时优先检查 `lead_freshness_ns`、`lag_freshness_ns`、`freshness_guard_pass`、`freshness_reject_reason`，并把 lag freshness 拆成 `lag_exchange_ns -> lag_local_ns` 与 `lag_local_ns -> signal_decision_ns`。
...
7. TUI：下一步做 monitor 专用 Gate orders raw parser、REST snapshot、account model 和 health sampler。

## 给下一个对话的提示

先运行 `git status --short --branch` 和 `git log --oneline -8`，再读 `AGENTS.md`、`README.md`、本文件和 `docs/evaluation_support.md`。当前 branch / ahead / dirty 只信 `git status`。下一轮重点任务是“结合实盘 order 和 data 检查未成交 IOC 是否在 BBO 视角可成交”：先读 `docs/lead_lag_cancelled_order_fillability_analysis.md`，再检查当前 live run `20260619_095317_28symbols_no_h_30d_fusion_off_l0_live` 是否仍在运行。该 run 目录是 `/home/liuxiang/tmp/20260619_095317_28symbols_no_h_30d_fusion_off_l0_live/`，使用 28 symbols（删除 `H_USDT` / `TON_USDT`）、Gate / Binance fusion `N=4`、data session L0、fusion metadata off、release build、Gate private plain order / feedback；Gate lag 侧 `gate_live_fusion_source0..3.bin` 和 `gate_live_fusion_canonical.bin` 已同时落地。2026-06-19 12:43 UTC 快照 report 在 `/home/liuxiang/tmp/20260619_095317_28symbols_no_h_30d_fusion_off_l0_live/status_reports_20260619_124323/20260619_095317_28symbols_no_h_30d_fusion_off_l0_live/`，当时 REST dry-run 无挂单 / 无持仓，recorder 继续增长，异常关键字为 0；但接手时必须重新检查进程、guard、REST dry-run、recorder 增长和最新 report。fillability 分析优先从 `order_detail.csv` 筛选完全未成交开仓 cancel，用 `signal_lag_id` / `ack_lag_id` / `accepted_lag_id` / `cancelled_lag_id` 对齐 Gate lag fusion canonical BookTicker bin；字段名是 `cancelled_*_id`，不是 `canceled_*_id`。拆 symbol bin 使用 `scripts/market_data/split_book_ticker_by_symbol.py`，输出目录建议 `/home/liuxiang/tmp/book_ticker_symbol_splits/<run_id>/`。30-symbol 30 天实盘 `20260606_052542_30symbols_30d_private_lagref_forceclaim` 已在 2026-06-09 09:04:09 UTC 因 feedback continuity lost 停止，guard 已 emergency flatten 并 `verified_flat`，正式 report 在 `reports/20260606_052542_30symbols_30d_private_lagref_forceclaim/`；新 30-symbol report 记得传 `--instrument-catalog config/instruments/usdt_futures_common_gate_binance_20260602.csv`。2026-06-17 fusion live run `20260617_073936_28symbols_no_h_30d_fusion_off_l0_live` 已停止并 guard flatten，曾暴露 Gate `500 INTERNAL / Request Timeout` 被旧逻辑当确定 `kRejected` 的问题；修复提交 `9a13268` 引入 `OrderResponseKind::kUnknownResult`，后续 auto-resume 修复支持对应 terminal feedback 精确解决 unknown order 后恢复该 symbol 新开仓。新实盘前必须 rebuild 实际使用的 release binary。Gate / Binance 多路最快行情融合先读 `docs/gate_fastest_route_fusion_design.md`、`docs/gate_fastest_route_fusion_implementation_plan.md`、`docs/gate_fastest_route_fusion_shadow_results.md` 和 `docs/gate_fastest_route_fusion_threaded_bundle_plan.md`；当前 V1 已落地为独立 fusion process，按 `(symbol_id, BookTicker.id)` first arrival 输出 canonical SHM，threaded bundle 已实现但 direct SPSC ring 不继续。Data session / recorder / RTT probe / benchmark 默认放 `16-31` 测试 core，实盘 hot path 保留 `0-15`。
