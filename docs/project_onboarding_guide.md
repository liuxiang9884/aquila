...
- 2026-06-04 LeadLag 已加入开仓行情 freshness guard：`lead_freshness_ns = signal_decision_ns - lead_exchange_ns`、`lag_freshness_ns = signal_decision_ns - lag_exchange_ns`，默认 lead `5ms`、lag `20ms`，只拦截 `kOpenLong` / `kOpenShort`；close / stoploss 不受该 guard 影响。配置入口为 `[lead_lag.freshness] max_lead_freshness` / `max_lag_freshness`，策略日志和 live report CSV 已同步 `lead_local_ns` / `lag_local_ns` 与 freshness 字段。
