...
- freshness 和 taker buffer 的 auto / warmup 逻辑均不在实时策略内运行。真实订单配置应先完成启动前 preflight：freshness 用 `lead_lag_freshness_preflight` 采样 fusion / data reader BBO 并生成固定 freshness；taker buffer 用 lag BBO spread pct 加 lag `price_tick` 生成固定 slippage ticks。注意当前只有一个 `close_slippage`，它同时影响 normal close 和 stoploss。
...
1. LeadLag Go reference 迁移：下一轮优先用 replay-only `lag_vol_guard` audit 跑目标历史 / live report，不直接改实盘策略。重点比较 blocked / allowed open signal 的 submitted、filled、zero-fill cancel 和 PnL，再决定是否设计 live shadow 或 enforce。迁移边界和命令见 `strategy/lead_lag/README.md` 的“Go reference 迁移边界”和“回放与输出”。
2. Drift / close 执行差异：迁移 `drift_guard` 前先决定它和现有 `drift_limit` 是替代还是并存；迁移 `normal_close_retry_aggressive` 前先拆 `normal_close_slippage` 与 `stoploss_slippage`，否则当前 `close_slippage` 会同时影响 normal close 和 stoploss。
3. Cancelled order fillability：如果继续订单成交复查，先读 `docs/lead_lag_cancelled_order_fillability_analysis.md`，再确认 `20260619_095317_28symbols_no_h_30d_fusion_off_l0_live` 是否仍在运行；如果仍运行，先做只读 REST dry-run、生成最新 status report，并把 `gate_live_fusion_canonical.bin` 按目标 symbol 拆分。
4. LeadLag live：新实盘启动仍按 `docs/lead_lag_live_operations_pipeline.md`，并先 rebuild release binary。启动前应串联 freshness preflight 和 taker-buffer pct->slippage-ticks preflight，生成最终策略 TOML 和 audit CSV。
5. Ack latency：复现 outlier 时用 private plain all-stage config，分开看 Ack RTT、Gate `x_in -> x_out`、上行 / 下行、socket timestamping 和 pcap residual。
6. Gate / Binance 多路行情 / data session latency：继续讨论 N 路最快行情融合时，先读 `docs/websocket_client_future_optimizations.md` 的 `Live Feed Selection`、`docs/gate_fastest_route_fusion_design.md`、`docs/gate_fastest_route_fusion_shadow_results.md` 和 `docs/gate_fastest_route_fusion_threaded_bundle_plan.md`。真实订单切换 fusion 行情前必须按 live pipeline 做 preflight。
7. Gate trading：后续优先补 REST reconcile、feedback 断线恢复和更完整的 stop-and-flat 语义；继续围绕 `kUnknownResult` 增加低频 REST drift guard，避免策略状态与 Gate single-mode 净仓长期分叉。

## 给下一个对话的提示

先运行 `git status --short --branch` 和 `git log --oneline -8`，再读 `AGENTS.md`、`README.md`、本文件和 `docs/evaluation_support.md`。当前 branch / ahead / dirty 只信 `git status`。最近围绕 Go reference 迁移已明确：`freshness_auto` 和 `taker_buffer` 不放入实时策略热路径；freshness 启动前用 `lead_lag_freshness_preflight` + `scripts/lead_lag/apply_freshness_preflight_summary.py` 生成固定 `max_lag_freshness_ms`；`freshness_shadow` 已删除；taker buffer 启动前用 `scripts/lead_lag/generate_preflight_config_params.py --lag-price-tick` 把 pct 转成 `open_slippage` / `close_slippage` ticks，再用 `scripts/lead_lag/apply_taker_buffer_slippage.py` 写回策略 TOML。`lag_vol_guard` 已有 replay-only audit：用 `lead_lag_replay --lag-vol-guard-audit-output` 输出 `lag_vol_guard_audit.csv`，再用 `scripts/lead_lag/summarize_guard_audit.py` 对齐 `order_detail.csv` / `position.csv`，先评估它对 open signal 的过滤效果；不要直接把 `lag_vol_guard` / `drift_guard` 放进实盘策略。后续再考虑 `drift_guard` 与现有 `drift_limit` 的替代 / 并存关系。若讨论 close retry，先拆 `normal_close_slippage` 与 `stoploss_slippage`，因为当前 `close_slippage` 同时影响 normal close 和 stoploss。订单 fillability 任务仍可继续：先读 `docs/lead_lag_cancelled_order_fillability_analysis.md`，使用 `signal_lag_id` / `ack_lag_id` / `accepted_lag_id` / `cancelled_lag_id` 对齐 Gate lag fusion canonical BookTicker bin；字段名是 `cancelled_*_id`。新实盘启动仍必须按 `docs/lead_lag_live_operations_pipeline.md`，先 rebuild release binary，并串联 freshness 与 taker-buffer slippage preflight。
