...
9. FixedOrderedSlotPool / multi-group state：如果继续把 LeadLag `ExecutionState` 从 `std::vector<ExecutionGroup>` 迁到 FIFO ordered slot pool，先读 `docs/lead_lag_fixed_ordered_slot_pool_parallel.md`、`core/base/fixed_ordered_slot_pool.h` 和 `benchmark/strategy/lead_lag_group_container_benchmark.cpp`；迁移时必须显式处理 slot index、`group_id` / `parent_id`、pending order lookup、terminal feedback erase、行情扫描中不 erase、`parallel > capacity` 行为。当前建议先做 `ExecutionState` group-container adapter 和 `parallel=2/4` focused tests，再切默认 backend；性能结论需重新跑 group-container benchmark 和 LeadLag strategy / submit focused tests。
