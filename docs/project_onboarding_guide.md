...
| Bitget BBO market data | `docs/agent-handoff-bitget-market-data.md`、`docs/data_session_config.md`、`docs/data_session_shm_communication_design.md` |
...
- Bitget UTA public SBE `books1` BBO 已接入：`seq -> BookTicker.id`，`sts * 1000 -> exchange_ns`，`ts * 1000 -> event_ns`，data session ingress `CLOCK_REALTIME -> local_ns`，payload 仍是 72-byte `aquila::BookTicker`。当前只支持 Bitget `book_ticker`，不支持 Bitget `publicTrade`、order/private feed 或交易端。入口和配置见 `docs/agent-handoff-bitget-market-data.md`。
