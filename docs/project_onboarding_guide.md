...
- 当前 LeadLag latency 状态：2026-05-25 `219.023ms` Ack RTT outlier 已补 diagnostic / affinity / report 字段，2026-05-26 拆核 30 分钟 run 未复现；Ack RTT outlier 当前处于 inactive investigation，等待复现后再结合 order write path diagnostic、调度证据和多连接对照分析。2026-05-29 已新增下单写路径分段时间戳、socket send queue 观测和 LeadLag BBO-to-signal / signal-to-order 字段，字段说明集中在 `docs/diagnostic_fields.md`。2026-05-26 exchange Ack-to-finish `37.336ms` 属于 terminal lifecycle latency，候选假设按 Gate 交易所内部订单队列 / IOC terminal lifecycle 延迟标记，仍需复现证明。
- 当前 LeadLag live 证据：仓库默认 12-symbol live 配置仍是 `open_slippage=2` / `close_slippage=2` ticks；2026-05-29 的 3-tick / 0-tick run 只用 `/home/liuxiang/tmp/<run_id>/configs` 临时覆盖。两轮隔离 run 中 Gate data / order / feedback 均使用 private non-TLS `fxws-private.gateapi.io:80`，Binance data 仍是 public TLS；3-tick run 和 0-tick run 都正常 flat，0-tick run 仅 DASH short 1 张成交并平仓，执行滑点 0 ticks，net PnL `-0.000026064`。0-tick run 的 6 个 `504 INTERNAL Request Timeout` 是 Gate final response 阶段服务端内部处理 timeout，Ack 很快，当前日志不能定位 Gate 内部具体组件。
