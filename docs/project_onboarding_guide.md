...
| Gate / Binance fastest-route fusion | `docs/gate_fastest_route_fusion_design.md`、`docs/gate_fastest_route_fusion_implementation_plan.md`、`docs/gate_fastest_route_fusion_shadow_results.md` |
...
| Gate / Binance fastest-route fusion | `core/market_data/book_ticker_fusion.h`、`tools/market_data/book_ticker_fusion.cpp`、`tools/market_data/binance_book_ticker_fusion.cpp`、`tools/market_data/book_ticker_fusion_runner.h`、`tools/market_data/book_ticker_fusion_config.*`、`tools/market_data/book_ticker_fusion_metadata.h`、`config/market_data_fusion/gate_book_ticker_fusion_4sources.toml`、`config/market_data_fusion/binance_book_ticker_fusion_4sources.toml`、`scripts/market_data/analyze_book_ticker_fusion_latency.py`、`docs/gate_fastest_route_fusion_shadow_results.md` |
...
4. Gate / Binance 多路行情 / data session latency：继续讨论 N 路最快行情融合时，先读 `docs/websocket_client_future_optimizations.md` 的 `Live Feed Selection`、`docs/gate_fastest_route_fusion_design.md` 和 `docs/gate_fastest_route_fusion_shadow_results.md`。当前 fastest-route V1 已采用独立 fusion process，按 `(symbol_id, BookTicker.id)` first arrival 输出 canonical SHM；30-symbol Gate / Binance L4 shadow 显示 tail 改善明显，fusion hop p99 仍是 us 级。下一步应做多轮重复 shadow，评估 L4 outlier log 扰动，确认 Binance `source_3` 类异常是否稳定，并把 canonical stream 与 LeadLag `lag_freshness_ns` p95 / p99、`stale_lag_quote` reject 对齐，再讨论策略切换。
