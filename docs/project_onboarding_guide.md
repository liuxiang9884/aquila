...
1. LeadLag Go reference 迁移：下一轮优先做独立 replay / signal-only 的 Go-like guard 对照工具，不直接改实盘策略。重点先评估 `lag_vol_guard`，输出会被阻断的 open signal、`lag_vol_jump_count`、`lag_vol_amplitude`、`drift_instant`、`ratio_std`、`drift_mean` 等 audit 字段，并与 `order_detail.csv` / `position.csv` 的成交、cancel 和 PnL 对齐。迁移边界见 `strategy/lead_lag/README.md` 的“Go reference 迁移边界”。
