...
- 2026-05-28 已新增 `docs/gate_order_session_rtt_probe_design.md`：第一版计划实现独立 `gate_order_session_rtt_probe` measurement tool，对多个 `connect_ip` 采集 `gtc_place_ack_rtt_ns`、`gtc_cancel_ack_rtt_ns` 和 `ioc_place_ack_rtt_ns`，暂不自动 score / 自动切换。GTC / IOC 使用 Gate latest BBO 推导的同一 passive price 和 instrument catalog 最小下单量；意外成交时 reduce-only market close 并标记样本 invalid。RTT probe 前置 IP discovery 脚本已支持 system resolver、explicit UDP resolver、历史连接日志和 pinned WebSocket handshake 验证。第一版默认每个活跃 `OrderSession` 一个 owner thread；rotating worker 和 coroutine multi-session scout 作为未来资源优化方向。
