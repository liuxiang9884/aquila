...
- Decimal-size contract：`double quantity` + `quantity_text`；带 `X-Gate-Size-Decimal: 1` 时把 JSON `size` 编码为 string。
...
- 业务计算使用 `double quantity`，wire encoding 使用 `quantity_text`；feedback/SHM 使用累计 `double` quantity。
...
`OrderGatewayCommand` 携带 `command_seq`、`parent_id`、`local_order_id`、`exchange_order_id`、
`owner_enqueue_ns`、symbol/quantity/price 固定数组、`route_id`、side/type/TIF/reduce-only 和 command kind。

Command kind 为 place、cancel、cache exchange id、forget exchange id 和 stop。每个 child 有唯一 `local_order_id`；同一
execution group/position lifecycle 的 open、close、stoploss 和 retry 可以共享 `parent_id`。`parent_id` 不是 fanout batch id。

### Event

`OrderGatewayEvent` 携带 command/local/exchange id、request correlation、route、response/reject kind，以及 owner enqueue、
...
SHM V3 增加了 command rejection reason `kRateLimited`，但没有 heartbeat/owner-death protocol。Gate gateway
当前不会生成该 reason；它供共用 SHM contract 的 Bitget account limiter 使用。Gateway `SIGKILL` 或 crash 后 header 可能残留 stale ready；生产运行依赖外部 supervisor，
