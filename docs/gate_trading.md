...
- 多路交易：单进程 `MultiOrderSessionGateway` baseline，以及独立 `order-gateway-process` + SHM V2 + N 个 worker。
...
SHM V2 没有 heartbeat/owner-death protocol。Gateway `SIGKILL` 或 crash 后 header 可能残留 stale ready；生产运行依赖外部 supervisor，
