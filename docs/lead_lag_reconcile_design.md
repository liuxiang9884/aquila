...
账户一致性；`mark-applied` 必须接收 gateway/feedback PID，并绑定 `/proc` start time、executable、`--connect` 和实际 config；
...
幂等撤单，撤单按 5 requests/s、close 按 10 requests/s pacing；dedicated account 使用 category-wide cancel。
