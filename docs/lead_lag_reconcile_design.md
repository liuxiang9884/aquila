...
- REST flat 使用 UTA open-orders/positions，position 的 `holdMode` 必须为 `one_way_mode`，且 `total`、`available`、
  `frozen` 必须全部为 0；
...
子集证明 flat；strategy command 必须直接执行 `lead_lag_strategy`，不接受会隐藏真实 argv 的 wrapper。Guard summary 的
