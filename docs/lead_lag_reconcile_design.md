...
- `scripts/lead_lag/run_live_with_guard.py` 已实现 Gate/Bitget 启动前 REST preflight、runner 退出后 final REST check、异常退出 flatten、
  final 非 flat flatten 和 flatten failure 映射；Bitget `--execute` 还强制 manifest v2 validation、进程/账户绑定和 REST 前 quiescence。
...
| Bitget isolation | `/home/liuxiang/dev/pyenv/lx/bin/python scripts/test/lead_lag/prepare_bitget_live_run_test.py` | run-specific SHM、manifest v2、PID/start-time、route/account/config 一致性覆盖 |
