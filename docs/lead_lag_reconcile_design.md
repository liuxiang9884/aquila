...
- REST flat 完整遍历 UTA open-orders cursor，并使用 `open orders → positions → open orders` 保守 snapshot；position 的
  `holdMode` 必须为 `one_way_mode`，且 `total`、`available`、`frozen` 必须全部为 0；
- emergency close 使用反向 reduce-only market order，并在 cancel/place 返回不明确时只通过独立 REST snapshot 判定结果。

Bitget V1 不修改跨进程持久化 `local_order_id`，但普通 `clientOid` 已加入 fresh 60-bit run namespace，避免相同
local counter 确定性复用 wire identity。安全边界仍是 strict stop-and-flat：任意 continuity/unknown/进程故障
...
跨 run 审计使用固定宽度 `clientOid → namespace/local_order_id → manifest/run_id`。只有未来要实现不平仓 resume、overlap 或 crash-state recovery 时，persistent ID
...
之前要求 manifest v3 已标记 external configs applied，并验证 config path/digest、run namespace exact equality、
run-specific SHM、`route_count=1/4`、交易 contract 和
账户一致性；四路还要求所有 Bitget pair 使用 `order_session_fanout=4`，并逐 route 复核 OrderSession contract。
`mark-applied` 必须接收 gateway/feedback PID，并绑定 `/proc` start time、executable、`--connect`
和实际绝对 config；
...
不接受会隐藏真实 argv 的 wrapper；真实 `--execute` 只允许生产 REST base URL。Guard summary 的
`runtime_isolation` 保存实际 manifest/config/SHM、`strategy_lag_symbols` 和验证结果。

strategy 返回或抛异常后，guard 先执行 mutation barrier：等待 gateway 因 strategy 的 stop command 自行退出，超时后使用
PID/start-time 绑定安全地升级 `SIGTERM`/`SIGKILL`，feedback 同样在 REST 前停止。只有两个原绑定进程都已消失，才允许 final REST
或 emergency flatten；否则返回 `11` 且不生成新的 flat 成功证明。Allowlist cleanup 即使首次 snapshot 为空也执行 per-symbol
幂等撤单，撤单按 5 requests/s、close 按 10 requests/s pacing；dedicated account 使用 category-wide cancel，并在撤单后再次执行
`max_position_count` 保护。
...
- `scripts/bitget/trading/emergency_flatten_futures.py` 已实现 UTA allowlist/dedicated-account scope、cursor 分页、保守双订单 snapshot、
  幂等范围 cancel、请求 pacing、reduce-only market close、Decimal flat predicate、unknown 后独立 REST 复核和 fail-closed exit code。
- `scripts/lead_lag/run_live_with_guard.py` 已实现 Gate/Bitget 启动前 REST preflight、runner 退出后 final REST check、异常退出 flatten、
  final 非 flat flatten 和 flatten failure 映射；Bitget `--execute` 还强制 manifest v3 validation、进程/账户绑定和 REST 前 quiescence。
- `scripts/lead_lag/prepare_bitget_live_run.py` 已实现 run-specific config/SHM/manifest 生成，以及带 PID/start-time/config/account
  证明的 `mark-applied` 复验。
...
| Bitget isolation | `/home/liuxiang/dev/pyenv/lx/bin/python scripts/test/lead_lag/prepare_bitget_live_run_test.py` | run-specific SHM、manifest v3、clientOid namespace/config digest、PID/start-time、route/account/config 一致性覆盖 |
| Guard process barrier | `/home/liuxiang/dev/pyenv/lx/bin/python scripts/test/lead_lag/run_live_with_guard_test.py` | gateway/feedback quiescence、TERM/KILL、有界 strategy stop 与 REST 前 fail-closed 顺序覆盖 |
