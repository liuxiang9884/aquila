...
2026-06-15 后续讨论新增一个同进程多线程 bundle 方向：一个 process 内运行 N 个 data session thread、
1 个 fusion thread 和 1 个统一 log backend thread，data session 到 fusion 仍通过 source SHM 通信，
fusion 后继续输出 canonical SHM，便于保留 `DataReader` / recorder 监控边界。该方向不是替换本文件
已完成的多进程 V1，而是作为下一轮 shadow 的 A/B 对照；详细 V1 threaded bundle 计划和 V2 direct
SPSC ring 建议见 `docs/gate_fastest_route_fusion_threaded_bundle_plan.md`。
