...
使用 order gateway 时先 validate config/SHM、route count/readiness 和 account consistency。Bitget 首个 live 固定 fanout=1，
...
`bitget_gateway__*.toml` 启动 `bitget_order_gateway --connect`，确认 feedback subscribed ready、gateway fanout=1/route ready，
再执行：
