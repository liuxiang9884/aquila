...
`121/113 ns`。local order ID、final JSON writer 和 decimal digit writer 已在当前
非拓扑 branch 完成多种 fresh candidate；虽然 component 一度改善 `3%–55%`，但
Gate/Bitget direct、SHM、相邻 place/cancel 或 handoff 至少一项未过门槛，所有候选均已
撤销。详细结果见性能报告“numeric request 之后的 OrderSession formatter 候选”。
当前状态仍只信 `git status --short --branch`。

非拓扑 formatter 阶段现已完成。线程 / 进程拓扑必须从已接受 HEAD 新建独立
branch/worktree。开始前
