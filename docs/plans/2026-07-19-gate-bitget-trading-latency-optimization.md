...
最新 fresh profile 包括 `gate-feedback-parser-gprof-after-d4102fc`、
`bitget-feedback-parser-gprof-after-d4102fc` 和
`gate-compiled-order-request-formats/gprofng-candidate-v1.er`。数值订单 request 候选因
Gate/Bitget submit 回归被撤销；继续 profile 后拆出的 Gate format compile 已接受。当前
worktree 应保持 clean，准确状态仍只信 `git status --short --branch`。
...
6. OrderSession 使用 `fmt::format_to_n` 和动态 fixed precision `"{:.{}f}"` 直接写最终固定
   JSON buffer，不生成中间 `std::string` 或价格/数量 text buffer。
...
6. **Gate / Bitget OrderSession**
   ...
   - 两边用 `fmt::format_to_n` 动态 precision 直接写最终 JSON buffer。
...
撤销后继续 profile，拆出的独立 Gate fixed-format 优化已作为 `30e27fe` 接受。它不修改
request contract，只消除现有 text request 最终 JSON 的运行时 format 解析；详细五组 A/B、
MAD、tail 边界和 replay 证据见
`docs/gate_bitget_trading_latency_optimization_report.md`。
