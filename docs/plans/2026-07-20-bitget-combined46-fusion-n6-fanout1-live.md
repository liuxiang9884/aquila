...
- 以 numeric order request 优化提交 `cd1163c` 为代码基线，并保留上一轮实盘使用的
  Bitget 账户级 command limiter。
- 沿用
...
- 代码从 `cd1163c` 建立独立 live branch/worktree，并组合已验证的账户 limiter 提交
  `f8ccfcf`；组合后重新构建和运行 focused tests。
...
- Order gateway 固定单 route、`order_session_fanout=1`，并启用
  `max_commands_per_second=5`、`reserved_exit_commands=2` 的 fail-closed limiter。
...
- 生产代码只组合 numeric order request 与既有 Bitget account limiter，不新增新的交易语义。
...
1. 组合并 review `f8ccfcf`，解决与 numeric request ABI 的最小冲突。
...
8. 每 10 分钟检查关键进程、route ready、freshness、订单生命周期、continuity/unknown、
   limiter、REST/SHM health 和 recorder overruns。
...
- `git diff --check`
- Bitget account limiter config/unit tests
...
- `UnknownResult`、`ContinuityLost`、unresolved order、limiter 触发或关键进程退出时立即结束
...
- Numeric request 与 account limiter 首次组合，需要 fresh build/test 和 live preflight 证明。
