...
- 2026-05-28 已形成并开始落地 Gate `OrderSession` RTT probe measurement-only 工具，见 `docs/gate_order_session_rtt_probe_design.md`。第一版目标是对多组 `connect_ip` 采集 `gtc_place_ack_rtt_ns`、`gtc_cancel_ack_rtt_ns` 和 `ioc_place_ack_rtt_ns`，暂不自动 score、暂不自动切换生产 `OrderSession`。多 resolver 1800s discovery 已得到 48 个候选 IP，随后 48/48 通过 `futures.login`；当前 RTT probe 连接集合已迁到 `probe.inputs.connections_file` 指向的 CSV，每行定义一条独立 `OrderSession` 连接，`connect_ip` 允许重复。当前 V1a 代码已有 dry-run plan、`--live-preflight`、pinned session config builder、sample flow / executor / id allocator 和 sample CSV writer；sample flow 已记录 Ack 接收时间 / stage status，校验 Ack / final response `local_order_id`，并已覆盖 GTC cancel reject、feedback fill / timeout 后进入 reduce-only close、close Ack 后等待 terminal feedback 和 close terminal confirmation 的纯状态流转。2026-05-29 已补 `probe.order.order_mode = "ioc" | "gtc" | "ioc+gtc"` 和 `probe.sampling.order_session_interval_ms`；间隔为 `0` 时下一个 order session 等下一条行情，非 `0` 时只设置非阻塞 deadline，到点用最新 BBO 下单，不额外等新行情。2026-05-31 已把默认 12 连接测试列表落到 `config/order_session_rtt_probe/gate_order_session_rtt_connections.csv`。
...
4. **OrderSession RTT probe 先做 measurement-only**
   ...
   - safety path 优先走 WebSocket reduce-only market close：GTC cancel reject / terminal 不确定时立即补 close，IOC place Ack 后因没有 cancel 也立即补 close；无仓位导致的 reduce-only reject 只有在 terminal feedback、REST 或 position-known-flat 证明 flat 后，才视为 flat-safe。
