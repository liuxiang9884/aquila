...
已落地：本轮 report 已能解析 `gate_order_ack_latency_diagnostic`，并将 outlier 字段同步写入 `order_detail.csv` / `latency.csv`；字段说明已同步到 `docs/lead_lag_live_report_csv_schema.md`。`report.md` 会展示最多 10 条 latency diagnostic outlier 摘要。
...
`run_live_with_guard.py` 的 guard stdout 如果包含 `affinity` summary，`generate_live_report.py` 会在 `report.md` 的 Runtime Profile 中记录 `profile_name`、`affinity_split`、`core_path` 和 generated config 路径。
