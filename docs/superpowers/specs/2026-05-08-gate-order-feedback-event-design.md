...
- 使用 Task1 已确认的宽结构 `OrderFeedbackEvent` 作为第一版 SHM ABI。
...
Task1 `OrderFeedback SHM Transport` 已选择方案 A：宽结构 `OrderFeedbackEvent` 作为第一版 SHM ABI。方案 B
tagged union 只作为后续对照方案保留，不进入第一版实现。
...
1. Task1 SHM transport 实现后的实际 publish / poll benchmark 数值和 queue full 诊断口径。
2. `local_receive_ns` 进入 event 后，Strategy 消费时间是否只保留在 diagnostics。
3. 断线 / 重连后未知订单状态如何通过 REST reconcile 恢复。
4. `OrderSession` 内部 `local_order_id -> exchange_order_id` cache 的公开更新接口命名和清理时机。
5. 是否需要单独的 unsupported terminal / parse error diagnostics。
