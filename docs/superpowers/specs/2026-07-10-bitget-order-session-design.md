...
- 实现状态：已实现并完成 login-only、真实 passive IOC RTT probe 和多轮 review
...
本文原始范围只定义上行 place / cancel 通道及其直接 operation response。后续已经分别实现
`OrderFeedbackSession`、feedback SHM、Bitget order gateway、LeadLag gateway 接入和真实 RTT probe；这些后续组件的职责不
反向改变本文的 operation response 语义。
...
- 已登录连接上的 `30004`、`30005`、`30007`、`30011`–`30015`、`30033` 立即进入 not-ready 并请求重连；即使
  error 带 operation `id`，也必须先完成该 request 的保守 response 映射，再使 session 失效。初始 login 收到明确 credential
  reject 时只保持 not-ready，不用相同 credential 盲目重连；`30004`、`30007`、`30033` 仍按 connection error 重连。
...
本设计完成后的统一未覆盖边界和未来方向见 `docs/bitget_trading_follow_up.md`。原始后续入口包括：
