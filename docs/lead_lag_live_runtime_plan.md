...
- Gate submit / cancel `5xx` response 只表示请求结果未知，不是确定拒单。当前 Gate runtime adapter 会把这类 response 映射为 `OrderResponseKind::kUnknownResult`；`OrderManager` 保留 pending order 等待 private feedback，LeadLag 标记对应 symbol `needs_reconcile` 并暂停新开仓，直到 terminal feedback 或后续 REST reconcile 能确认状态。
