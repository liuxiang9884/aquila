...
- `exchange/gate/market_data/data_session_config.*` 和
  `exchange/binance/market_data/data_session_config.*`：解析 top-level `[book_ticker_shm]`。
...
- `test/core/market_data/data_shm_test.cpp`：producer / reader / overrun / heartbeat 单元测试。
- `benchmark/core/market_data/data_shm_benchmark.cpp`：publisher 和 reader microbenchmark。

配置入口为 top-level `[book_ticker_shm]`：
...
```text
BM_DataShmPublisherOnBookTicker   4.67 ns CPU 4.67 ns
BM_BookTickerShmReaderTryReadOne  1.94 ns CPU 1.95 ns
```
