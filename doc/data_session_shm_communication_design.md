...
`Consumer::OnBookTicker()` 接口把 `aquila::BookTicker` 发布到跨进程共享内存。
...
`aquila::BookTicker`。直接 typed broadcast ring 更短，也更贴合现有 data session consumer
接口。
...
using BookTickerQueue =
    nova::static_impl::SPBroadcastQueue<aquila::BookTicker, kCapacity>;
```
...
producer 是 data session 进程中的 SHM consumer，例如：

```cpp
class ShmBookTickerConsumer {
 public:
  void OnBookTicker(const aquila::BookTicker& book_ticker) noexcept {
    queue_.Push(book_ticker);
    ++published_count_;
    header_->published_count = published_count_;
    header_->heartbeat_ns = NowNs();
  }
};
```
...
DataSession
  ...
  -> ShmBookTickerConsumer::OnBookTicker(BookTicker)
...
- 后续可以通过更严格的 slot sequence 方案升级，而不改变上层 `Consumer::OnBookTicker()` 抽象。
...
- `ShmBookTickerConsumer`，适配现有 Gate / Binance `DataSession<Consumer>`。
- `BookTickerShmReader`，提供 `TryReadOne()`。
- data session tool 增加可选 `--shm-publish` 或 TOML 字段。
