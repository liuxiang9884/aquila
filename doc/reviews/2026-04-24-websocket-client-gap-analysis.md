...
- 处理方案：fix — 提交 `ded4899` 将 `FrameCodec` 入站解码改为 Linux mirrored receive ring + fixed `ReadyFrame` metadata ring。`CriticalSession::DriveRead` 不再先读入独立 `read_buffer_` 后 `Feed`，而是直接把 `FrameCodec::WritableSpan()` 传给 TLS socket，`CommitWritten()` 后 `Poll()` 交付 `MessageView::payload`。payload 在常规路径和跨物理尾部路径都以单段 `std::span<const std::byte>` 指向 mirrored ring 的虚拟连续区间；ready queue 不拥有 payload。后续 direct delivery 优化把默认 `Poll()` 路径改为每次直接解析并交付一帧，ready metadata ring 保留为 future batch / parse-ahead fallback。
- 验证证据：新增 `websocket_frame_codec_test` 覆盖 mirrored boundary payload 连续交付、`ready_frame_slots == 0` 时 coalesced 多帧 direct delivery、receive ring 初始化失败返回 `kCapacityExceeded`；新增 `websocket_critical_session_test` 覆盖 codec capacity exhaustion 只计入指标、不触发 reconnect；新增 `websocket_degraded_evaluator_test` 覆盖 codec capacity 1s 窗口触发 degraded。debug 与 release 下 `ctest --test-dir build/<type> -R websocket_ --output-on-failure` 均为 14/14 通过。静态检查 `rg -n "std::vector|std::deque|inbound_bytes_|decoded_payload_|ready_frames_\\.push|erase\\(" core/websocket/frame_codec.h` 无命中。release pinned benchmark（`taskset -c 2`）显示：旧 `frame_codec` encode p50/p99/p99.9 = 376/531/577ns；P2-A 后 `frame_codec_encode` = 382/534/604ns、`frame_codec_decode_contiguous` = 15/19/86ns、`frame_codec_decode_mirrored_boundary` = 77/90/213ns；direct delivery 后 `frame_codec_encode` = 377/532/590ns、`frame_codec_decode_contiguous` = 9/10/24ns、`frame_codec_decode_mirrored_boundary` = 57/72/193ns。端到端 `session_read_path` pinned 当前值为 p50/p99/p99.9 = 408/439/2478ns。
- 确认日期：2026-04-25
...
- 处理方案：fix — 提交 `ded4899` 新增 `core/websocket/mirrored_buffer.h`，在构造期用 `memfd_create` + 双 `mmap` 建立 mirrored ring；运行期不依赖 `std::vector` / `std::deque` 扩容。新增 `DecodeStatus::kCapacityExceeded`、`Metrics::frame_codec_capacity_exhaustions`、`Metrics::frame_codec_ready_high_watermark`、`ConnectionConfig::max_frame_payload_bytes`、`ConnectionConfig::ready_frame_slots`，并将 codec capacity 事件接入 `DegradedEvaluator` 的 1s 窗口。
- 验证证据：`websocket_types_test` 覆盖新增配置默认值；`websocket_critical_session_test` 覆盖 codec receive ring 初始化失败时的 capacity exhaustion 指标；`websocket_degraded_evaluator_test` 覆盖 `frame_codec_capacity_events_per_second` 默认触发路径。direct delivery 后，`ready_frame_slots == 0` 不再阻止单帧 direct delivery，ready ring 容量不再作为默认 decode 主路径的容量耗尽来源。release pinned 无关热路径对比：`session_write_path` p50/p99/p99.9 = 414/435/949ns → 415/436/564ns，`active_spin` = 42/43/44ns → 42/43/44ns，`prepared_write` = 2/2/3ns → 2/2/2ns。
- 确认日期：2026-04-25
