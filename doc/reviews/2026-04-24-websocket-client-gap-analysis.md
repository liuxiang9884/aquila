...
- 验证证据：`websocket_types_test` 覆盖新增配置默认值；`websocket_critical_session_test` 覆盖 ready frame slot 为 0 时的 capacity exhaustion 指标；`websocket_degraded_evaluator_test` 覆盖 `frame_codec_capacity_events_per_second` 默认触发路径。release pinned 无关热路径对比：`session_write_path` p50/p99/p99.9 = 414/435/949ns → 415/436/564ns，`active_spin` = 42/43/44ns → 42/43/44ns，`prepared_write` = 2/2/3ns → 2/2/2ns。
- 确认日期：2026-04-25
