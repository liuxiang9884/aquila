...
- 当前边界：LeadLag strategy 层生产订单闭环已完成；真实 runner `--execute` 仍禁用，当前优先补 `ContinuityLost` 后 stop-and-flat 应急链路、account / position 复核、unfilled-cancel / failure live smoke 和端到端 benchmark。更复杂的 read-only reconcile / resume 作为后续 V2。
- 当前建议分支入口：`main`。
...
- `doc/lead_lag_live_runtime_plan.md` 已记录 LeadLag signal-only 长时间实盘观察、strategy 层真实订单闭环、runner `--execute` 禁用边界、`ContinuityLost` stop-and-flat 应急链路、异常 smoke 和端到端 benchmark 的推进顺序。
- LeadLag default production accounting 已可提交 IOC limit order intent；`tools/lead_lag/live_strategy.cpp::RunLiveOrders()` 仍是禁用 stub，未打开真实订单 runner。
...
3. 扩展 live smoke：Python emergency flatten、feedback session 断线 / `ContinuityLost` stop-and-flat、unfilled-cancel、rejected / cancel-rejected。
...
3. 保持 `lead_lag_strategy --execute` 为禁用边界，先补 `ContinuityLost` stop-and-flat 应急链路。
4. 按计划补 Python emergency flatten、feedback 断线 / `ContinuityLost` stop-and-flat smoke、unfilled-cancel、rejected / cancel-rejected 和端到端 benchmark，再评估打开真实 runner。
