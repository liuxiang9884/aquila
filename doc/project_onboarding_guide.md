...
- 当前边界：LeadLag strategy 层生产订单闭环已完成；`lead_lag_strategy --execute` 已接到真实 live-orders runtime，并在 `ContinuityLost` 后停止、返回 handoff exit code。V1 flat-account、tiny-position、continuity-lost stop-and-flat、ZEC 小额 filled open / close、unfilled-cancel smoke 和本地端到端 benchmark 已完成；外围 `run_live_with_guard.py` 已负责 preflight、final REST check 和异常 stop-and-flat。2026-05-22 release 11-symbol live-orders guarded run 只完成 1 组完整 strategy open / close，并暴露 Gate IOC partial-fill terminal feedback 缺失和 decimal-size flat 判断不足；修复前不要继续无人值守真实订单长跑。当前版本不新增独立 `AccountPositionFeedbackSession`；account / position realtime feedback 作为 V2 可选能力。TUI 当前仍是只读 monitor demo：market data 可从现有 Gate / Binance `BookTicker` SHM 读取并降级显示 `NA`，订单、仓位、PnL 和 health 还未接真实账户数据。
- 当前建议分支入口：`main`。
...
- Gate decimal-size 合约的 catalog metadata 已从 `order_size_min` 推导 `quantity_step` / `quantity_decimal_places`；`RAVE_USDT`、`SIREN_USDT`、`RIVER_USDT` 的 Gate 行当前为 `quantity_step=0.1`、`quantity_decimal_places=1`。这只是 metadata 修正，完整 decimal quantity 下单、回报和 REST flat 判断仍需后续覆盖。
...
2. 如继续 signal-only 长跑，可使用 `config/strategies/lead_lag_requested_11symbols_strategy_20260522.toml` 观察 11 个 requested symbol；decimal-size 合约的 catalog metadata 已能表达 1 位小数，但完整 decimal quantity 下单 / 回报 / REST flat 支持仍未完成。
...
4. 下一步优先修复 2026-05-22 release run 暴露的两个 blocker：Gate private feedback 对 IOC partial-fill / partial-cancel terminal event 的发布 / 消费，以及 REST final check / emergency flatten 的 decimal quantity / residual flat 判断。修复前不要继续无人值守真实订单长跑。
