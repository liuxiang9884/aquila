...
62. `BookTickerShmReader` overrun 边界恢复为完整 capacity 窗口：`unread_count == capacity` 时仍读取当前 ring 中最老的一条已发布 `BookTicker`，只有 `unread_count > capacity` 才记录 overrun 并拉回 `current - capacity`；第一版不主动丢边界 BBO，若未来需要严格检测边界覆盖，应升级 per-slot sequence。2026-05-19 release benchmark：`BM_BookTickerShmReaderTryReadOne` 为 `1.95ns`，`BM_RealtimeDataReaderEmptyPoll` 1/2/4 source 为 `1.21ns` / `2.29ns` / `4.04ns`。
