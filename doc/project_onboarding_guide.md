...
- 当前边界：LeadLag strategy 层生产订单闭环已完成；`lead_lag_strategy --execute` 已接到真实 live-orders runtime，并在 `ContinuityLost` 后停止、返回 handoff exit code。V1 flat-account、tiny-position、continuity-lost stop-and-flat 和 ZEC 小额 filled open / close smoke 已完成；仍缺 unfilled-cancel、rejected / cancel-rejected、account / position feedback 和端到端 benchmark。更复杂的 read-only reconcile / resume 作为后续 V2。
- 当前建议分支入口：`main`。
...
3. V1 emergency smoke、外围 guard wrapper 和 ZEC 小额 filled open / close smoke 已完成；下一步通过 `scripts/lead_lag/run_live_with_guard.py` 继续 unfilled-cancel、rejected / cancel-rejected。
