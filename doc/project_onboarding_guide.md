...
3. 扩展 live smoke：继续补 rejected / cancel-rejected，并在后续 account / position feedback 接入后补对应 smoke。
...
3. V1 emergency smoke、外围 guard wrapper、ZEC 小额 filled open / close 和 unfilled-cancel smoke 已完成；下一步通过 `scripts/lead_lag/run_live_with_guard.py` 继续 rejected / cancel-rejected。
