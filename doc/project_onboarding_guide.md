...
| `core/strategy/strategy_context.h` | strategy 的窄下单接口，封装 `OrderManager` 的 `PlaceLimitOrder()`、`CancelOrder()` 和 `FindOrder()`，不暴露 runtime 内部对象。 |
| `core/strategy/trading_runtime.h` | `TradingRuntime<StrategyT, OrderSessionT, DataReaderT>` production runtime；从已解析 config 构造 data reader / order session / order manager / context / strategy / feedback reader，`Run()` 负责 ready gating、order response、feedback 和行情 poll，并在订单事件 hook 前先更新 `OrderManager`。 |
| `test/core/strategy/trading_runtime_test.cpp` | runtime 回归测试，覆盖 non-copy/move、行情分发、order response 分发、order feedback 分发、feedback disabled、session 构造失败、正式 `Run()`、ready gating、`ShouldStop`、feedback reader poll 和 session stopped failure。 |
...
- Gate-specific 构造留在 `tools/gate/trading_runtime_adapter.h` 和 tool 层，core runtime 不 include `exchange/`；Gate adapter 不创建后台线程、不维护 command queue，place/cancel 在 StrategyThread / Gate session 同线程直接调用。完整结构图见 `doc/strategy_order_component_model.md` 的“TradingRuntime 组装链路”：`DataReader -> TradingRuntime -> Strategy`，下单走 `StrategyContext -> OrderManager -> GateOrderSessionAdapter -> Gate OrderSession`，feedback 走 `OrderFeedbackSession -> SHM -> TradingRuntime -> OrderManager -> Strategy`。
...
5. 如果继续 trading runtime，从 `core/strategy/trading_runtime.h`、`tools/gate/trading_runtime_adapter.h`、`tools/gate/demo_strategy.h` 和 `tools/gate/demo_strategy.cpp` 接手；当前 production `Run()` hook mode / Gate adapter / demo dry-run tool 已实现。
...
9. 增加端到端 benchmark：覆盖 `TradingRuntime -> GateOrderSessionAdapter -> OrderSession` 下单请求和 `OrderFeedbackSession -> SHM -> TradingRuntime` 回报消费；真实链路性能结论必须另跑 live probe 或 profile。
