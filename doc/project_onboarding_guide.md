...
- 第 4 部分 drift / alignment phase 和第 5 部分 threshold engine 已补 C++ 落地设计：第 4 部分目标文件是 `strategy/lead_lag/types.h`、`window_stats.h`、`alignment.h` 和 `test/strategy/lead_lag_alignment_test.cpp`；第 5 部分目标是 `MoveQuantileWindow` 和 threshold state，使用两个 histogram、显式 range / precision 配置、fixed Go roll 顺序和 `fee*2 + LeadNoise + LagNoise` profit buffer。当前仅文档落地，尚未执行实现。
...
4. 第 3 部分 recorder / queue / noise / spread / move quantile 设计已补齐，底层抽象数据结构已落地到 `core/base/`；第 4 部分 drift / alignment phase 和第 5 部分 threshold engine 的 `aquila` C++ 落地设计已写入设计文档，但代码尚未执行。
5. 在实现计划前，先决定 BBO extrema 是否继续用 `BookTicker.local_ns`，还是为了 fixed replay 对账引入 server timestamp 输入。
6. 进入实现时，可先执行第 4 部分 `AlignmentState` / `MeanWindow` / `MeanStdWindow` 单元测试与实现，再实现第 5 部分 `MoveQuantileWindow` / threshold state；随后继续组合 `BboExtremaWindow`、`NoiseState`、`SpreadState`，并补针对 fixed Go 语义的单元测试和 replay 对账测试。
