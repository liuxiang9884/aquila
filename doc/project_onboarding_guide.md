...
- 当前边界：LeadLag strategy 层生产订单闭环已完成；`lead_lag_strategy --execute` 已接到真实 live-orders runtime，并在 `ContinuityLost` 后停止、返回 handoff exit code。V1 flat-account、tiny-position、continuity-lost stop-and-flat、ZEC 小额 filled open / close 和 unfilled-cancel smoke 已完成；submit rejected / cancel-rejected 安全 live 探测未拿到最终 failure response，不计入已完成 smoke。仍缺 account / position feedback 和端到端 benchmark。更复杂的 read-only reconcile / resume 作为后续 V2。
- 当前建议分支入口：`main`。
...
4. 之后补 account / position feedback、端到端 benchmark 和更长时间真实订单运行 guardrails；failure response 继续前先确认 Gate 可返回最终 error 的请求形态。
