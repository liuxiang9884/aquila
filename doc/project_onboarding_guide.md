...
| `doc/runtime_config.md` | 修改 `config/runtime*.toml`、`config/runtime/` 或新增 session 配置时读 | 生产按进程拆分 runtime、`instrument_catalog`、`data_sessions.subscribe_symbols`、symbol pool 生成、WebSocket endpoint / execution_policy / read_path / heartbeat / reconnect 字段和默认值。 |
