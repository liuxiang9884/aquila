...
23. `OrderManager` 第一版订单框架已按 Sirius 风格重构：`core/trading/order_pool.h`、`core/trading/order_types.h` 和 `core/trading/order_manager.h` 覆盖通用固定容量订单池、订单创建、状态推进和直接 session 发送；公共 order/runtime contract 统一在 `aquila::core` 下，旧 `core/strategy/*` 与 `strategy/order_types.h` / `strategy/order_manager.h` 兼容头已删除，新代码不再通过 `aquila::strategy` 暴露公共订单类型；`OrderManager` 不维护 exchange order id 索引，不再缓存 Gate wire fields，也不再暴露 `PrepareOrder()` / `SubmitOrder()` 两阶段接口。
...
27. Gate `OrderFeedbackSession` 第一版 event 语义已收敛到文档：第一版只以 private `futures.orders` 为生命周期事实源，不接 `futures.usertrades`；已定义 accepted、partial filled、filled、cancelled/terminal finished、rejected 和 `kContinuityLost` 的边界。Task1 已实现宽结构 `OrderFeedbackEvent` 作为第一版 SHM ABI，并加入 continuity lost control event；tagged union 暂不进入第一版实现。
...
29. Order feedback Task1 SHM transport 已实现：固定 8 lane、Nova SPSC、宽结构 `OrderFeedbackEvent`、`OrderFeedbackShmPublisher`、`OrderFeedbackShmReader`、TOML config parser、tests 和 `order_feedback_shm_benchmark` 均已落地；feedback continuity lost 以 `OrderFeedbackKind::kContinuityLost` event 投递到 lane，不再通过 shared epoch atomic 进入 Strategy。`OrderFeedbackShmManager` 初始化 / attach 通过 `Create()` / `Open()` / `OpenOrCreate()` 返回 `Result`，不向上层暴露 throwing constructor。
...
31. Task2 Gate order feedback session 已实现：`exchange/gate/trading/order_feedback_parser.h` 解析 private `futures.orders` SBE orders payload，`OrderFeedbackSession` 登录后订阅 `futures.orders`，binary path 解析后调用 Task1 `OrderFeedbackShmPublisher::Publish()`，断线时使用 `PublishGlobalContinuityLost(...)` 向 8 lane fanout `kContinuityLost` event。
32. `OrderManager` 已实现 `OnOrderFeedback()`：accepted 保存 `exchange_order_id` 并通知同线程 `OrderSession` cache；partial fill 更新累计成交但在 `kCancelSent` 下保持撤单挂起状态；filled / cancelled terminal event 幂等终结订单并清理 cache；`kContinuityLost` event 设置 `feedback_continuity_lost_detected`。
...
35. `doc/leadlag-fixed-strategy-reconstruction-guide.md` 已保存 current fixed LeadLag 策略重建手册；`config/strategy.zip` 已解压到被 git ignore 的 `third_party/strategy/` 作为 fixed Go 源码参考；`doc/superpowers/specs/2026-05-08-leadlag-fixed-strategy-aquila-design.md` 已按 fixed 语义还原 + `aquila` 链路对齐方式完成 1-7 部分设计拆解。LeadLag 第 1-7 部分策略层模块已落地：config / metadata、raw market state、recorder wrappers、drift / alignment、threshold、signal / execution state、feedback state / order retire 均已有 GTest 覆盖；后续又补齐了 replay `OnBookTicker()` 信号主链路和 ORDI_USDT Tardis / HDF 对账入口。
36. Trading runtime production loop 已落地：`core/trading/trading_runtime.h` 支持 `TradingRuntime<StrategyT, OrderSessionT, DataReaderT>`，生产 `Create()` 从已解析 `StrategyConfig` / `DataReaderConfig` 构造 `DataReader`、`OrderSession`、`OrderManager`、`StrategyContext`、strategy 和可选 feedback reader；Gate production 路径使用 `OrderSessionT::SetRuntimeHook()` 在 WebSocket active spin loop 同线程轮询 feedback SHM / data reader，`OnOrderResponse()` 和 `OnOrderFeedback()` 都先更新 `OrderManager` 再调用 strategy hook，并支持 `OnStart` / `OnLoop` / `OnIdle` / `OnStop` / `ShouldStop`、`spin` / `yield` idle policy、`max_loop_seconds` 和 best-effort `bind_cpu_id`。
37. Gate order session runtime adapter 和 `demo` 策略工具已落地：`exchange/gate/trading/order_session_runtime_adapter.h` 把 Gate `OrderSession` 包装为 runtime 可用的 `OrderSessionT`，通过 `BindRuntime()` 让 Gate response handler 同线程直接回调 `TradingRuntime::OnOrderResponse()`，不再保留 production response queue、background order session thread 或 command queue；place / cancel 直接转发给同线程 Gate session。`tools/gate/demo_strategy.h` / `tools/gate/demo_strategy.cpp` 提供 `demo` strategy 和 `gate_demo_strategy` 工具，默认 dry-run 只解析配置，不打开 WebSocket / SHM；显式 `--execute` 才进入实盘 runtime。2026-05-20 已用临时 3 轮配置完成 `gate_demo_strategy --execute` live smoke。
...
44. LeadLag `Strategy::OnBookTicker()` replay 信号主链路已串接：raw market state、drift / alignment、recorder、threshold、signal engine 和 synthetic position accounting 已形成闭环；`tools/lead_lag/replay.cpp` 可用 `PositionAccountingMode::kSyntheticSignals` 从 `BookTicker` binary 生成 signal CSV。`strategy/lead_lag/README.md` 已作为策略目录级快速说明。
45. ORDI_USDT 20260415～20260417 Tardis `book_ticker` CSV 已转换为每日一个 `aquila::BookTicker` binary，默认路径为 `/home/liuxiang/tardis/merged_book_ticker/ORDI_USDT/*.bin`；`config/data_readers/lead_lag_ordi_binary_replay.toml` 指向这组三天文件。
...
51. `doc/trading_component_architecture_discussion.md` 已作为交易组件架构持续讨论入口，当前已细化 `DataReader`：`DataReader` 是 Strategy-facing concept / 接口约束，不要求虚基类；`RealtimeDataReader` 和 `HistoricalDataReader` 通过 `Poll(handler)` 单事件接口和 `Drain(handler, max_events)` 批量接口保持一致；`HistoricalDataReader` 额外提供 `finished()`，且目标语义中 replay `Poll() == 0` 表示 EOF；`DataReader` 不做 merge，realtime merge 归上游 data session / producer，history merge 归离线预处理；第一版 replay 只要求 binary source；`poll_calls` / `empty_polls` 归 runtime / scheduler / 组装层 diagnostics，不放在 reader 内部 stats。
52. DataReader Poll / Drain 目标语义已落地到当前实现：`core/market_data/data_reader_concepts.h` 提供 `PollDataReaderLike` / `DrainCapableDataReader` / `DataReaderLike` / `FiniteDataReader`；`core/market_data/realtime_data_reader.h` 的 `Poll()` 已改成单事件 round-robin，`Drain()` 承担批量消费；`core/market_data/historical_data_reader.h` 的 `Poll()` 已改成单事件 replay step，并新增 `Drain()` / `finished()` / `kFiniteDataReader`；reader stats 已改为 `total_count`、per-source `book_ticker_count` 和 replay `files_completed`。`TradingRuntime` 对 live reader 每轮调用 `Poll()`，只对同时满足 `FiniteDataReader` 和 `DrainCapableDataReader` 的 replay / finite reader 使用 `Drain(runtime, data_reader.max_events_per_source)`。
53. Reader 实现类已完成命名整理且不保留兼容 alias：原实时 SHM reader 现为 `RealtimeDataReader`（`core/market_data/realtime_data_reader.h`），原 binary replay reader 现为 `HistoricalDataReader`（`core/market_data/historical_data_reader.h`）；对应 diagnostics / stats / tests / CMake target 已同步改名。`DataReaderConfig` 和 `data_reader_concepts.h` 仍保持通用命名。
54. 第一轮 reader 性能 review 的 P1/P2 已落地：新增 `data_reader_benchmark`；`RealtimeDataReader::Poll()` 去掉运行期取模并增加单 source 快路径，hot `Source` 不再保存完整 config；`RealtimeDataReader` / `HistoricalDataReader` 去掉读前 `BookTicker` 清零；`HistoricalDataReader` 把文件状态机边界处理移出单条记录热路径。2026-05-19 release benchmark：realtime empty poll 1/2/4 source 从 `3.08ns` / `6.18ns` / `12.3ns` 降到 `1.36ns` / `2.63ns` / `4.29ns`；historical drain 4096 从 `109459ns` 降到 `98639ns`。
55. `RealtimeDataReader` 的空 source 行为已收口：构造期要求至少一个 realtime source，空配置直接抛 `std::invalid_argument`；`Poll()` 语义不依赖空 reader 分支，本轮 A/B benchmark 显示当前编译结果下强删该冗余分支更慢，因此实现暂保留该保护分支。空 loop / idle 次数统计仍归 runtime / scheduler diagnostics，不放入 reader。
56. `RealtimeDataReader` 多 source round-robin 已按 benchmark 改为构造期双表扫描：`Source*` 表和 source index 表各重复一轮，热路径按 `[next_source_index_, next_source_index_ + source_count)` 线性扫描，不在循环内做 wrap 分支。2026-05-19 A/B：2 source empty poll 从 `2.53ns` 到 `2.32ns`，4 source 从 `4.19ns` 到 `4.02ns`；index-only 表更慢（2 source `3.64ns`、4 source `4.90ns`），未采用。
57. `core/utils/mapped_file.h` 已新增 read-only mmap RAII helper；`HistoricalDataReader` 文件读取已从 `std::ifstream::read` 切换为 `MappedFile + cursor + memcpy`，当前非空 replay 文件在构造期完成 mmap，`Poll()` / `Drain()` 热路径不打开文件、不抛异常。2026-05-19 release benchmark：historical drain 1/64/4096 从 `25.0ns` / `1550ns` / `98639ns` 降到 `6.37ns` / `369ns` / `23821ns`。ORDI_USDT 三天 LeadLag replay 保持 `book_tickers=94799061`、`signals=2350`、`open=1175`、`close=1173`、`stoploss=2`，耗时 `7.73s`，`max_rss_kb=3486796`；mmap 会提升 reader 吞吐，但完整顺序读大文件时 peak RSS 会反映 fault-in 的映射页。
58. 交易全链路 runtime 命名已从 `StrategyRuntime` 收敛为 `TradingRuntime`，`UserStrategyT` / `user_strategy_` 收敛为 `StrategyT` / `strategy_`；公共 contract 主入口已迁到 `core/trading/order_types.h`、`core/trading/order_manager.h`、`core/trading/strategy_context.h` 和 `core/trading/trading_runtime.h`，namespace 为 `aquila::core`；Gate adapter 主入口已迁到 `exchange/gate/trading/order_session_runtime_adapter.h`，namespace 为 `aquila::gate`。
...
62. `BookTickerShmReader` overrun 边界恢复为完整 capacity 窗口：`unread_count == capacity` 时仍读取当前 ring 中最老的一条已发布 `BookTicker`，只有 `unread_count > capacity` 才记录 overrun 并拉回 `current - capacity`；第一版不主动丢边界 BBO，若未来需要严格检测边界覆盖，应升级 per-slot sequence。这次改动是语义恢复，不是性能优化；2026-05-19 release benchmark：`BM_BookTickerShmReaderTryReadOne` 从保守窗口记录的 `1.94ns` 到恢复后的 `1.95ns`，基本持平；`BM_RealtimeDataReaderEmptyPoll` 1/2/4 source 为 `1.21ns` / `2.29ns` / `4.04ns`。
...
70. 2026-05-20 交易 runtime / Gate adapter 收尾验证已完成：`ctest --test-dir build/debug --output-on-failure` 结果为 68/68 passed；`data_reader_probe` 在 Gate / Binance data session 实时写 SHM 时读到 `handler_book_tickers=14`、两个 source `skipped=0` / `overruns=0`；`gate_demo_strategy` 使用临时 `rounds=3` / `wait_seconds=1` 配置完成 BTC_USDT live smoke，`gate_order_feedback_session` 发布 6 个 `kFilled` event，REST 复核 open orders 为空、`position size=0`、`pending_orders=0`。
...
以 onboarding 的“最近已完成”“代码入口”“当前重要结论”和“下一步建议”为事实源；本轮 push 后预期 `main` 与 `origin/main` 同步，工作区仍可能只有未跟踪的 `signal.csv`，默认不要处理。当前公共 order / runtime contract 已迁到 `core/trading/*` + `aquila::core`，旧 `core/strategy/*` 和 `strategy/order_types.h` / `strategy/order_manager.h` 兼容头已删除；Gate runtime adapter 已迁到 `exchange/gate/trading/order_session_runtime_adapter.h` + `aquila::gate::OrderSessionRuntimeAdapter`，不再放在 `tools/`。当前 `main` 已完成 Task1 order feedback SHM transport、Task2 Gate private `futures.orders` parser、`OrderFeedbackSession`、`OrderManager::OnOrderFeedback()`、trading runtime production loop、Gate adapter 和 `demo` 策略 3 轮 live smoke；本轮 `ctest --test-dir build/debug --output-on-failure` 为 68/68 passed。交易组件边界讨论入口是 `doc/strategy_order_component_model.md` 和 `doc/trading_component_architecture_discussion.md`；当前已确认 `DataReader` 是 concept / 接口约束，不做 merge，`RealtimeDataReader` / `HistoricalDataReader` 用 `Poll(handler)` 单事件接口和 `Drain(handler, max_events)` 批量接口保持一致，`RealtimeDataReader` 构造期要求至少一个实时 source，且多 source round-robin 使用构造期双表扫描，`HistoricalDataReader` 在构造期 mmap 非空 binary 文件并额外用 `finished()` 表达 EOF，`Poll()` / `Drain()` 都是 `noexcept` 热路径，`poll_calls` / `empty_polls` 归 runtime / scheduler diagnostics。2026-05-20 `gate_demo_strategy` 用临时 3 轮配置完成 BTC_USDT live smoke，feedback 发布 6 个 filled event，REST 复核 open orders 为空、`position size=0`、`pending_orders=0`。LeadLag fixed 策略迁移当前完成的是 1-7 部分设计拆解、C++ 策略层模块、`Strategy::OnBookTicker()` replay 信号主链路和 fixed Go / C++ 静态语义审计；`strategy/lead_lag/README.md` 是策略目录入口，`doc/lead_lag_go_cpp_semantic_audit.md` 是 Go/C++ 差异讨论入口。Tardis/HDF ORDI_USDT replay 对比、signal/PnL 结论见 `doc/lead_lag_ordi_tardis_hdf_signal_pnl_comparison.md`。后续如果继续 Gate 交易架构，再读 `doc/agent-handoff-gate-trade-architecture.md`、`doc/superpowers/specs/2026-05-07-gate-order-session-design.md`、`doc/superpowers/specs/2026-05-08-gate-order-feedback-event-design.md`、`doc/superpowers/specs/2026-05-08-order-feedback-shm-transport-design.md` 和 `doc/superpowers/specs/2026-05-08-gate-order-feedback-session-strategy-design.md`，优先补 REST reconcile、feedback WS 断线未知订单恢复、account / position feedback、unfilled-cancel / failure live smoke 和端到端 benchmark；如果继续 Binance 行情，再读 `doc/agent-handoff-binance-market-data.md`；如果继续 data session / config，再读 `doc/data_session_config.md`；如果继续 strategy data reader / binary replay，再读 `doc/data_reader_config.md` 和 `doc/trading_component_architecture_discussion.md`，优先讨论 `DataReaderConfig::max_events_per_source` 是否改名为 `max_events_per_drain` 与 runtime loop diagnostics；如果继续 LeadLag fixed 策略迁移，下一步先确认时间口径、是否需要 replay-only exact empirical quantile、以及 synthetic replay 的边界，再补生产 `OnOrderResponse()` / `OnOrderFeedback()` execution state 和 REST reconcile。修改后按项目规则跑对应验证并自动提交；如果用户输入“结束对话”，先整理相关文档和 onboarding，写好下一轮交接提示，验证后提交，除非用户明确要求不要提交或要求 push。
...
| `doc/trading_component_architecture_discussion.md` | 继续逐个讨论 DataReader / OrderSession / OrderFeedbackSession / OrderManager / Strategy 架构时读 | 当前进行中的组件架构讨论；已记录 DataReader concept、RealtimeDataReader / HistoricalDataReader 接口一致性、no-merge 约束、binary-only replay 第一版，以及 reader stats 与 runtime poll diagnostics 的边界。 |
...
| `doc/superpowers/specs/2026-05-08-gate-order-feedback-event-design.md` | 继续 Gate `OrderFeedbackSession` 架构和实现时读 | 第一版只使用 `futures.orders` 的订单生命周期 event、quantity/price/role/finish reason 语义、Strategy 状态推进和宽结构 event carrier。 |
| `doc/superpowers/specs/2026-05-08-order-feedback-shm-transport-design.md` | 审查或维护 Task1 订单 feedback SHM transport 时读 | 固定 8 lane、Nova SPSC、宽结构 event ABI、`kContinuityLost` control event、显式 reader ownership、无 heartbeat / stale owner 自动判断和 drain-only reader 语义。 |
| `doc/superpowers/specs/2026-05-08-gate-order-feedback-session-strategy-design.md` | 实现 Task2 Gate orders feedback parser / session / Strategy apply 前读 | 多进程共享 feedback session、Gate `futures.orders` 到 event 的映射、Strategy 状态机、OrderSession cache 更新和 continuity lost handling。 |
...
| `core/utils/mapped_file.h` | read-only mmap RAII helper；空文件返回 `data()==nullptr` / `size()==0`，非空文件构造期 `mmap`，析构自动 `munmap`，当前供 `HistoricalDataReader` replay 文件读取使用。 |
...
| `core/base/histogram_quantile.h` | `HistogramQuantile<T>` fixed-bin 近似 quantile，记录 underflow / overflow，支持 lower / midpoint / upper edge 返回模式，使用 touched-bin 查询 / reset；作为 LeadLag 设计中的低延迟 move quantile 默认方向。 |
...
| `core/config/strategy_config.h` | Trading runtime `[strategy]` TOML parser / loader，解析 strategy name/id、dry-run/live mode、order capacity、strategy config path、data reader / order session config path 和 feedback SHM reader 参数。 |
| `strategy/lead_lag/config.h` / `strategy/lead_lag/config.cpp` | LeadLag strategy config parser / loader，解析 `[lead_lag]`、pair、trigger、execute、bbo_record、capacity、quantile range / precision，并用 instrument catalog 压缩 lag 下单 metadata。 |
| `strategy/lead_lag/types.h` / `strategy/lead_lag/raw_market_state.h` | LeadLag raw market state：`QuoteSnapshot`、pair role、lead/lag side state、`symbol_id` slot lookup、same-price quote 语义和 Active seed 选择。 |
| `strategy/lead_lag/window_stats.h` / `strategy/lead_lag/recorders.h` | LeadLag recorder wrappers：时间窗口 mean/std、BBO rolling extrema、lead/lag noise、lag spread mean 和 move quantile window。 |
| `strategy/lead_lag/alignment.h` | LeadLag drift / alignment phase：paired drift rolling mean/std、Bootstrap / Aligning / Active、Active seed drift scaling 和 resume lead tick。 |
| `strategy/lead_lag/threshold.h` | LeadLag threshold state：固定初始 threshold、move quantile roll 后更新 up/down entry/exit、freeze lead/lag noise 和 profit buffer。 |
| `strategy/lead_lag/cost_model.h` / `strategy/lead_lag/signal.h` / `strategy/lead_lag/execution_state.h` | LeadLag signal / execution / feedback state：open/close/stoploss gate、order intent、execution group、terminal feedback position apply 和 degraded / reconcile 状态。 |
...
| 文件 | 职责 |
| --- | --- |
| `core/market_data/realtime_data_reader.h` | Strategy 侧 `RealtimeDataReader`，从 Gate / Binance SHM book ticker source poll 行情；支持 `latest` 和 `drain` 两种 read mode，diagnostics 由编译期 policy 决定。 |
...
| 文件 | 职责 |
| --- | --- |
| `core/trading/order_types.h` | `aquila::core` 公共订单创建请求、`StrategyOrder`、place/cancel/result event 类型；订单对象保存 symbol、price_text、数量、TIF 和状态，不保存 Gate wire cache。 |
| `core/trading/order_manager.h` | 模板化 `OrderManager<OrderSessionT>`，提供 `PlaceLimitOrder()`、`CancelOrder()`、order response apply 和 feedback apply；发单时直接把订单 struct 交给 session。 |
| `core/trading/strategy_context.h` | strategy 的窄下单接口，封装 `OrderManager` 的 `PlaceLimitOrder()`、`CancelOrder()` 和 `FindOrder()`，不暴露 runtime 内部对象。 |
| `core/trading/trading_runtime.h` | `TradingRuntime<StrategyT, OrderSessionT, DataReaderT>` production runtime；从已解析 config 构造 data reader / order session / order manager / context / strategy / feedback reader，`Run()` 负责 ready gating、order response、feedback 和行情 poll，并在订单事件 hook 前先更新 `OrderManager`。 |
| `test/core/trading/trading_runtime_test.cpp` | runtime 回归测试，覆盖 non-copy/move、行情分发、order response 分发、order feedback 分发、feedback disabled、session 构造失败、正式 `Run()`、ready gating、`ShouldStop`、feedback reader poll 和 session stopped failure。 |
...
| `exchange/gate/trading/order_session_runtime_adapter.h` | Gate-specific runtime adapter；把 Gate `OrderSession` 包装为 `TradingRuntime` 的 `OrderSessionT`，同线程 `BindRuntime()` 回调 `OnOrderResponse()`，`SetRuntimeHook()` 让 runtime 在 Gate WebSocket active loop 内轮询 feedback / data，不使用 production response queue、background order session thread 或 command queue。 |
| `tools/gate/demo_strategy.h` | `demo` strategy：从 Gate BTC_USDT 行情 ask price 下 1 手 buy limit，等待 `wait_seconds`，成交则 `price=0` / IOC / reduce-only sell 平仓，未成交则撤单，循环 `rounds` 次；一轮 terminal 前不再开下一单。 |
...
| `config/strategies/demo_strategy.toml` / `config/strategies/demo.toml` | `demo` runtime 配置和 strategy 参数配置。 |
| `config/strategies/lead_lag_btc_strategy.toml` / `config/strategy/lead_lag.toml` | `lead_lag` runtime 配置和第一版 strategy 参数配置；`trigger.quantile` 显式配置 up/down range 和 precision。 |
...
| `test/strategy/lead_lag_config_test.cpp` | LeadLag config parser 回归测试，覆盖 checked-in TOML、catalog metadata、entry spread fallback、重复 `symbol_id`、catalog mismatch 和 window ordering。 |
| `test/strategy/lead_lag_raw_market_state_test.cpp` | LeadLag raw market state 回归测试，覆盖 `symbol_id` / exchange 路由、same-price 不替换 quote、previous quote 和 Active seed / resume flag。 |
| `test/strategy/lead_lag_recorders_test.cpp` | LeadLag recorder wrapper 回归测试，覆盖 rolling mean/std、BBO extrema、noise、spread、move quantile roll 和 seed snapshot。 |
| `test/strategy/lead_lag_alignment_test.cpp` | LeadLag alignment 回归测试，覆盖 paired drift、warmup、phase transition、drifted seed 和 resume lead。 |
| `test/strategy/lead_lag_threshold_test.cpp` | LeadLag threshold 回归测试，覆盖 initial seed、roll 边界、当前 tick 进入新窗口和 profit buffer fallback。 |
| `test/strategy/lead_lag_signal_test.cpp` | LeadLag signal 回归测试，覆盖 cost model、open long / short gate、lead close before open 和 lag stoploss before signal close。 |
| `test/strategy/lead_lag_feedback_state_test.cpp` | LeadLag feedback state 回归测试，覆盖 `OrderManager::RetireFinishedOrder()`、open/close terminal apply、rejected close 和 feedback continuity lost 暂停新开仓。 |
...
| `core/trading/order_feedback_event.h` | Task1 已实现：订单 feedback 宽结构 event ABI，承载 accepted / partial filled / filled / cancelled / rejected，并加入 `kContinuityLost` control event、continuity scope / reason / sequence。 |
| `core/trading/order_feedback_shm.h` | Task1 已实现：固定 8 lane 的订单 feedback SHM channel、`Result` factory manager、publisher、8 lane global continuity lost fanout、pending lane/global continuity lost、reader claim / release 和 drain-only `Poll()`。 |
...
| `test/core/trading/order_feedback_event_test.cpp` | Task1 已实现：`OrderFeedbackEvent` ABI、`kContinuityLost` 和 route id 编码测试。 |
| `test/core/trading/order_feedback_shm_test.cpp` | Task1 已实现：SHM layout / manager、publisher route、queue full pending continuity lost、global continuity lost fanout、reader claim / release / poll 测试。 |
...
| `benchmark/core/trading/order_feedback_shm_benchmark.cpp` | Task1 已实现：`PublishThenDrain`、`PollOneWithRefill`、`PublishPollLoop`、`PublishGlobalContinuityLostThenDrain` transport benchmark；单操作 case 包含 drain / refill 维护，不写成纯 publish / poll latency 或端到端性能结论。 |
...
| `exchange/gate/trading/order_feedback_session.h` | Task2 已实现：Gate `OrderFeedbackSession`，负责 login / subscribe / JSON control parse / SBE binary parse / SHM publish；断线使用 `PublishGlobalContinuityLost(...)` 投递 global `kContinuityLost`，不访问 Strategy order。 |
...
| `test/exchange/gate/trading/order_feedback_session_test.cpp` | Task2 session login / subscribe / binary publish / disconnect continuity lost / publish failure 测试。 |
...
- 断线时清空 correlation，不构造假的 rejected/cancelled response，不产生 `OrderFeedbackKind::kContinuityLost`，也不直接改变订单状态；`OrderManager` / 策略后续通过 feedback 或 REST reconcile 处理未知状态。
...
- 第一版选择宽结构 `OrderFeedbackEvent` 作为 SHM ABI；它保持 trivial / standard-layout，承载 `exchange_update_ns`、本地诊断用 `local_receive_ns`，并用 `OrderFeedbackKind::kContinuityLost` 表达 transport control continuity loss。
- continuity lost 不再通过 shared epoch atomic 进入 `OrderManager`；reader 只通过 `Poll()` 按 FIFO 消费 event，`kContinuityLost` 和普通 order feedback 一样交给 handler。
- `PublishGlobalContinuityLost()` 对 8 lane fanout `kContinuityLost`；某条 lane queue full 时 publisher 保留 pending continuity lost，后续本地重试。
- 普通 publish 遇到当前 lane queue full 时不阻塞、不覆盖、不影响其他 lane；只更新当前 lane `queue_full_count` / `dropped_count` 异常计数，并产生 pending lane `kContinuityLost` event。
...
当前 `TradingRuntime<StrategyT, OrderSessionT, DataReaderT>` 已经是可组合的 production runtime，并已完成一次 `demo` live smoke：
...
- Gate-specific 构造留在 `exchange/gate/trading/order_session_runtime_adapter.h`，core runtime 不 include `exchange/`；Gate adapter 不创建后台线程、不维护 command queue，place/cancel 在 StrategyThread / Gate session 同线程直接调用。完整结构图见 `doc/strategy_order_component_model.md` 的“TradingRuntime 组装链路”：`DataReader -> TradingRuntime -> Strategy`，下单走 `StrategyContext -> OrderManager -> OrderSessionRuntimeAdapter -> Gate OrderSession`，feedback 走 `OrderFeedbackSession -> SHM -> TradingRuntime -> OrderManager -> Strategy`。
- `gate_demo_strategy` 默认 dry-run，不打开 WebSocket / SHM；真实提交必须显式 `--execute`，并需要先启动 `gate_order_feedback_session --connect`。
- 2026-05-20 本机 live smoke：先启动 Gate / Binance data session 写 SHM，再启动 `gate_order_feedback_session --connect`，最后用临时 `/tmp` 配置把 `demo.rounds` 设为 3、`wait_seconds` 设为 1 运行 `gate_demo_strategy --execute`。`gate_order_feedback_session` 收到并发布 6 个 `kFilled` event，`parse_errors=0`、`publish_failures=0`；停止 session 时发布 1 个全局 continuity-lost control event。REST 复核 `orders --status open` 为空，BTC_USDT position `size=0`、`pending_orders=0`。
...
- `RealtimeDataReader` 和 `HistoricalDataReader` 的 `Poll()` / `Drain()` 都是 `noexcept` 热路径；配置校验、SHM attach、
  binary 文件检查和 mmap 失败保留在构造 / 启动冷路径通过异常报告。
...
LeadLag fixed 策略迁移当前处于 1-7 策略层模块已落地、`Strategy::OnBookTicker()` replay 信号主链路已串接阶段。`strategy/lead_lag/config.h` / `config.cpp`、`raw_market_state.h`、`window_stats.h`、`recorders.h`、`alignment.h`、`threshold.h`、`cost_model.h`、`signal.h`、`execution_state.h` 已实现，`strategy.h` 已串起 raw market state、alignment、recorder、threshold、signal engine 和 synthetic position accounting；`OnOrderResponse()` 仍为空，`OnOrderFeedback()` 当前只处理 feedback continuity lost，生产订单回报闭环仍待补齐。事实源：
...
- 3-4 move quantile：fixed Go 是 append-only `Up` / `Down` slice，roll 时 sort 后用 `gonum/stat.Quantile(..., stat.Empirical)`；`DoubleHeap<T>` 可作为单 quantile exact 对照，能避免 roll tick `O(n log n)` spike，但同一批样本若要算多组 quantile，需要维护多组 heap，更新和内存成本随 quantile 数线性增加。当前 LeadLag threshold engine 的低延迟设计方向是 `HistogramQuantile<T>`：第 5 部分只需要两个 histogram 各算一个 quantile，up 查询 `quantile.move` 并返回 upper edge，down 查询 `1 - quantile.move` 并返回 lower edge；固定 range / bins 时热路径不分配，误差由 `bin_width` 和 underflow / overflow 统计约束；实现已支持 touched-bin 查询 / reset、AVX2 / AVX512 显式查询接口，benchmark 显示当前机器上 AVX2 更稳，exact replay 对账仍保留 exact 对照口径。
- `lead_noise` / `lag_noise` 不使用单调队列；fixed Go 使用 4 个 `StreamRecorder` / 8 个本地 FIFO queue。Aquila 设计为 4 个 `RingQueue<TimedValue>`：lead/lag 各一个 mid window 和 ratio window；`RingQueue<T>` 使用 vector、capacity 必须为 2 的次幂、索引用 `& mask`，扩容后当前 `RecorderStats.ring_queue_capacity_grow_count` 只统计 recorder 内部 RingQueue 扩容总次数，不区分 lead noise / lag noise / lag spread，也不记录具体 queue / capacity 或写 log。
...
- `ExecutionGroup` 对齐 fixed Go 的 `ExecuteCache`，表示“和订单生命周期绑定的一份策略 position”：`open order -> held position -> close order`。Aquila 不保存完整 order object，只保存 pending `local_order_id` 并从 `OrderManager` 读取通用订单状态；`ExecutionState` 当前不维护 `local_order_id -> group index` map，而是在很小的 `execute.parallel` 硬上限内线性扫描 `groups_` 查找 pending order。
...
./build/release/benchmark/core/trading/order_feedback_shm_benchmark --benchmark_filter='PublishThenDrain|PollOneWithRefill|PublishPollLoop|PublishGlobalContinuityLostThenDrain' --benchmark_min_time=0.01s
```
...
LeadLag replay / HDF 转换验证：

```bash
/home/liuxiang/dev/pyenv/lx/bin/python scripts/hdf_book_ticker_to_binary_test.py
...
./build/debug/tools/lead_lag_replay --config config/strategies/lead_lag_ordi_replay.toml --data-reader-config /tmp/lead_lag_compare/lead_lag_ordi_hdf_binary_replay.toml --signals-output /tmp/lead_lag_compare/hdf_signal.csv
...
PnL 滑点 `0..5` 需要分别运行 `scripts/lead_lag_replay_pnl.py`，每次传一个 `--slippage-ticks` 值。
...
LeadLag strategy module tests：

```bash
cmake --build build/debug --target lead_lag_config_test lead_lag_raw_market_state_test lead_lag_recorders_test lead_lag_alignment_test lead_lag_threshold_test lead_lag_signal_test lead_lag_feedback_state_test lead_lag_strategy_interface_test -j8
ctest --test-dir build/debug -R lead_lag --output-on-failure
```
...
2026-05-20 本机 C++ trading runtime `demo` live smoke 摘要：Gate / Binance data session 实时写 SHM，
`data_reader_probe` 读到 `handler_book_tickers=14`，两个 source 的 `skipped=0` / `overruns=0`；
`gate_demo_strategy --execute` 用临时 3 轮配置完成 BTC_USDT 下单 / 平仓闭环，`gate_order_feedback_session`
发布 6 个 `kFilled` event，REST 复核 `open orders=[]`、`position size=0`、`pending_orders=0`。
...
2. 读取 `doc/trading_component_architecture_discussion.md`，当前正在逐项细化这些组件；`DataReader` 部分已经确认 no-merge、binary-only replay 第一版、`Poll()` / `Drain()` concept 接口、`RealtimeDataReader` / `HistoricalDataReader` 关系和 stats / diagnostics 边界。
3. 若继续 `DataReader`，现有实现类已经命名为 `RealtimeDataReader` / `HistoricalDataReader`；下一步优先讨论 `DataReaderConfig::max_events_per_source` 是否改名为 `max_events_per_drain`，以及 runtime / scheduler 层是否需要新增 loop diagnostics。
...
7. 当前 `aquila` C++ replay 信号主链路已落地，测试入口是 `ctest --test-dir build/debug -R lead_lag --output-on-failure`。时间口径 / exact quantile / synthetic replay 边界确认后，再补生产 `OnOrderResponse()` / `OnOrderFeedback()` execution state、REST reconcile / feedback continuity lost 后恢复；做性能结论时重新跑 `lead_lag_strategy_benchmark` 或 replay profiling。

如果新对话从 Gate 交易继续，建议顺序：

1. 读取 `doc/agent-handoff-gate-trade-architecture.md`。
2. 读取 `doc/superpowers/specs/2026-05-07-gate-order-session-design.md`。
3. 读取 `doc/superpowers/specs/2026-05-08-gate-order-feedback-event-design.md`、`doc/superpowers/specs/2026-05-08-order-feedback-shm-transport-design.md` 和 `doc/superpowers/specs/2026-05-08-gate-order-feedback-session-strategy-design.md`。
4. 直接从“代码入口”和 `doc/agent-handoff-gate-trade-architecture.md` 追溯已实现边界、验证命令和 benchmark 口径；已完成的执行计划文档不再保留。
5. 如果继续 trading runtime，从 `core/trading/trading_runtime.h`、`exchange/gate/trading/order_session_runtime_adapter.h`、`tools/gate/demo_strategy.h` 和 `tools/gate/demo_strategy.cpp` 接手；当前 production `Run()` hook mode / Gate adapter / demo 工具和 3 轮 filled-close live smoke 已实现。
6. 继续扩展 live smoke：在用户明确允许后覆盖 unfilled-cancel、rejected / cancel-rejected、feedback session 断线和 REST reconcile 分支，保留原始输出，并用 REST 查询确认无残留订单 / 仓位。
...
9. 增加端到端 benchmark：覆盖 `TradingRuntime -> OrderSessionRuntimeAdapter -> OrderSession` 下单请求和 `OrderFeedbackSession -> SHM -> TradingRuntime` 回报消费；真实链路性能结论必须另跑 live probe 或 profile。
