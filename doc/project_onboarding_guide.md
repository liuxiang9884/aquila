...
如果继续 LeadLag fixed 策略迁移，优先读 `doc/leadlag-fixed-strategy-reconstruction-guide.md` 和 `doc/superpowers/specs/2026-05-08-leadlag-fixed-strategy-aquila-design.md`；fixed Go 源码参考在 `third_party/strategy/wt-invariant-strategy-leadlag-must-fix/`。LeadLag 第 1-7 部分当前都是策略层设计，尚未实现；仓库中已有的是 `config/strategy/lead_lag.toml` 样例配置，以及第 3 部分可复用的 `core/base` 通用数据结构。
...
以 onboarding 的“最近已完成”“代码入口”“当前重要结论”和“下一步建议”为事实源；当前 `main` 至少包含 `81e1fd2 Add base data structures for lead lag recorders`，默认不 push，除非用户明确要求 push；下一轮先用 `git status --short --branch` 和 `git log --oneline -8` 重新确认分支状态。当前 `main` 已完成 Task1 order feedback SHM transport、Task2 Gate private `futures.orders` parser、`OrderFeedbackSession`、`OrderManager::OnOrderFeedback()`、strategy runtime production loop、Gate runtime adapter 和 `demo` 策略 dry-run 工具。LeadLag fixed 策略迁移当前完成的是 1-7 部分设计拆解、`config/strategy/lead_lag.toml` 样例配置和 `core/base` 通用底层结构；LeadLag strategy config parser、raw market state、recorder 组合层、drift / alignment、threshold、signal、execution feedback state 和整体 `leadlag::Strategy` 均尚未实现。后续如果继续 Gate 交易架构，再读 `doc/agent-handoff-gate-trade-architecture.md`、`docs/superpowers/specs/2026-05-07-gate-order-session-design.md`、`docs/superpowers/specs/2026-05-08-gate-order-feedback-event-design.md`、`docs/superpowers/specs/2026-05-08-order-feedback-shm-transport-design.md` 和 `docs/superpowers/specs/2026-05-08-gate-order-feedback-session-strategy-design.md`；如果继续 Binance 行情，再读 `doc/agent-handoff-binance-market-data.md`；如果继续 data session / config，再读 `doc/data_session_config.md`；如果继续 strategy data reader，再读 `doc/data_reader_config.md`；如果继续 LeadLag fixed 策略迁移，下一步优先开始执行第 1-3 部分策略层 parser / raw market state / recorder wrapper，或按实现计划从第 4 部分 `strategy/lead_lag/alignment.h`、`window_stats.h` 和 `test/strategy/lead_lag_alignment_test.cpp` 接入第 5-7 部分。修改后按项目规则跑对应验证并自动提交；如果用户输入“结束对话”，
...
LeadLag fixed 策略迁移当前处于设计拆解阶段；仅 `core/base` 通用底层数据结构和样例配置已存在，策略层 parser / raw state / recorder / signal / order 组合均尚未实现。事实源：
...
4. 第 1-7 部分 `aquila` C++ 落地设计已写入设计文档，但策略层代码尚未执行；当前仅第 3 部分可复用的通用底层数据结构已落地到 `core/base/`。
5. 在实现计划前，先决定 BBO extrema 是否继续用 `BookTicker.local_ns`，还是为了 fixed replay 对账引入 server timestamp 输入。
6. 进入实现时，先补第 1-3 部分策略层 parser / raw market state / recorder wrapper；随后执行第 4 部分 `AlignmentState` / `MeanWindow` / `MeanStdWindow` 单元测试与实现，再实现第 5 部分 `MoveQuantileWindow` / threshold state，随后实现第 6-7 部分 `EntryCostBreakdown` / `ExecutionGroup` / signal engine / feedback state，并补 `OrderManager::RetireFinishedOrder()`；最后补针对 fixed Go 语义的单元测试和 replay 对账测试。
