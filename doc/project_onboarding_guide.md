...
| `strategy/lead_lag/config.h` / `strategy/lead_lag/config.cpp` | LeadLag user strategy config parser / loader，解析 `[lead_lag]`、pair、trigger、execute、bbo_record、capacity、quantile range / precision，并用 instrument catalog 压缩 lag 下单 metadata。 |
...
| `config/strategies/lead_lag_btc_strategy.toml` / `config/strategy/lead_lag.toml` | `lead_lag` runtime 配置和第一版 user strategy 参数配置；`trigger.quantile` 显式配置 up/down range 和 precision。 |
