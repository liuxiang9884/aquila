...
如果继续 LeadLag fixed 策略迁移，优先读 `doc/leadlag-fixed-strategy-reconstruction-guide.md` 和 `doc/superpowers/specs/2026-05-08-leadlag-fixed-strategy-aquila-design.md`；fixed Go 源码参考在 `third_party/strategy/wt-invariant-strategy-leadlag-must-fix/`，第一部分配置已落到 `config/strategy/lead_lag.toml`，下一步可以继续第 2 部分 raw market state 的 Aquila / fixed Go 对照。
