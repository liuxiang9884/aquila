...
- [x] **Step 1: 写 reconcile 设计**

Completed: `doc/lead_lag_reconcile_design.md` 已定义 continuity lost、WS reconnect、进程重启后的本地订单、open orders、finished orders、position、account 和 execution group 恢复规则；并明确 identity / matching、account / position guardrails、failure policy、后续任务、验证命令和 commit 边界。
