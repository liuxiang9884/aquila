...
- 在 recorder / probe 启动时打印运行模式、连接的 SHM、source 列表和输出文件路径；
- 在 recorder / probe 结束、收到停止信号或连接中断退出时，若 reader 打开 Diagnostics，则打印统计量；
...
3. 启动 `data_reader_recorder`。
   ...
   - 启动日志必须包含运行模式、`max_events_per_drain`、每个 source 的 `name` / `exchange` /
     `read_mode` / `start_position` / `shm_name` / `channel_name`，以及输出文件路径。
...
- recorder / probe 启动日志应明确当前模式、连接的 SHM source、输出文件路径和 drain budget；
- recorder / probe 退出日志应在 Diagnostics 已启用时打印 total count、per-source count、`skipped`、
  `overruns` 和最后看到的 `BookTicker.id`；
- 不把临时 live 输出写入仓库；
- 不引入新依赖；
...
- 启动日志能直接看出当前模式、连接的 SHM、source 和输出文件路径；
- 输出 binary 非空且 record size 对齐；
- binary 可由 `HistoricalDataReader` / replay 路径顺序读取；
- recorder source stats 中明确记录 `skipped` 和 `overruns`；如果相关 reader 未启用 Diagnostics，
  日志必须明确说明统计不可用；
