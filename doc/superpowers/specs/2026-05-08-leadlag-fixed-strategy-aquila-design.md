...
具体到现有 `BookTicker`，live 和 replay 默认都用 `ticker.local_ns` 作为策略时间 `now`。`exchange_ns` 暂时只作为诊断字段，不驱动 alignment warmup、recorder window、move queue roll 或 noise 统计。
...
UpdateMarketSide(side, ticker):
    ...
    price_changed =
        ticker.bid_price != side.prev_price.bid_price ||
        ticker.ask_price != side.prev_price.ask_price
    side.latest_quote = {ticker.local_ns, ticker.bid_price, ticker.ask_price}
    return {price_changed}
```

`price_changed` 只比较 bid / ask price，不比较 volume。重复价格 tick 仍然先更新 raw state、drift 和 alignment；Active 后如果 `price_changed=false` 且不是 Active 切换恢复 tick，则不推进 recorder / threshold / signal。
...
1. 配置与 instrument catalog metadata 是否完整。
