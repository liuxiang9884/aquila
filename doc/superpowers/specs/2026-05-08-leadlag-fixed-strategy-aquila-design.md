...
- `last_event_ns` 使用当前 `BookTickerEventTimeNs(ticker)`，即优先 `BookTicker.exchange_ns`，缺失时 fallback 到 `BookTicker.local_ns`。
...
时间口径当前已定为使用 `BookTickerEventTimeNs(ticker)` 推进窗口和 warmup；binary replay 数据有 `exchange_ns` 时使用交易所时间，缺失时 fallback 到本地收到时间。

Go 源码中 `Context.Now()` 驱动 drift / alignment warmup、`MoveQueue` roll、noise、spread 和 order 日志时间；`BBOVolRecorder.OnBBO()` 例外，它用 `bbo.ServerTime` 做 extrema 窗口淘汰。Aquila 当前用 `BookTickerEventTimeNs(ticker)` 作为策略事件时间 `now`。若后续要做 byte-for-byte fixed replay 对账，应单独评估该口径是否与 fixed 的 server timestamp 完全兼容。
...
UpdateMarketSide(side, ticker):
    if !side.has_quote:
        side.latest_quote = {BookTickerEventTimeNs(ticker), ticker.bid_price, ticker.ask_price}
        side.has_quote = true
...
- raw state 不保存完整 `BookTicker`，只保存 `QuoteSnapshot{event_ns, bid_price, ask_price}`、`previous_quote` 和对应 valid flag。
- 策略窗口时间使用 `BookTickerEventTimeNs(ticker)`，优先 `exchange_ns`，缺失时 fallback 到 `local_ns`。
...
注意：fixed Go 这里使用 `bbo.ServerTime`，而非 `Context.Now()`。Aquila 当前统一使用 `BookTickerEventTimeNs(ticker)` 驱动策略窗口；严格 fixed replay 对账时需要单独验证该时间口径差异。
...
OnLeadActiveTick(drifted_lead):
    lead_extrema.Update(drifted_lead.event_ns, drifted_bid, drifted_ask)

OnLagActiveTick(raw_lag):
    lag_extrema.Update(raw_lag.event_ns, lag_bid, lag_ask)
```
...
lead_extrema.Update(lead_seed.event_ns, drifted_lead_seed_bid, drifted_lead_seed_ask)
lag_extrema.Update(lag_seed.event_ns, lag_seed_bid, lag_seed_ask)
```
...
```text
OnBookTicker(ticker):
  now = BookTickerEventTimeNs(ticker)
