...
- Strategy reader drain event 并调用 `OrderManager::OnOrderFeedback()`。
- `OrderManager` 根据 event 更新 `StrategyOrder` 状态、成交累计和终态字段。
- accepted event 建立 `local_order_id -> exchange_order_id` 映射所需信息；`OrderManager` 再通知自己线程内的 `OrderSession` 更新 cancel cache。
- terminal event 触发 `OrderManager` 清理或通知 `OrderSession` forget exchange id cache。

## 非目标

- 不订阅 `futures.usertrades`。
- 不订阅 `futures.positions`。
- 不实现 REST reconcile，但需要保留 continuity lost detected 状态。
- 不实现跨 strategy 风控。
...
- 在 disconnect / reconnect / parse 不可恢复场景标记 global continuity lost。
...
第一版 parser 遇到下列情况应计数并丢弃该条 event，必要时标记 continuity lost：
...
对于单条 malformed order update，第一版可以只计数并丢弃；对于 session 级 decode 不可恢复、断线、重连后未知缺口，必须调用 Task1 当前接口 `PublishGlobalContinuityLost(...)`。
...
`OrderManager::OnOrderFeedback()` 基本规则：
...
- `OrderFeedbackKind::kContinuityLost` event 会让 Strategy 进入 continuity lost detected 状态；
