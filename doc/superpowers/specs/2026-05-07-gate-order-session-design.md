...
> 2026-05-09 更新：本设计中的“Strategy 缓存 Gate wire-ready fields”边界已被后续 struct-flow 实现调整。当前事实源以 `doc/project_onboarding_guide.md`、`doc/agent-handoff-gate-trade-architecture.md` 和当前代码为准：Strategy 直接发送订单 struct，Gate `OrderSession` 在发送路径完成 JSON 序列化。
