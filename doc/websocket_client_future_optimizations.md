...
| 方案 | 核心模型 | 优点 | 主要风险 | 当前建议 |
|---|---|---|---|---|
| 单 private WS + fanout | 外部一条 WS，内部 fanout 给策略 | 简单、顺序稳定、资源少 | 只能吃单一路外部延迟 | 生产 baseline |
| 双 private WS fastest-win | 每条 update 在 WSA / WSB 中选最快合法数据 | 理论首包延迟最低，可抗一路抖动 | 去重、sequence、gap、重连恢复复杂 | 暂缓，等证据 |
| warm-up feed selection | 交易前双路 warmup，选 primary，secondary standby | 复杂度低，避免随机选慢路 | warmup 最优不保证交易中一直最优 | 下一阶段优先 |
| 多连接 N-best warmup | N 路 warmup 排名，选 primary，其余 standby | 有机会避开更多慢 flow | 收益递减，连接和风控成本上升 | 先只测 `N=3/4` |
...
4. **CriticalSession read loop 微优化**：只有当 benchmark 证明 session 逻辑占比明显时再做。
5. **FrameCodec 微优化**：保持小步修改，每次只改一个分支，并重跑 codec + session benchmark。

核心判断：后续 read path 优化不应继续只抠 `FrameCodec`，而应先把 TLS、bounded read pump、consumer 解析放进同一套 benchmark，找真实生产热路径的最大成本。

### 路线 B：Live feed 稳定性和选路

1. **网络路径基线**：记录 remote peer、local port、NIC、IRQ/RSS/RPS/XPS、owner CPU 和 `/proc/interrupts` 增量。
2. **warm-up feed selection 工具化**：继续完善 `websocket_latency_compare` 的 `selected=... reason=...` 输出，保留 health、gap、p50、p99、`SO_INCOMING_CPU` 证据。
3. **FeedSelector 设计**：把 warm-up selection 从工具输出升级为可接入生产的 selector，交易中采用 primary publish、secondary monitor。
4. **N=3 / N=4 多连接实验**：只在 2 路 warmup 有稳定收益后，再测多连接收益是否递减。
5. **fastest-win arbiter 评估**：只有 live 证据证明 `min(WSA, WSB)` 明显优于 warm-up primary，才进入每条 update 仲裁设计。
6. **生产部署基线**：记录 CPU affinity、IRQ/RSS、NUMA、socket buffer、OpenSSL、kernel、交易所 endpoint 和连接限额。

核心判断：双 private WS 的毫秒级差异不能只靠 owner thread 绑核解决。下一阶段优先做 warm-up selection，策略侧仍只消费一条 canonical market data stream。
...
核心判断：后续不应只抠单轮 fake session 的 42ns，而应先用 read/write mixed benchmark 证明 atomic、调度顺序、write budget 和 idle pause 策略对 P99 / P99.9 的真实影响。

### 下一步推荐

若继续 WebSocket / 行情方向，优先做 **FeedSelector 设计和 2 路 primary / standby 接入**：
...
- warmup 输出 `selected`、`reason`、health、gap、p50、p99、`SO_INCOMING_CPU`。
- 交易中策略只消费一条 canonical stream。
- secondary 持续 monitor，不直接进入策略。
- 切换必须有 hysteresis，避免 feed flapping。

若继续 WebSocket core 性能方向，优先做 **TLS read path benchmark**，因为当前无 TLS 的 `session_read_path` 已不足以解释真实交易所链路成本。
