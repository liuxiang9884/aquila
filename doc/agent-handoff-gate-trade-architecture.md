...
第一版 `OrderSession` 已落地；当前实现入口是 `exchange/gate/trading/order_session.h`、`exchange/gate/trading/order_request_encoder.h`
和 `exchange/gate/trading/submit_response_parser.h`。
...
`doc/strategy_order_component_model.md` 和当前代码作为事实源。
...
旧设计 spec 已清理；本节只保留当前实现仍遵守的边界。实现细节以 `exchange/gate/trading/*`、`core/trading/*` 和测试为准。
...
旧 feedback event / SHM / strategy apply spec 已清理；本节只保留当前实现仍遵守的边界。实现细节以 `core/trading/order_feedback_*`、
`exchange/gate/trading/order_feedback_*`、`core/trading/order_manager.h` 和测试为准。
