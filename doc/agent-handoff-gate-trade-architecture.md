...
- Strategy runtime production loop、Gate runtime adapter 和 `demo` 策略 dry-run tool 已落地；本轮没有做 `gate_demo_strategy --execute` 实盘测试。
- 新接手时先执行：
...
4. `Run()` 先启动 order session，poll order response 和 feedback reader，再在 `OrderSessionT::Ready()` 为 true 后 poll data reader；如果 `OrderSessionT::Running()` 变 false，runtime 返回失败，避免 private session 启动失败后空转。
...
7. core runtime 不 include `exchange/`；Gate-specific 构造放在 `tools/gate/strategy_runtime_adapter.h` 和 tool 层。adapter 内 Gate 回调线程只把 response 转成 `OrderResponseEvent` 入队，runtime 线程统一 apply；place/cancel 命令通过 WebSocket runtime hook 回到 Gate session 线程执行，避免跨线程直接修改 `OrderSession` request map。
8. `demo` 策略行为：按匹配 `symbol_id` 的行情 ask price 下 1 手 buy limit，等待 `wait_minutes`，buy filled 后发 market sell reduce-only 平仓，否则撤单；循环 `cycles` 次后 `ShouldStop()`。当前只做 dry-run / unit test，未做实盘。
