...
class AnalyzeOrderDetailTest(unittest.TestCase):
    def test_parses_legacy_order_and_calculates_fill_quality(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            write_file(
                log_path,
                """
                I2026-05-25 02:29:35.105545332 1:1 strategy.h:LogStrategySignalTriggered:155] lead_lag_signal_triggered trigger_ticker_id=10624277384126 trigger_exchange=kBinance trigger_symbol_id=4 symbol=PROVE_USDT symbol_id=4 role=kLead action=kOpenLong side=kBuy reduce_only=false group_id=0 raw_price=0.2711
                I2026-05-25 02:29:35.105549026 1:1 strategy.h:LogStrategyOrderIntent:189] lead_lag_order_intent trigger_ticker_id=10624277384126 symbol=PROVE_USDT symbol_id=4 action=kOpenLong side=kBuy reduce_only=false group_id=0 quantity=36 price=0.2714 raw_price=0.2711 order_price=0.2714 slippage_ticks=3 price_tick=0.0001 target_open_notional=100 estimated_notional=97.704 active_groups=0
        ...
        self.assertEqual(row["source_schema"], "legacy_intent_order_sequence")
        self.assertIn("missing_submitted_log", row["warnings"])
    ...
    def test_prefers_submitted_log_for_final_group_and_quantity_text(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            write_file(
                log_path,
                """
                I2026-05-25 02:29:35.105549026 1:1 strategy.h:LogStrategyOrderSubmitted:1] lead_lag_order_submitted local_order_id=288230376151711750 trigger_ticker_id=10624277385364 trigger_exchange=kBinance trigger_symbol_id=4 symbol=PROVE_USDT symbol_id=4 signal_role=kLead order_role=exit action=kCloseLong side=kSell reduce_only=true group_id=1 quantity=36 quantity_text=36 raw_price=0.2713 order_price=0.271 price_text=0.2710 slippage_ticks=3 price_tick=0.0001 target_open_notional=100 estimated_notional=97.56 active_groups=1 place_status=kOk
