#include "monitor/demo/symbol_workbench_demo_data.h"

#include <array>

namespace aquila::monitor {
namespace {

constexpr std::array<SymbolSummary, 11> kSymbolSummaries{{
    {.symbol = "PROVE_USDT",
     .net_position = -8.0,
     .open_order_count = 3,
     .total_pnl = -18.4,
     .health = SymbolHealth::kDrift},
    {.symbol = "RAVE_USDT",
     .net_position = 0.0,
     .open_order_count = 2,
     .total_pnl = 3.1},
    {.symbol = "ZEC_USDT",
     .net_position = 3.0,
     .open_order_count = 4,
     .total_pnl = 38.50 + 4.29 - 0.11,
     .health = SymbolHealth::kSelected},
    {.symbol = "SIREN_USDT",
     .net_position = 0.0,
     .open_order_count = 1,
     .total_pnl = -0.6},
    {.symbol = "ETC_USDT",
     .net_position = 12.0,
     .open_order_count = 0,
     .total_pnl = 9.3},
    {.symbol = "DASH_USDT",
     .net_position = -1.0,
     .open_order_count = 1,
     .total_pnl = -2.0,
     .health = SymbolHealth::kStale},
    {.symbol = "RIVER_USDT"},
    {.symbol = "SUI_USDT",
     .net_position = 80.0,
     .open_order_count = 2,
     .total_pnl = 6.4},
    {.symbol = "INJ_USDT", .total_pnl = 1.1},
    {.symbol = "ENA_USDT",
     .net_position = -240.0,
     .open_order_count = 1,
     .total_pnl = 11.5},
    {.symbol = "BRETT_USDT"},
}};

constexpr AccountBalance kAccountBalance{
    .currency = "USDT",
    .total_equity = 12548.72,
    .available = 9876.54,
    .used_margin = 2672.18,
    .realized_pnl = 38.50,
    .unrealized_pnl = 4.29,
    .total_pnl = 42.68,
};

constexpr std::array<ServerMetric, 5> kServerMetrics{{
    {.metric = "cpu",
     .value = "18%",
     .state = "ok",
     .updated_time = "11:40:12.384"},
    {.metric = "memory",
     .value = "42%",
     .state = "ok",
     .updated_time = "11:40:12.384"},
    {.metric = "disk_root",
     .value = "71%",
     .state = "warn",
     .updated_time = "11:40:12.384"},
    {.metric = "clock_offset",
     .value = "0.8ms",
     .state = "ok",
     .updated_time = "11:40:12.384"},
    {.metric = "network",
     .value = "connected",
     .state = "ok",
     .updated_time = "11:40:12.384"},
}};

constexpr std::array<ProcessHealth, 4> kProcesses{{
    {.name = "gate_data_session",
     .role = "market",
     .pid = 12345,
     .uptime = "02:14:03",
     .cpu_percent = 3.2,
     .memory_percent = 1.1,
     .status = "up",
     .heartbeat = "120ms ago",
     .updated_time = "11:40:12.384"},
    {.name = "binance_data_session",
     .role = "market",
     .pid = 12346,
     .uptime = "02:13:55",
     .cpu_percent = 2.8,
     .memory_percent = 1.0,
     .status = "up",
     .heartbeat = "118ms ago",
     .updated_time = "11:40:12.384"},
    {.name = "lead_lag_strategy",
     .role = "trading",
     .pid = 12360,
     .uptime = "00:42:10",
     .cpu_percent = 6.4,
     .memory_percent = 2.3,
     .status = "up",
     .heartbeat = "80ms ago",
     .updated_time = "11:40:12.384"},
    {.name = "gate_order_feedback_session",
     .role = "feedback",
     .pid = 12380,
     .uptime = "00:42:08",
     .cpu_percent = 1.2,
     .memory_percent = 0.8,
     .status = "stale",
     .heartbeat = "5.2s ago",
     .updated_time = "11:40:12.384"},
}};

constexpr std::array<ConnectionHealth, 3> kConnections{{
    {.venue = "Gate",
     .channel = "futures.orders",
     .state = "connected",
     .latency = "14ms",
     .last_message = "120ms ago",
     .reconnects = 0,
     .updated_time = "11:40:12.384"},
    {.venue = "Gate",
     .channel = "book_ticker",
     .state = "connected",
     .latency = "9ms",
     .last_message = "40ms ago",
     .reconnects = 0,
     .updated_time = "11:40:12.384"},
    {.venue = "Binance",
     .channel = "book_ticker",
     .state = "connected",
     .latency = "11ms",
     .last_message = "38ms ago",
     .reconnects = 0,
     .updated_time = "11:40:12.384"},
}};

constexpr std::array<RuntimeAlert, 2> kRuntimeAlerts{{
    {.severity = "warning",
     .message = "disk_root usage reached 71%",
     .updated_time = "11:40:12.384"},
    {.severity = "error",
     .message = "gate_order_feedback_session heartbeat stale for 5.2s",
     .updated_time = "11:40:12.384"},
}};

constexpr RuntimeHealth kRuntimeHealth{
    .server_state = "ok",
    .cpu_percent = 18.0,
    .memory_percent = 42.0,
    .disk_percent = 71.0,
    .market_processes_up = 2,
    .market_processes_total = 2,
    .trading_processes_up = 3,
    .trading_processes_total = 3,
    .stale_count = 1,
    .restart_count = 0,
    .server_metrics = kServerMetrics,
    .processes = kProcesses,
    .connections = kConnections,
    .alerts = kRuntimeAlerts,
};

constexpr std::array<MarketDataRow, 3> kZecMarketData{{
	    {.exchange = "Gate",
	     .exchange_symbol = "ZEC_USDT",
	     .market_data_id = "gate-zec-9841",
	     .has_last_price = false,
	     .bid_price = 62.85,
	     .bid_volume = 18.4,
	     .ask_price = 62.86,
	     .ask_volume = 9.7,
	     .has_volume = false,
	     .has_turnover = false,
	     .updated_time = "11:40:12.384"},
	    {.exchange = "Binance",
	     .exchange_symbol = "ZECUSDT",
	     .market_data_id = "binance-zec-4427",
	     .has_last_price = false,
	     .bid_price = 62.84,
	     .bid_volume = 31.2,
	     .ask_price = 62.87,
	     .ask_volume = 24.1,
	     .has_volume = false,
	     .has_turnover = false,
	     .updated_time = "11:40:12.377"},
    {.exchange = "OKX",
     .exchange_symbol = "ZEC-USDT",
     .market_data_id = "okx-zec-empty",
     .has_data = false,
     .updated_time = "-"},
}};

constexpr std::array<MonitorOrder, 6> kZecOrders{{
    {.exchange = "Gate",
     .exchange_symbol = "ZEC_USDT",
     .source = OrderSource::kAquila,
     .source_label = "Aquila#2",
     .side = OrderSide::kBuy,
     .side_value = 1,
     .quantity = 3.0,
     .filled_quantity = 3.0,
     .price = 62.10,
     .average_fill_price = 62.08,
     .fee = -0.06,
     .status = "filled",
     .exchange_order_id = 9138472801,
     .local_order_id = 144115188075855874,
     .text = "t-144115188075855874",
     .updated_time = "11:40:02.118",
     .age_seconds = 188},
    {.exchange = "Gate",
     .exchange_symbol = "ZEC_USDT",
     .source = OrderSource::kManual,
     .source_label = "Manual",
     .side = OrderSide::kSell,
     .side_value = -1,
     .quantity = 2.0,
     .left_quantity = 2.0,
     .price = 63.40,
     .status = "open",
     .exchange_order_id = 9138472901,
     .updated_time = "11:40:05.442",
     .age_seconds = 134},
    {.exchange = "Gate",
     .exchange_symbol = "ZEC_USDT",
     .source = OrderSource::kExternal,
     .source_label = "External",
     .side = OrderSide::kBuy,
     .side_value = 1,
     .quantity = 1.0,
     .left_quantity = 0.4,
     .filled_quantity = 0.6,
     .price = 61.80,
     .average_fill_price = 61.78,
     .fee = -0.01,
     .status = "partial",
     .exchange_order_id = 9138473001,
     .text = "hedger-zec-42",
     .updated_time = "11:40:08.907",
     .age_seconds = 97},
    {.exchange = "Gate",
     .exchange_symbol = "ZEC_USDT",
     .source = OrderSource::kAquila,
     .source_label = "Aquila#2",
     .side = OrderSide::kSell,
     .side_value = -1,
     .quantity = 1.0,
     .left_quantity = 1.0,
     .price = 64.00,
     .status = "open",
     .exchange_order_id = 9138473101,
     .local_order_id = 144115188075855875,
     .text = "t-144115188075855875",
     .updated_time = "11:40:10.021",
     .age_seconds = 66},
    {.exchange = "Gate",
     .exchange_symbol = "ZEC_USDT",
     .source = OrderSource::kManual,
     .source_label = "Manual",
     .side = OrderSide::kSell,
     .side_value = -1,
     .quantity = 1.0,
     .filled_quantity = 1.0,
     .price = 62.90,
     .average_fill_price = 62.91,
     .fee = -0.02,
     .status = "filled",
     .exchange_order_id = 9138473201,
     .updated_time = "11:40:11.264",
     .age_seconds = 55},
    {.exchange = "Gate",
     .exchange_symbol = "ZEC_USDT",
     .source = OrderSource::kExternal,
     .source_label = "External",
     .side = OrderSide::kBuy,
     .side_value = 1,
     .quantity = 1.0,
     .filled_quantity = 1.0,
     .price = 60.80,
     .average_fill_price = 60.82,
     .fee = -0.02,
     .status = "filled",
     .exchange_order_id = 9138473301,
     .text = "maker-bot-zec",
     .updated_time = "11:40:12.033",
     .age_seconds = 44},
}};

constexpr SymbolDetail kZecDetail{
    .symbol = "ZEC_USDT",
    .market_data = kZecMarketData,
    .orders = kZecOrders,
    .position =
        {
            .net_position = 3.0,
            .average_entry = 61.43,
            .mark_price = 62.86,
            .notional = 188.58,
            .realized_pnl = 38.50,
            .unrealized_pnl = 4.29,
            .fees = -0.11,
            .total_pnl = 38.50 + 4.29 - 0.11,
            .exposure = "long",
            .ledger_state = "estimated",
            .drift_state = "none",
            .ws_update_age_seconds = 3,
            .rest_snapshot_age_seconds = 4,
        },
    .source_mix =
        {
            .aquila = 2,
            .manual = 2,
            .external = 2,
        },
};

constexpr std::array<std::string_view, 6> kEvents{{
    "11:40:02 ZEC_USDT Aquila buy filled qty=3 avg=62.08 fee=-0.06",
    "11:40:05 ZEC_USDT Manual sell open qty=2 px=63.40",
    "11:40:09 PROVE_USDT drift: REST open order not seen in WS",
    "11:40:12 health warning disk_root usage 71%",
    "11:40:14 feedback stale gate_order_feedback_session heartbeat 5.2s",
    "11:40:16 ZEC_USDT market refresh",
}};

}  // namespace

std::span<const SymbolSummary> DemoSymbolSummaries() noexcept {
  return kSymbolSummaries;
}

std::string_view DemoSelectedSymbol() noexcept {
  return "ZEC_USDT";
}

const SymbolDetail* DemoSelectedSymbolDetail() noexcept {
  return &kZecDetail;
}

AccountMonitorSnapshot DemoAccountMonitorSnapshot() noexcept {
  return AccountMonitorSnapshot{
      .title = "AQUILA GATE USDT ACCOUNT",
      .websocket_state = "WS connected 14ms",
      .rest_state = "REST ok 4s",
      .mode = "read-only demo",
      .selected_symbol = DemoSelectedSymbol(),
      .balance = kAccountBalance,
      .runtime_health = kRuntimeHealth,
      .symbols = DemoSymbolSummaries(),
      .selected_detail = DemoSelectedSymbolDetail(),
      .events = kEvents,
  };
}

}  // namespace aquila::monitor
