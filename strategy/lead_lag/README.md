...
实盘长时间运行和测试计划见 `doc/lead_lag_live_runtime_plan.md`。该计划按 signal-only live runner、长时间观察、生产订单闭环、runner execute 禁用边界、REST reconcile、异常 smoke 和端到端 benchmark 的顺序推进。
...
- 遇到 feedback continuity lost 后会暂停新开仓，等待后续 reconcile 能力；真实订单 smoke 前仍需补 REST reconcile、异常 smoke 和账户 / 仓位复核边界。
