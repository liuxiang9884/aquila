...
| `config/strategies/lead_lag_requested_11symbols_live_strategy_20260522.toml` | 2026-05-22 requested 11-symbol 实盘订单 runtime 配置，必须显式 `--execute` 才会提交真实订单。 |
...
- `[lead_lag.risk]` 提供 strategy 全局持仓硬限制：`max_gross_notional` 统计所有持仓和 pending open reservation 的绝对 notional；`max_holding_position` 可选，配置后统计绝对张数。触达限制后只拒绝新开仓，reduce-only close 继续允许。
