...
实盘运行 runbook 见 `doc/lead_lag_live_runtime_plan.md`；live / replay 信号一致性测试见 `doc/lead_lag_live_replay_testing.md`；REST reconcile / feedback 恢复设计见 `doc/lead_lag_reconcile_design.md`。当前未完成重点是 decimal-size / IOC partial-fill 修复的小额 strategy live smoke 复核、failure response live 探测，以及复核通过后的 guarded 长时间真实订单运行。
...
signal CSV 由 `tools/lead_lag/signal_csv_writer.*` 输出，包含触发信号、`ticker_id`、价格、reduce_only、lead / lag quote、drift、threshold、noise、active group 等诊断字段，主要用于策略研发对账。`lead_lag_replay` 和 live runner 的 signal-only 模式只有在显式传入 `--signals-output` 时才写 per-signal CSV；未配置时只输出 summary。

真实订单模式不接 `SignalCsvWriter`，不写 per-signal CSV。信号触发与下单关联通过策略日志追踪：`lead_lag_signal_triggered` 输出触发行情的 `trigger_ticker_id`、`trigger_exchange` 和 `trigger_symbol_id`；`lead_lag_order_intent` / `lead_lag_order_intent_rejected` 输出同一个 `trigger_ticker_id`，用于把信号和订单意图或拒绝原因对齐；成功提交后 `lead_lag_order_submitted` 输出 `local_order_id`、最终 `position_id`、`position_event`、`position_direction`、`entry_local_order_id`、`signal_role`、`order_role`、`quantity_text` 和 `price_text`，作为订单分析的主事实源。`lead_lag_order_finished` 同步输出 `position_id`、`position_direction`、`order_role`、`entry_local_order_id` 和 `order_finished_local_ns`，便于后续生成 `position.csv`。可用 `scripts/lead_lag/analyze_order_detail.py` 从 live log 生成 `order_detail.csv`，传入 `--positions-output` 时同时生成 `position.csv`。
