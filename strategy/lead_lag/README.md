...
`execute.parallel=0` 无效；大于 `16` 时 Strategy 输出 warning，并把内部 effective value clamp 为
`16`。每个 pair 使用编译期上限为 16 的 `FixedOrderedSlotPool<ExecutionGroup, 16>` 保存 active
group，slot payload 在 Strategy 初始化时按 effective parallel 一次性分配，运行期不扩容。
`(symbol_id, group_id)` 是稳定 group identity，`group_index` 只用于进程内精确定位。terminal 回报的
...
2026-07-23 已完成 `parallel=1/2/4/8/16` synthetic replay、同时间窗 Binance+Bitget 离线行情 replay、
Debug/Release/ASAN/SHM 测试和 production-like A/B benchmark；这些证据只覆盖基础设施 merge gate，
不构成 `parallel>1` 真实订单授权或 live production-readiness。
