...
### Multi-route fanout 与最小 entry quantity
...
`execute.require_min_entry_quantity` 默认 `false`，用于需要严格最小开仓量的显式 live 配置。开启后，策略在 route selection
和第一张 child 提交前，用初始化时缓存的整数 quantity units 比较实际 order price 推导数量与 instrument `min_quantity`；
不相等时输出 `lead_lag_order_intent_rejected reason=entry_quantity_not_minimum`，不提交任何 child。该 gate 只作用于 entry。
Normal close、stoploss 和 retry 必须按 group 已知总持仓生成 full-size `reduce-only` child，不能因小于或不等于 entry
`min_quantity` 而阻断平仓。Bitget 四路 live prepare 额外要求每个 Bitget pair
`order_session_fanout=4` 且 `require_min_entry_quantity=true`。
