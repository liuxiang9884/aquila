...
- `lag_freshness_scope_summary.csv`: latest lag quote freshness 的 scope / status / action / trigger 分组
- `lag_freshness_symbol_summary.csv`: latest lag quote freshness 的 symbol 分组
- `lag_freshness_bucket_summary.csv`: latest lag quote freshness bucket 与成交 / 滑点分组
- `lag_freshness_outliers.csv`: latest lag quote freshness 最大的 100 条 order
...
### Latest Lag Quote Freshness

`lag_freshness_ms = (signal_decision_ns - lag_exchange_ns) / 1e6`，用于衡量 signal 满足阈值并准备用 lag 对手价下单时，策略看到的 latest lag quote 距离 signal decision 的时间。该 run 在 freshness guard 代码生效前启动，因此本节是事后分析，不是当时的拦截结果。

| scope | count | avg_ms | p50_ms | p95_ms | p99_ms | max_ms | >20ms | >100ms | >1000ms |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| all signals | 7017 | 180.842 | 30.896 | 789.799 | 2478.972 | 15371.972 | 3980 | 2018 | 252 |
| open actions | 6837 | 182.485 | 32.205 | 791.429 | 2476.087 | 15371.972 | 3921 | 1988 | 248 |
| filled orders | 242 | 141.325 | 5.306 | 693.248 | 2913.679 | 3560.297 | 99 | 48 | 8 |
| entry orders | 6837 | 182.485 | 32.205 | 791.429 | 2476.087 | 15371.972 | 3921 | 1988 | 248 |
| exit orders | 180 | 118.462 | 2.081 | 500.241 | 2352.282 | 3558.452 | 59 | 30 | 4 |

| trigger_exchange | count | avg_ms | p50_ms | p95_ms | max_ms | >20ms |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| kBinance | 6954 | 182.477 | 32.019 | 793.368 | 15371.972 | 3980 |
| kGate | 63 | 0.398 | 0.303 | 0.857 | 2.241 | 0 |

如果事后按 `max_lag_freshness_ms=20` 过滤开仓，开仓数量和开仓成交数量变化如下。开仓只统计 `kOpenLong` / `kOpenShort`；开仓成交订单数按 `cumulative_filled_quantity > 0` 统计。

| freshness filter | open_orders | open_order_ratio | filled_open_orders | filled_open_order_ratio | open_fill_rate |
| --- | ---: | ---: | ---: | ---: | ---: |
| no freshness limit | 6837 | 100.00% | 112 | 100.00% | 1.638% |
| lag freshness <=20ms | 2916 | 42.65% | 55 | 49.11% | 1.886% |
| lag freshness >20ms | 3921 | 57.35% | 57 | 50.89% | 1.454% |

按 `20ms` 过滤后，开仓尝试会减少 `3921` 个，开仓成交订单会减少 `57` 个；剩余开仓成交率从 `1.638%` 小幅提高到 `1.886%`。

下面按 lag freshness bucket 统计成交、胜率、滑点和 PnL。`signal_count` 与本 run 的 order 数一致；`filled_order_count` 按 `cumulative_filled_quantity > 0` 统计；胜率和 PnL 按 entry order 所在 freshness bucket 归因，使用 `position.csv` 中有 `net_pnl` 的 closed / partial_closed 行，不包含 guard 停机后通过 REST emergency flatten 平掉的仓位。

| bucket | signal_count | filled_order_count | fill_rate | pnl_rows | win_rate | avg_slip_ticks | slip_quote_sum | net_pnl | raw_net_pnl |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| <=1ms | 860 | 82 | 0.095 | 28 | 0.321 | -1.497 | -1.566041 | -1.346158 | -2.841952 |
| 1-5ms | 1078 | 39 | 0.036 | 22 | 0.182 | -2.119 | -0.483249 | -0.909988 | -0.829130 |
| 5-20ms | 1099 | 22 | 0.020 | 11 | 0.545 | 0.521 | 0.213491 | 1.711144 | 1.733199 |
| 20-50ms | 1097 | 27 | 0.025 | 19 | 0.368 | 0.386 | 0.161017 | -0.591992 | -0.937914 |
| 50-100ms | 865 | 24 | 0.028 | 19 | 0.263 | 0.850 | 0.462334 | -0.876911 | -0.327253 |
| 100-500ms | 1454 | 33 | 0.023 | 22 | 0.500 | 0.277 | 0.109425 | -0.611168 | -0.623298 |
| 500-1000ms | 312 | 7 | 0.022 | 3 | 0.333 | 1.063 | 0.118504 | -0.200653 | -0.055677 |
| >1000ms | 252 | 8 | 0.032 | 6 | 0.000 | 0.731 | 0.104450 | -0.192750 | -0.045720 |

当前结论：

- lag quote stale 是真实问题，不是偶发；全量 p50 `30.896ms`、p95 `789.799ms`，开仓 `3921/6837` 个 signal 超过 `20ms`。
- `max_lag_freshness_ms=20` 是合理的第一刀；`>20ms` bucket 合计 net PnL 为负，启用 guard 可以过滤大量 stale lag quote 下单，但它不是充分盈利条件。
- freshness 越新不等于越赚钱；`<=1ms` 和 `1-5ms` bucket 成交率较高、滑点更有利，但 net PnL 分别为 `-1.346158` 和 `-0.909988`，说明亏损还与 signal 方向、symbol、fee 和触发条件有关。
- `5-20ms` 是当前最好 bucket，win rate `54.545%`、net PnL `1.711144`、raw net PnL `1.733199`；但只有 `11` 个 PnL rows，不能直接外推为稳定规律。
- `>20ms` 后整体质量明显变差，多个 stale bucket 的 net PnL 为负；`>1000ms` bucket win rate 为 `0`。
- fill rate 和 PnL 不一致；`<=1ms` fill rate `9.535%` 最高，但 PnL 亏损最明显之一，因此“更容易成交”不能直接等价为“更好的交易”。
- 下一步需要按 freshness bucket 继续拆 `symbol + action + direction`：重点解释 `<=5ms` 为什么亏、`5-20ms` 是否由少数 symbol 偶然贡献，以及 `>20ms` 中哪些 symbol 应禁用或使用更严格 freshness 阈值。

按 symbol 的 p95 看，lag quote stale 最明显的是：

| symbol | count | p50_ms | p95_ms | max_ms | open_gt_20ms |
| --- | ---: | ---: | ---: | ---: | ---: |
| CLO_USDT | 259 | 75.054 | 2961.142 | 8544.329 | 156 |
| PIEVERSE_USDT | 110 | 60.566 | 2583.716 | 6152.052 | 73 |
| STO_USDT | 175 | 108.128 | 1991.393 | 6497.822 | 119 |
| XPL_USDT | 53 | 48.147 | 1799.641 | 9571.870 | 35 |
| MYX_USDT | 208 | 78.663 | 1688.706 | 4764.529 | 142 |
| ICP_USDT | 67 | 81.150 | 1632.881 | 1889.363 | 51 |
| AIA_USDT | 452 | 58.157 | 1412.186 | 15371.972 | 289 |
| INJ_USDT | 154 | 60.720 | 1346.425 | 5646.143 | 99 |
| UB_USDT | 202 | 44.978 | 966.029 | 7667.971 | 120 |
| SLX_USDT | 259 | 33.183 | 929.862 | 6935.335 | 153 |

结论：旧 run 中 Binance 触发 signal 时，latest lag quote 经常明显 stale；以当前 `max_lag_freshness_ms=20` 事后估算，`3921/6837` 个 open action 会被拦截。后续 30-symbol live 必须用包含 freshness guard 的最新 binary 重启，并在 report 中优先检查 `lag_freshness_ns`、`freshness_guard_pass` 和 `freshness_reject_reason`。
