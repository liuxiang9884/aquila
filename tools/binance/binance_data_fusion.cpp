...
class BinanceDataFusionSourceWorker {
 public:
  explicit BinanceDataFusionSourceWorker(aq_binance::DataSessionConfig config)
      : publisher_(config.data_shm), session_(std::move(config), publisher_) {}

  ~BinanceDataFusionSourceWorker() {
    Stop();
    Join();
  }
...
int RunConnected(const aq_tool::BinanceDataFusionConfig& launch_config,
  ...
  std::vector<std::unique_ptr<BinanceDataFusionSourceWorker<WebSocketPolicy>>>
      workers;
  workers.reserve(sources.size());
  for (PreparedBinanceSource& source : sources) {
    workers.push_back(
        std::make_unique<BinanceDataFusionSourceWorker<WebSocketPolicy>>(
  ...
  while (!signal_stop_requested.load(std::memory_order_relaxed)) {
    ...
    for (const std::unique_ptr<BinanceDataFusionSourceWorker<WebSocketPolicy>>&
             worker : workers) {
      if (worker->finished()) {
        unexpected_stop = true;
        break;
