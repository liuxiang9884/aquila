...
[[nodiscard]] BinanceDataFusionConfigResult Success(
    BinanceDataFusionConfig config) {
  BinanceDataFusionConfigResult result;
...
class Parser {
 public:
  ...
  [[nodiscard]] BinanceDataFusionConfigResult Parse() {
    ParseLaunch();
    if (!ok_) {
 ...
 private:
  ...
  void ParseSources() {
    ...
    for (const toml::node& source_node : *sources) {
      ...
      BinanceDataFusionSourceConfig source;
      source.source_id =
  ...
  [[nodiscard]] bool HasSourceId(std::int32_t source_id) const noexcept {
    for (const BinanceDataFusionSourceConfig& source : config_.sources) {
  ...
  BinanceDataFusionConfig config_;
  std::string error_;
  bool ok_{true};
};
