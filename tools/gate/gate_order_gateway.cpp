...
namespace aq_ws = aquila::websocket;
...
struct PreparedRoute {
  ...
  aq_gate::OrderSessionConfig order_session_config;
  aq_gate::LoginCredentials credentials;
};

[[nodiscard]] bool BuildCredentials(const aq_gate::OrderSessionConfig& config,
                                    aq_gate::LoginCredentials* credentials,
                                    std::string* error) {
...
[[nodiscard]] bool LoadPreparedRoutes(
  ...
  for (const aq_config::OrderGatewayRouteConfig& route_config :
    ...
    aq_gate::OrderSessionConfigResult order_session_result =
        aq_gate::LoadOrderSessionConfigFile(
    ...
    if (require_credentials) {
      if (!BuildCredentials(order_session_result.value, &credentials, error)) {
        return false;
      }
    }
...
void BindCurrentThreadToCpu(int cpu_id) noexcept {
  ...
  if (rc != 0 && ::nova::kLogManager.logger() != nullptr) {
    NOVA_WARNING("gate_order_gateway_bind_cpu_failed cpu_id={} error={}",
                 cpu_id, std::strerror(rc));
  }
#else
  if (::nova::kLogManager.logger() != nullptr) {
    NOVA_WARNING("gate_order_gateway_bind_cpu_unsupported cpu_id={}", cpu_id);
  }
#endif
}
...
void LogDryRun(const aq_config::OrderGatewayConfig& gateway_config,
  ...
  NOVA_INFO(
      "gate_order_gateway_dry_run name={} shm_name={} route_count={} "
...
class GateOrderGatewayRouteWorker {
 public:
  using Session =
      aq_gate::OrderSession<aq_gate::OrderGatewaySessionEventHandler,
                            WebSocketPolicy,
                            aq_gate::NoopOrderSessionDiagnostics>;
  using CommandWorker = aq_gate::OrderGatewayCommandWorker<Session>;
  ...
  GateOrderGatewayRouteWorker(std::uint16_t route_id, int worker_cpu_id,
      ...
      : worker_cpu_id_(worker_cpu_id),
        ...
        session_(std::move(config.connection), std::move(credentials), handler_,
                 ...
                 aq_gate::OrderSessionSocketDiagnosticsConfig{
  ...
  GateOrderGatewayRouteWorker(const GateOrderGatewayRouteWorker&) = delete;
  GateOrderGatewayRouteWorker& operator=(const GateOrderGatewayRouteWorker&) =
      delete;
 ...
 private:
  static void RuntimeHook(void* raw) noexcept {
    auto* self = static_cast<GateOrderGatewayRouteWorker*>(raw);
  ...
  aq_gate::OrderGatewayWorkerPublisher publisher_;
  aq_gate::OrderGatewaySessionEventHandler handler_;
  Session session_;
...
int RunConnected(const aq_config::OrderGatewayConfig& gateway_config,
  ...
  std::vector<std::unique_ptr<GateOrderGatewayRouteWorker<WebSocketPolicy>>>
      workers;
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (routes.front().order_session_config.connection.enable_tls) {
      return RunConnected<aq_gate::OrderSessionDefaultTlsWebSocketPolicy>(
          gateway_result.value, std::move(routes), remove_existing_shm,
          max_runtime_ms);
    }
    return RunConnected<aq_gate::OrderSessionDefaultPlainWebSocketPolicy>(
        gateway_result.value, std::move(routes), remove_existing_shm,
        max_runtime_ms);
  } catch (const std::exception& exc) {
    fmt::print(stderr, "gate_order_gateway failed: {}\n", exc.what());
    return 1;
  }
}
