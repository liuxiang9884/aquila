...
class CycleScheduler {
 public:
  explicit CycleScheduler(CycleSchedulerOptions options)
    ...
    if (options_.samples_per_session == 0) {
      options_.samples_per_session = 1;
    }
    (void)options_.cycles_per_connection_generation;
  }

  [[nodiscard]] bool HasNextCycle() const noexcept {
    return !options_.connections.empty() &&
           next_cycle_index_ < options_.samples_per_session;
  }
  ...
  [[nodiscard]] ProbeCycle NextCycle() {
    ProbeCycle cycle{
        ...
        .group_index = 0,
        .connections = options_.connections,
    };
    return cycle;
  }
