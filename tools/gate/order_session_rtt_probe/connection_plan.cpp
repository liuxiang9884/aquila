...
[[nodiscard]] bool IsNumericIpAddress(std::string_view text) {
...
[[nodiscard]] bool NonEmpty(std::string_view text) noexcept {
  return !text.empty();
}

[[nodiscard]] bool ParseWorkerCpuId(std::string_view text,
...
ProbeConnectionsCsvResult LoadProbeConnectionsCsvFile(
  ...
  try {
    ...
    for (csv::CSVRow& row : reader) {
      ...
      const std::string enable_tls_text = ReadString(row, "enable_tls");
      if (!NonEmpty(connection.name) || !NonEmpty(connection.group) ||
          !NonEmpty(connection.host) || !NonEmpty(connection.connect_ip) ||
          !NonEmpty(connection.port)) {
      ...
      if (!IsNumericIpAddress(connection.connect_ip)) {
        return Failure(
            fmt::format("connections CSV row {} has invalid connect_ip '{}'",
                        row_index, connection.connect_ip));
      }
      if (!ParseBool(enable_tls_text, &connection.enable_tls)) {
        return Failure(fmt::format(
            "connections CSV row {} enable_tls must be true or false",
            row_index));
      }
      if (!ParseWorkerCpuId(ReadString(row, "worker_cpu_id"),
