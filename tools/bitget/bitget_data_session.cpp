...
void PrintSession(const SessionT& session) {
  ...
  for (const aq_bitget::SymbolBinding& symbol : session.symbols()) {
...
int RunDataSessionWithSink(aq_bitget::DataSessionConfig data_session_config,
                           DataSink& data_sink, bool connect) {
  using Session =
      aq_bitget::DataSession<DataSink, WebSocketPolicy,
...
int RunDataSession(aq_bitget::DataSessionConfig data_session_config,
                   bool connect) {
...
int main(int argc, char** argv) {
  ...
  CLI::App app{"Bitget data session"};
  ...
  aq_bitget::DataSessionConfigResult config_result =
      aq_bitget::ParseDataSessionConfig(toml, config_path);
  if (!config_result.ok) {
  ...
  if (config_result.value.connection.enable_tls) {
    return RunDataSession<aq_bitget::DefaultTlsWebSocketPolicy>(
        std::move(config_result.value), connect);
  }
  return RunDataSession<aq_bitget::DefaultPlainWebSocketPolicy>(
      std::move(config_result.value), connect);
}
