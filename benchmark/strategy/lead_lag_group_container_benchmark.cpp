...
class FixedOrderedSlotPoolGroupContainer {
 public:
  ...
  void Reset(std::uint16_t active_count,
             ScanScenario scenario = ScanScenario::kNoTrigger) noexcept {
    [[maybe_unused]] const std::size_t initialized =
        groups_.Initialize(kCapacity);
    active_count = std::min(active_count, kCapacity);
    next_group_id_ = 1;
    for (std::uint16_t i = 0; i < active_count; ++i) {
      if (!InsertHoldGroup()) {
        break;
  ...
  [[nodiscard]] std::uint16_t active_group_count() const noexcept {
    return static_cast<std::uint16_t>(groups_.active_count());
  }

  template <typename Fn>
  void ForEachActive(Fn&& fn) const noexcept {
    for (const std::uint16_t index : groups_.active_indices()) {
      if (!fn(index, groups_.At(index))) {
        break;
      }
    }
  }
  ...
  [[nodiscard]] std::uint64_t ClearMiddleThenInsert() noexcept {
    return ClearActiveOffsetAndInsert(active_group_count() / 2U);
  }

  [[nodiscard]] std::uint64_t ClearTailThenInsert() noexcept {
    return ClearActiveOffsetAndInsert(active_group_count() - 1U);
  }

 private:
  [[nodiscard]] bool InsertHoldGroup() noexcept {
    const std::uint16_t group_ordinal =
        static_cast<std::uint16_t>(next_group_id_ - 1U);
    const std::uint16_t index =
        groups_.EmplaceBack(MakeHoldGroup(next_group_id_, group_ordinal));
    if (index == Slots::kInvalidIndex) {
      return false;
    }
    ++next_group_id_;
    return true;
  ...
  [[nodiscard]] std::uint16_t IndexAtActiveOffset(
      std::uint16_t offset) const noexcept {
    const std::span<const std::uint16_t> active_indices =
        groups_.active_indices();
    if (offset >= active_indices.size()) {
      return kInvalidIndex;
    }
    return active_indices[offset];
  }

  [[nodiscard]] std::uint64_t ClearActiveOffsetAndInsert(
      std::uint16_t offset) noexcept {
    if (active_group_count() == 0 || offset >= active_group_count()) {
      return 0;
    }
    const std::uint16_t removed_index = IndexAtActiveOffset(offset);
    const std::uint64_t removed_group_id = groups_.At(removed_index).group_id;
    [[maybe_unused]] const bool erased = groups_.Erase(removed_index);
    const bool inserted = InsertHoldGroup();
    std::uint64_t checksum = MixChecksum(kChecksumOffset, removed_group_id);
    checksum = MixChecksum(checksum, inserted ? next_group_id_ : 0U);
    checksum = MixChecksum(checksum, active_group_count());
    return checksum;
  }

  void ApplyScenario(ScanScenario scenario) noexcept {
    if (scenario == ScanScenario::kNoTrigger || active_group_count() == 0) {
      return;
    }
    std::uint16_t offset = 0;
    switch (scenario) {
      ...
      case ScanScenario::kTriggerFirst:
        ...
        break;
      case ScanScenario::kTriggerMiddle:
        offset = active_group_count() / 2U;
        break;
      case ScanScenario::kTriggerLast:
        offset = active_group_count() - 1U;
        break;
    }
    MakeGroupTrigger(groups_.At(IndexAtActiveOffset(offset)));
  }
