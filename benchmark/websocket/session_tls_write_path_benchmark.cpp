...
void BenchmarkSessionTlsWritePathWithEncode(benchmark::State& state) {
  aquila::websocket::test::TlsBlackholeServer server({
      aquila::websocket::test::TlsServerAction::kHandshake101ThenDrain,
  });
  if (!server.Start()) {
  ...
  state.counters["drained_bytes"] =
      static_cast<double>(server.drained_bytes());
  state.SetLabel(BuildBenchmarkLabel(true, "local-tls-drain-with-encode",
                                     FormatAffinity(),
                                     FormatSchedulingPolicy()));
}

BENCHMARK(BenchmarkSessionTlsWritePathWithEncode)
    ->Name("session_tls_write_path_with_encode")
    ->Iterations(4096)
    ->UseManualTime()
    ->Unit(benchmark::kNanosecond);
