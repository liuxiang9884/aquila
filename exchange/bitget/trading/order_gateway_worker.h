#ifndef AQUILA_EXCHANGE_BITGET_TRADING_ORDER_GATEWAY_WORKER_H_
#define AQUILA_EXCHANGE_BITGET_TRADING_ORDER_GATEWAY_WORKER_H_
...
ToOrderGatewayRejectReason(OrderSendStatus status) noexcept {
  ...
  switch (status) {
    ...
    case OrderSendStatus::kOrderIdCacheFull:
      return Reason::kInflightFull;
    case OrderSendStatus::kInvalidSymbol:
    case OrderSendStatus::kInvalidQuantityText:
    case OrderSendStatus::kInvalidPriceText:
...
class OrderGatewayCommandWorker {
 ...
 private:
  ...
  void Cancel(const core::OrderGatewayCommand& command,
    ...
    const core::StrategyOrder order = ToStrategyOrder(command);
    const bitget::OrderSendResult sent = session_->CancelOrder(order);
    HandleSendResult(command, sent, worker_dequeue_ns);
  }

  void HandleSendResult(const core::OrderGatewayCommand& command,
                        const bitget::OrderSendResult& sent,
                        std::int64_t worker_dequeue_ns) noexcept {
...
#endif  // AQUILA_EXCHANGE_BITGET_TRADING_ORDER_GATEWAY_WORKER_H_
