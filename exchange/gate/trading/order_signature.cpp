...
bool WriteHmacSha512Hex(
    std::string_view api_secret, std::string_view sign_input,
  ...
  const unsigned char* const hmac =
      HMAC(EVP_sha512(), api_secret.data(), static_cast<int>(api_secret.size()),
           ...
           sign_input.size(), digest.data(), &digest_size);
...
bool GenerateGateApiSignatureHex(
  ...
  return WriteHmacSha512Hex(
      api_secret, std::string_view(sign_input.data(), sign_input_result.size),
      output);
}

bool GenerateGateChannelSignatureHex(
    std::string_view api_secret, std::string_view channel,
    std::string_view event, std::int64_t timestamp,
    std::array<char, kGateSignatureHexSize>& output) noexcept {
  std::array<char, kGateSignatureInputBufferSize> sign_input;
  const auto sign_input_result = fmt::format_to_n(
      sign_input.data(), sign_input.size(), "channel={}&event={}&time={}",
