...
[[nodiscard]] TradeFusionConfigResult Failure(std::string error) {
  TradeFusionConfigResult result;
  result.error = std::move(error);
  return result;
}

[[nodiscard]] TradeFusionConfigResult Success(md::TradeFusionConfig config) {
  TradeFusionConfigResult result;
...
class Parser {
 public:
  ...
  [[nodiscard]] TradeFusionConfigResult Parse() {
    ParseFusion();
    if (!ok_) {
 ...
 private:
  ...
  void ParseSources() {
    ...
    for (const toml::node& source_node : *sources) {
      ...
      md::TradeFusionSourceConfig source;
      source.source_id =
      ...
      source.channel_name =
          OptionalString((*source_table)["channel_name"], "trade_channel");
  ...
  [[nodiscard]] bool HasSourceId(std::int32_t source_id) const noexcept {
    for (const md::TradeFusionSourceConfig& source : config_.sources) {
...
TradeFusionConfigResult LoadTradeFusionConfigFile(
  ...
  try {
    ...
    return ParseTradeFusionConfig(parsed);
  } catch (const std::exception& exc) {
    return Failure(std::string{"failed to load trade fusion config: "} +
                   exc.what());
  }
}
