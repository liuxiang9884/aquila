...
`probe.order.order_mode` 选择 `ioc`、`gtc` 或默认 `ioc+gtc`；`probe.sampling.order_session_interval_us=0` 表示同一
cycle 内下一个 order session 等下一条行情触发，非 0 表示发完当前 order session 后设置微秒粒度非阻塞 deadline，到点后用当时最新
...
`scripts/gate/` 和 `scripts/binance/` 按交易所和职责分目录。Gate 使用
`market_data/`、`account/`、`trading/`、`diagnostics/`；Binance 当前使用 `market_data/`。跨交易所工具保留在
`scripts/instruments/`、`scripts/market_data/`、`scripts/lead_lag/` 和 `scripts/tardis/`。
...
```bash
scripts/gate/trading/place_futures_order.py \
  --contract BTC_USDT \
  --side buy \
  --size 1 \
  --price 1 \
