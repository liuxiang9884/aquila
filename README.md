...
`local_order_id` 分配和 sample CSV writer 已作为 live sample 前置逻辑落地。可用 `--live-preflight` 加载 candidate IP 和
base order session config，构建 single-session live plan 并打印输出路径；该模式不连接 WebSocket、不下单。`--execute`
仍会显式失败。
