...
TEST(BookTickerFusionLaunchConfigTest, ParsesFourSources) {
...
bind_cpu_id = 19
  ...
  const auto result =
      aquila::tools::market_data::ParseBookTickerFusionLaunchConfig(parsed);

  ASSERT_TRUE(result.ok) << result.error;
  EXPECT_EQ(result.value.name, "gate_book_ticker_fusion_launch_4sources");
  ...
  EXPECT_EQ(result.value.sources[3].bind_cpu_id, 19);
}

TEST(BookTickerFusionLaunchConfigTest, RejectsDuplicateSourceId) {
  const toml::parse_result parsed = ParseToml(R"toml(
[launch]
name = "gate_book_ticker_fusion_launch"
fusion_config = "config/market_data_fusion/gate_book_ticker_fusion_4sources.toml"
