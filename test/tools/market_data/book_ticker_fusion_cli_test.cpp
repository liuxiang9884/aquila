...
TEST(BookTickerFusionCliTest, LogsMissingConfigLoadError) {
  const std::filesystem::path missing_config{
      AQUILA_MISSING_BOOK_TICKER_FUSION_CONFIG};
  std::error_code error;
  std::filesystem::remove(missing_config, error);
  ASSERT_FALSE(std::filesystem::exists(missing_config));

  const std::string command =
      ShellQuote(AQUILA_GATE_BOOK_TICKER_FUSION_BINARY) + " --config " +
