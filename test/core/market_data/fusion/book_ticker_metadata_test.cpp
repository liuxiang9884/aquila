...
std::vector<aquila::market_data::FusionMetadataRecord> ReadRecords(
    const std::filesystem::path& path) {
  const std::uintmax_t size = std::filesystem::file_size(path);
  EXPECT_EQ(size % sizeof(aquila::market_data::FusionMetadataRecord), 0U);
  std::vector<aquila::market_data::FusionMetadataRecord> records(
      size / sizeof(aquila::market_data::FusionMetadataRecord));
  ...
  if (!records.empty()) {
    input.read(reinterpret_cast<char*>(records.data()),
               static_cast<std::streamsize>(
                   records.size() *
                   sizeof(aquila::market_data::FusionMetadataRecord)));
    EXPECT_TRUE(input.good());
  }
  return records;
}

void ExpectRecordEq(const aquila::market_data::FusionMetadataRecord& actual,
...
TEST(BookTickerFusionMetadataWriterTest, WritesFixedSizeMetadataRecords) {
  ...
  {
    aquila::market_data::FusionMetadataWriter writer(output_path);
    ASSERT_TRUE(writer.Write(first));
    ASSERT_TRUE(writer.Write(second));
    ASSERT_TRUE(writer.Flush());
  }

  const std::vector<aquila::market_data::FusionMetadataRecord> records =
